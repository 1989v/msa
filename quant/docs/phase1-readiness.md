...
**Phase**: 1 (백테스트 엔진)
**작성일**: 2026-04-26
**기준 ADR**: [ADR-0024](../../docs/adr/ADR-0024-quant-service.md) (Status: Proposed)
...
### 빌드 & 테스트
- [x] `./gradlew :quant:domain:test` — 35 tests green (도메인 + INV-01~07 property)
- [x] `./gradlew :quant:app:test` — UseCase / Backtest / Bithumb / Outbox / Maskng 모두 green
- [x] `./gradlew :quant:app:bootJar` — 실행 JAR 생성 성공
- [x] `cd quant/frontend && npm run build` — FE 번들 성공 (541 KB JS / 14 KB CSS, gzip 170 / 3.86 KB)
- [ ] `./gradlew :quant:app:goldenTest` — Phase 2 도입 (TG-12 deferred)
- [ ] `./gradlew :quant:app:test --tests '*Phase1E2ESpec*' -PincludeIntegration=true` — Docker 환경에서 1회 수동 실행 (TG-15)
...
- [x] `TrancheSlot.fillSell` precondition → `StopLossAttemptException` (INV-01)
- [x] 민감정보 (`ExchangeCredential`, `DecryptedCredential`) `toString()` 마스킹 검증

### 인프라
- [x] Flyway V001 — 9 테이블 (모두 tenant_id + 인덱스)
- [x] ClickHouse `quant` DB 별도 신설 (analytics와 격리)
- [x] Outbox 패턴 (`OutboxEventPublisher` + `OutboxRelay`)
- [x] K8s base manifest 등록 (`k8s/base/quant/`) + k3s-lite overlay 자동 포함
- [ ] `kubectl apply -k k8s/overlays/k3s-lite` 후 quant Pod Ready 검증 (수동)
- [ ] `/actuator/health`, `/actuator/prometheus` 정상 응답 검증 (수동)

### 데이터
- [ ] 빗썸 분봉 2023-01 ~ 현재 BTC/KRW, ETH/KRW 적재 완료 (수동 1회 실행 필요)
  - 명령: `./gradlew :quant:app:bootRun --args='--spring.profiles.active=ingest-bithumb'`
- [x] 백테스트 골든셋 fixture 3종 (tight / volatile / exhausted) green

### 문서
- [x] `quant/CLAUDE.md` 존재 + 루트 Navigation 갱신
- [x] `quant/docs/README.md` 작성
- [x] [ADR-0024](../../docs/adr/ADR-0024-quant-service.md) Proposed 상태로 작성됨
- [x] [Spec](../../docs/specs/2026-04-24-quant-crypto-trading/planning/spec.md) revised 상태
- [x] [Requirements](../../docs/specs/2026-04-24-quant-crypto-trading/planning/requirements.md) v1
- [x] [Test Quality](../../docs/specs/2026-04-24-quant-crypto-trading/planning/test-quality.md)
- [x] [Phase 1 Release Notes](../../docs/specs/2026-04-24-quant-crypto-trading/planning/phase1-release-notes.md)
