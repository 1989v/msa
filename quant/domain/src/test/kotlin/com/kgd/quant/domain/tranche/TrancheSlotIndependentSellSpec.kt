 ...
 */
class TrancheSlotIndependentSellSpec : BehaviorSpec({
        ...
        When("슬롯 i 의 evaluateSellTrigger 를 평가하면") {
            Then("슬롯 j 의 entryPrice 에 무관하게 슬롯 i 의 entryPrice/tp 만으로 결정된다") {
                ...
                checkAll(25, arbEntryI, arbEntryJ, arbTp, arbDelta) { eI, eJ, tp, deltaPct ->
                    ...
                    val slotI = TrancheFixtures.newSlot(
                        runId = runId,
                        roundIndex = 0,
                    ...
                    val slotJ = TrancheFixtures.newSlot(
                        runId = runId,
                        roundIndex = 1,
                        takeProfitPercent = tpPercent,
                        targetQty = Quantity.of("1")
    ...
    Given("슬롯 i 의 entryPrice 만 변경하고 j 는 고정") {
        When("동일 currentPrice 로 두 슬롯 각각 평가하면") {
            Then("슬롯 i 결과는 i 의 entryPrice 변화에만 반응한다") {
                ...
                val slotI = TrancheFixtures.newSlot(runId = runId, roundIndex = 0, takeProfitPercent = tp)
                val slotJ = TrancheFixtures.newSlot(runId = runId, roundIndex = 1, takeProfitPercent = tp)
