            ...
            Then("논리적으로 하나의 주문만 남는다") {
                ...
                checkAll(15, Arb.uuid(), Arb.int(2..10)) { uuid, attempts ->
                    ...
                    val events = (1..attempts).map {
                        OrderPlaced(
                            tenantId = com.kgd.quant.domain.common.TenantId("tenant-1"),
                            orderId = orderId,
                            slotId = slotId,
                            side = OrderSide.BUY,
    ...
    Given("OrderPlaced 와 OrderFilled 이벤트") {
        When("동일 orderId 를 기준으로 이벤트 스트림을 필터링") {
            Then("동일 orderId 로만 매칭되어 다른 주문과 섞이지 않는다") {
                ...
                val tenant = com.kgd.quant.domain.common.TenantId("tenant-1")
                val slotId = SlotId.newId()
                val events = listOf<com.kgd.quant.domain.event.DomainEvent>(
                    OrderPlaced(
