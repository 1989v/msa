...
import com.kgd.quant.domain.exception.StopLossAttemptException

/**
 * TrancheSlot — 분할 매수의 한 회차를 표현하는 Entity.
 *
...
class TrancheSlot internal constructor(
    ...
    fun requestBuy(price: Price) {
        state.ensureTransition(TrancheSlotState.PENDING_BUY)
        this.entryPrice = price
        this.state = TrancheSlotState.PENDING_BUY
    }

    /** 매수 체결 반영. partial fill은 targetQty 도달 전까지 PENDING_BUY 유지. */
    fun fillBuy(executedPrice: Price, executedQty: Quantity) {
        if (state != TrancheSlotState.PENDING_BUY) {
        ...
        if (newFilledQty >= targetQty) {
            ...
            state.ensureTransition(TrancheSlotState.FILLED)
            this.state = TrancheSlotState.FILLED
        }
    }

    /** 매수 주문이 취소/실패했을 때 EMPTY 로 되돌린다. */
    fun cancelBuy() {
        state.ensureTransition(TrancheSlotState.EMPTY)
        this.entryPrice = null
        this.filledQty = Quantity.ZERO
        this.state = TrancheSlotState.EMPTY
    }

    /** 매도 주문 요청 — FILLED 에서만 허용. */
    fun requestSell() {
        state.ensureTransition(TrancheSlotState.PENDING_SELL)
        this.state = TrancheSlotState.PENDING_SELL
    }
    ...
    fun fillSell(executedPrice: Price) {
        if (state != TrancheSlotState.PENDING_SELL) {
            throw IllegalSlotTransitionException(
        ...
        state.ensureTransition(TrancheSlotState.CLOSED)
        this.state = TrancheSlotState.CLOSED
    }
    ...
    fun evaluateSellTrigger(currentPrice: Price): Boolean {
        if (state != TrancheSlotState.FILLED) return false
    ...
    companion object {
        ...
        ): TrancheSlot = TrancheSlot(
            id = id,
            runId = runId,
            roundIndex = roundIndex,
            state = TrancheSlotState.EMPTY,
            entryPrice = null,
            targetQty = targetQty,
