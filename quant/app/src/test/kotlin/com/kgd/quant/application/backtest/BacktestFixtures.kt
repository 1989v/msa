 ...
 * - `TrancheStrategy` / `StrategyRun` / `TrancheSlot` 전부 Repository 복원 factory 를 써서
...
object BacktestFixtures {
    ...
    fun defaultConfig(): TrancheStrategyConfig = TrancheStrategyConfig(
        roundCount = 7,
    ...
    fun activeStrategy(config: TrancheStrategyConfig = defaultConfig()): TrancheStrategy {
    ...
    fun emptySlots(config: TrancheStrategyConfig, runId: RunId): MutableList<TrancheSlot> {
        val result = mutableListOf<TrancheSlot>()
