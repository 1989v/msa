 ...
 * 새 매수(`OrderPlaced` with [OrderSide.BUY], `TrancheSlotOpened`) 이벤트가 발행되지 않음을 검증.
 *
 * Principle (FR-ENG-06):
...
class AwaitingExhaustedScenarioSpec : BehaviorSpec({
    ...
    Given("지속 하락 exhausted fixture 를 실행하면") {
        When("전체 결과를 확인한다") {
            ...
            Then("TrancheSlotOpened 이벤트도 7 회만 발행된다") {
                val result = runBlocking { BacktestRunner.execute("exhausted", seed = 7L) }
                val opened = result.events.filterIsInstance<TrancheSlotOpened>()
