    ...
    Given("tight CSV fixture 와 동일 seed(42) 를 2 회 실행한다") {
        ...
        When("tight 시나리오는 1 회차만 진입한다") {
            Then("정확히 1 건의 BUY OrderPlaced 이벤트가 발행된다") {
                ...
                val buyPlacements = result.events.filterIsInstance<com.kgd.quant.domain.event.OrderPlaced>()
                    .filter { it.side == com.kgd.quant.domain.order.OrderSide.BUY }
                buyPlacements shouldHaveSize 1
                // tight 시나리오는 TP 에 도달하지 않으므로 체결 SELL 이 0 건
                result.events.filterIsInstance<com.kgd.quant.domain.event.TrancheSlotClosed>() shouldHaveSize 0
            }
        }
    }
