package com.kgd.quant.application.paper.service
 ...
 */
@Component
class StartPaperTradingService(
...
) : StartPaperTradingUseCase {

    override suspend fun execute(command: StartPaperTradingCommand): RunId {
        // 1. 전략 조회 + PAPER 모드 검증
        ...
        check(strategy.executionMode == ExecutionMode.PAPER) {
            "StartPaperTradingService: strategy executionMode must be PAPER but was ${strategy.executionMode} (strategyId=${strategy.id})"
        }

        // 2. PaperAccount 초기화 (없으면 default 잔고 생성)
        val initialBalance = command.initialBalance ?: StartPaperTradingUseCase.DEFAULT_INITIAL_BALANCE
