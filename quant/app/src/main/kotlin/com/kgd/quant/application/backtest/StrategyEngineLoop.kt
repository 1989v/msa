...
import com.kgd.quant.domain.event.OrderPlaced
...
class StrategyEngineLoop(
    ...
    private suspend fun processSellTriggers(
        ...
        val triggered = slots
            .filter { it.state == TrancheSlotState.FILLED && it.evaluateSellTrigger(currentPrice) }
    ...
    private suspend fun processBuyTrigger(
        ...
        val emptySlots = slots.filter { it.state == TrancheSlotState.EMPTY }
        if (emptySlots.isEmpty()) return

        val lastFilled = slots
            .filter { it.state == TrancheSlotState.FILLED && it.entryPrice != null }
    ...
    private fun syncAwaitingExhaustedStatus(
        ...
        val allFilled = slots.all { it.state == TrancheSlotState.FILLED }
        when {
    ...
    companion object {
        ...
        fun realizedPnlFromEvents(events: List<com.kgd.quant.domain.event.DomainEvent>): BigDecimal {
            var pnl = BigDecimal.ZERO
            for (e in events) {
                if (e is TrancheSlotClosed) {
                    pnl = pnl.add(e.pnl)
                }
            }
            return pnl
