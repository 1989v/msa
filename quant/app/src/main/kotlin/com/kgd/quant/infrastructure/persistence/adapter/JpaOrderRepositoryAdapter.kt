...
import com.kgd.quant.infrastructure.persistence.mapper.OrderMapper
import com.kgd.quant.infrastructure.persistence.repository.OrderJpaRepository
...
class JpaOrderRepositoryAdapter(
    ...
    private val slotJpa: TrancheSlotJpaRepository,
        ...
        val tenantId: TenantId = if (existing != null) {
            ...
            val slot = slotJpa.findById(order.slotId.value).orElse(null)
                ?: throw IllegalStateException(
                    "Order cannot be persisted before its parent TrancheSlot (slotId=${order.slotId.value})"
                )
