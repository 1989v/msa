...
import com.kgd.quant.infrastructure.persistence.repository.TrancheStrategyJpaRepository
 ...
 * - `save` 는 도메인 `TrancheStrategy.tenantId` 를 그대로 저장. 변경 시 도메인 메서드가 책임.
 */
@Component
class JpaStrategyRepositoryAdapter(
    private val jpa: TrancheStrategyJpaRepository,
    private val mapper: TrancheStrategyMapper,
    private val clock: Clock
) : StrategyRepositoryPort {

    override suspend fun save(strategy: TrancheStrategy): TrancheStrategy = withContext(Dispatchers.IO) {
        val now = clock.now()
    ...
    override suspend fun findById(tenantId: TenantId, id: StrategyId): TrancheStrategy? =
        withContext(Dispatchers.IO) {
            jpa.findByStrategyIdAndTenantId(id.value, tenantId.value)
                ?.let { mapper.toDomain(it) }
        }

    override suspend fun findAll(tenantId: TenantId): List<TrancheStrategy> =
        withContext(Dispatchers.IO) {
