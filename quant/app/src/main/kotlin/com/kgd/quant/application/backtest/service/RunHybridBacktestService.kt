...
import com.kgd.quant.application.backtest.usecase.RunHybridBacktestUseCase
...
/** RunHybridBacktestService — [RunHybridBacktestUseCase] 구현. 평가 규칙은 인터페이스 문서 참조 */
@Component
class RunHybridBacktestService(
...
) : RunHybridBacktestUseCase {
    override suspend fun execute(
        strategy: HybridStrategy,
        from: Instant,
        to: Instant,
        interval: String,
    ): RunHybridBacktestUseCase.HybridBacktestSummary {
        ...
        val rounds = entryBarIndices.firstOrNull()?.let { firstIdx ->
            (0 until roundCount).mapNotNull { round ->
                ...
                if (barIdx < bars.size) RunHybridBacktestUseCase.RoundFill(round, bars[barIdx].ts, bars[barIdx].close) else null
            }
        } ?: emptyList()
        ...
        return RunHybridBacktestUseCase.HybridBacktestSummary(
    ...
    companion object {
        private val EMPTY = RunHybridBacktestUseCase.HybridBacktestSummary(
            strategyId = "",
            from = Instant.EPOCH,
            to = Instant.EPOCH,
            barCount = 0,
