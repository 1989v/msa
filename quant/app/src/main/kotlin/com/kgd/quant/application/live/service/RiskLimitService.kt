...
class RiskLimitService(
...
) : ManageRiskLimitUseCase {
    override suspend fun limitOrDefault(tenantId: TenantId, defaultUserId: Long, at: Instant): RiskLimit {
    ...
    override suspend fun update(limit: RiskLimit) = repo.save(limit)

    /**
     * 단일 주문 placement 직전 평가 — 사전 게이트.
     * 차단되면 ManageRiskLimitUseCase.PreOrderResult.Reject 반환. 통과 시 Allow.
     */
    override suspend fun evaluatePreOrder(
        tenantId: TenantId,
        orderKrw: BigDecimal,
    ): ManageRiskLimitUseCase.PreOrderResult {
        val limit = repo.findByTenantId(tenantId)
            ?: return ManageRiskLimitUseCase.PreOrderResult.Reject(SuspendReason.DAILY_VOLUME_LIMIT, "RiskLimit not configured")
        if (limit.singleOrderExceeds(orderKrw)) {
            return ManageRiskLimitUseCase.PreOrderResult.Reject(SuspendReason.DAILY_VOLUME_LIMIT, "single-order-max breach")
        }
        val today = LocalDate.now(clock)
        val now = metrics.snapshot(tenantId, today)
        if (limit.lossBreached(now.lossKrw)) {
            return ManageRiskLimitUseCase.PreOrderResult.Reject(SuspendReason.DAILY_LOSS_LIMIT, "daily-loss-limit breach")
        }
        if (limit.volumeBreached(now.volumeKrw.add(orderKrw))) {
            return ManageRiskLimitUseCase.PreOrderResult.Reject(SuspendReason.DAILY_VOLUME_LIMIT, "daily-volume-limit breach")
        }
        return ManageRiskLimitUseCase.PreOrderResult.Allow(limit, now)
    }
     ...
     */
    override suspend fun recordOrderAndCheck(
        tenantId: TenantId,
        orderKrw: BigDecimal,
        pnlKrw: BigDecimal,
    ): SuspendReason? {
