...
class LiveModeService(
...
) : ManageLiveModeUseCase {
    override suspend fun current(tenantId: TenantId): LiveTradingMode = repo.findByTenantId(tenantId)

    override suspend fun enable(tenantId: TenantId, userId: Long, twoFaTokenHash: String): ManageLiveModeUseCase.ToggleResult {
        if (!tokenStore.redeem(userId, twoFaTokenHash)) {
            return ManageLiveModeUseCase.ToggleResult.TwoFaRequired
        }
        val now = Instant.now()
        ...
        return ManageLiveModeUseCase.ToggleResult.Ok(state)
    }

    override suspend fun disable(tenantId: TenantId, userId: Long, twoFaTokenHash: String): ManageLiveModeUseCase.ToggleResult {
        if (!tokenStore.redeem(userId, twoFaTokenHash)) {
            return ManageLiveModeUseCase.ToggleResult.TwoFaRequired
        }
        val now = Instant.now()
     ...
     */
    override suspend fun suspend(tenantId: TenantId, reason: SuspendReason, by: Long?) {
        val now = Instant.now()
