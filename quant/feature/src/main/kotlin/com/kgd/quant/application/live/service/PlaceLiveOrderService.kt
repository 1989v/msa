...
/**
 * PlaceLiveOrderService — Phase 3 7-stage 실주문 게이트 (ADR-0037 / TG-P3-26).
 *
 * 게이트 순서:
...
class PlaceLiveOrderService(
...
) : PlaceLiveOrderUseCase {
    override suspend fun execute(input: PlaceLiveOrderUseCase.Input): LiveOrderRecord {
        val now = Instant.now()
        ...
        if (mode !is LiveTradingMode.Enabled) {
            ...
            throw PlaceLiveOrderUseCase.OrderRejectedByGate("live-mode is ${mode::class.simpleName}", SuspendReason.USER_KILL_SWITCH)
        }
        ...
        if (ks.anyEnabled) {
            ...
            metrics.liveOrderRecorded(input.placement.marketCode.value, "rejected")
            throw PlaceLiveOrderUseCase.OrderRejectedByGate("kill-switch ON ($reason)", SuspendReason.USER_KILL_SWITCH)
        }

        // 게이트 5~7: risk-limit (단일 주문 + 일일 누적 평가)
        when (val r = riskLimitService.evaluatePreOrder(input.tenantId, orderKrw)) {
            is ManageRiskLimitUseCase.PreOrderResult.Reject -> {
                ...
                throw PlaceLiveOrderUseCase.OrderRejectedByGate(r.detail, r.reason)
            }
            is ManageRiskLimitUseCase.PreOrderResult.Allow -> { /* 통과 */ }
        }
    ...
    private suspend fun audit(
        input: PlaceLiveOrderUseCase.Input,
        type: AuditEventType,
