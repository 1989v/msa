package com.kgd.quant.application.backtest.service
...
import com.kgd.quant.application.backtest.InMemoryEventPublisher
import com.kgd.quant.application.backtest.MutableClock
import com.kgd.quant.application.backtest.StrategyExecutor
import com.kgd.quant.application.backtest.dto.BacktestRunResultView
import com.kgd.quant.application.backtest.dto.EventSummary
...
/**
 * RunBacktestService — 백테스트 1 회 실행 오케스트레이터.
 *
 * ## 흐름 (ADR-0020 트랜잭션 경계)
 *  1. 전략 조회 (no txn)
 ...
 *
 * ## 메트릭 (TG-14.2)
 * - 성공/실패 `quant_backtest_run_total{status}` 증가
 * - 전체 소요시간 `quant_backtest_run_duration_seconds` 기록
...
class RunBacktestService(
    ...
    private val metrics: QuantMetricsPort? = null,
    @Value("\${quant.backtest.initial-balance:100000000000}")
    private val initialBalanceRaw: String = "100000000000",
    @Value("\${quant.backtest.bar-interval:MINUTE_1}")
    private val barIntervalRaw: String = "MINUTE_1"
) : RunBacktestUseCase {

    override suspend fun execute(command: RunBacktestCommand): BacktestRunResultView {
        val startNanos = System.nanoTime()
        try {
            val view = doExecute(command)
            metrics?.backtestRunSucceeded()
            return view
        } catch (e: Throwable) {
            metrics?.backtestRunFailed()
            throw e
        } finally {
    ...
    private suspend fun saveBacktestRecordBestEffort(
        ...
        runCatching {
            ...
            logger.warn { "RunBacktestService: ClickHouse backtest_run save failed runId=${result.runId} msg=${e.message}" }
        }
    }
}
