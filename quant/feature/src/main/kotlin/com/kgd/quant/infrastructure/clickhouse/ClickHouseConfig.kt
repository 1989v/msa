...
class ClickHouseConfig {
    ...
    fun quantClickHouseJdbcTemplate(): JdbcTemplate {
        val ds: DataSource = HikariDataSource(HikariConfig().apply {
            ...
            isAutoCommit = true
            // 기동 때 연결을 열지 않는다. 기본값은 첫 연결을 즉시 시도하고 실패하면
            // PoolInitializationException 으로 컨텍스트가 죽는데, ADR-0093 이후 이 JVM 에는
            // chatbot·gifticon 이 함께 있다 — ClickHouse 한 대가 죽으면 관계없는 두 도메인까지
            // 내려간다. 지표 조회만 실패하고 나머지는 살아 있어야 한다.
