...
import com.kgd.quant.domain.event.TrancheSlotClosed
import com.kgd.quant.domain.event.TrancheSlotOpened
 ...
 * 포함한다. 본 spec 은 **동일 slotId 에 대해 TrancheSlotOpened 이벤트가 2 회 이상 발행** 되고,
 * 각 오픈 사이에 TrancheSlotClosed 가 끼어 있음을 검증한다 — 슬롯 재활용 경로 보호.
 */
class SlotRebuyScenarioSpec : BehaviorSpec({
    ...
    Given("volatile fixture 를 실행하면") {
        ...
        val opens = result.events.filterIsInstance<TrancheSlotOpened>()
        val closes = result.events.filterIsInstance<TrancheSlotClosed>()
        ...
        When("slotId 별로 open 횟수를 세면") {
            ...
            Then("재활용된 slot 의 이벤트 순서는 Opened → Closed → Opened 이다") {
                ...
                val slotEvents = result.events.filter { event ->
                    when (event) {
                        is TrancheSlotOpened -> event.slotId == reusedSlotId
                        is TrancheSlotClosed -> event.slotId == reusedSlotId
                        else -> false
                    }
                }
                ...
                (slotEvents[0] is TrancheSlotOpened) shouldBe true
                (slotEvents[1] is TrancheSlotClosed) shouldBe true
                (slotEvents[2] is TrancheSlotOpened) shouldBe true
            }
            Then("SELL 체결이 최소 1 회 발생한다 (TrancheSlotClosed 존재)") {
