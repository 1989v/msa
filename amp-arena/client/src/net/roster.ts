...
import { MODES, MAX_PLAYERS, makeRng, randomLoadout, sanitizeStatDelta, sanitizeSkin, type Pick, type RoomSettings, type RosterEntry } from '@amp/shared';
...
export function buildRoster(occ: number[], seats: (SeatLike | null | undefined)[], settings: RoomSettings, seed: number): { roster: RosterEntry[]; spectators: number[] } {
  ...
  for (const seat of occ) {
    ...
    count[team]++;
    roster.push({ id: seat, name: s?.name ?? `${seat + 1}번`, team, acc: pick?.acc ?? 'none', style: pick?.style ?? 'fighter', bot: false, stats: pick?.stats ? sanitizeStatDelta(pick.stats) : undefined, skin: sanitizeSkin(pick?.skin) });
  }
