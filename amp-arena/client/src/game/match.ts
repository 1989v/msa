...
export class Match {
  ...
  private render(now: number, dt: number): void {
    ...
    for (const rp of players) {
      ...
      if (wasG === false && rp.grounded) this.renderer.spawnDust(rp.x, rp.y, rp.z);
      const ps = this.prevState.get(rp.id);
      if (ps !== 'run' && rp.state === 'run') this.renderer.spawnDust(rp.x, rp.y, rp.z);
  ...
  private onEvent(ev: WorldEvent): void {
    ...
    switch (ev.t) {
      case 'hit': {
        ...
        if (ev.kind === 'guard') { this.renderer.spawnHit(ev.x, ev.y, ev.z, 'guard'); if (mine) { const s = this.renderer.project(ev.x, ev.y + 0.4, ev.z); this.hud.showDamage(s.x, s.y, '가드', 'guard'); } break; }
        if (ev.kind === 'guardBreak') { this.renderer.spawnHit(ev.x, ev.y, ev.z, 'launch'); const s = this.renderer.project(ev.x, ev.y + 0.4, ev.z); this.hud.showDamage(s.x, s.y, '가드 크러시!', 'guard'); break; }
