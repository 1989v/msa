...
export const ACCESSORIES: Record<AccessoryId, AccessoryDef> = {
  ...
  // 2026-09-13 소감: 원거리로 쪼기만 해서 셌다 → 약공은 총열로 후려치는 근접, 강공만 두 줄 5m.
  // `ranged` 는 이제 「봇이 거리를 두는가」만 뜻한다 — 탄·분기는 무브가 투사체인지로 갈린다(PROJECTILE_MOVES).
  pistols:    { id: 'pistols', name: '더블탭', combo: ['gunWhip', 'gunWhip2'], heavy: ['gunBurst'], special: 'gunRoll', specialCooldownSec: 6, speedMult: 1, canGrab: true, guardAngleDeg: 180, guardCrushImmune: false, ranged: false, ammo: 12, reloadTicks: 90, airDashes: 0 },
  ...
  chakram:    { id: 'chakram', name: '차크람', combo: ['ckThrow'], heavy: ['ckHeavy'], special: 'ckSpin', specialCooldownSec: 6, speedMult: 1, canGrab: true, guardAngleDeg: 180, guardCrushImmune: false, ranged: true, ammo: 10, reloadTicks: 85, airDashes: 0 },
