...
export interface StyleDef {
  ...
  throwDamage: number;
  airDashes: number;
  // 이 직업이 들 수 있는 악세서리 — **맨손 + 전용 셋**이다 (2026-09-13 소감 두 번: 직업과 무관한 무기가 섞여
  // 보였고, 그다음 하나뿐이라 고를 게 없었다). 무기 하나는 직업 하나에만 붙어야 「그 직업의 무기」로 읽히고,
  // KO 드랍도 아무나 주워 가지 않는다. 게이트는 shared/test/accexclusive.test.ts — 배타성과 개수를 둘 다 본다.
...
export const STYLES: Record<StyleId, StyleDef> = {
  fighter: {
    ...
    accessories: ['none', 'rocket', 'knuckle', 'chain'],
  ...
  grappler: {
    ...
    accessories: ['none', 'shield', 'claw', 'anchor'],
  ...
  speedster: {
    ...
    accessories: ['none', 'pistols', 'dagger', 'chakram'],
  ...
  heavy: {
    ...
    accessories: ['none', 'greatsword', 'hammer', 'cannon'],
  ...
  martial: {
    ...
    accessories: ['none', 'spear', 'staff', 'nunchaku'],
...
/**
 * 봇 장비: 직업을 무작위로 고르고 그 직업의 전용 악세서리 중 하나를 3에 2 확률로 든다.
 * 맨손 확률을 따로 두는 이유 — 균등하게 뽑으면 선택지 수에 따라 맨손 비율이 흔들리고,
 * 맨손이 많으면 판에 무기가 잘 안 보인다. 여기서 1/3 로 못 박아 둔다.
 */
export function randomLoadout(rng: () => number): { style: StyleId; acc: AccessoryId } {
  ...
  if (rng() >= 2 / 3) return { style, acc: 'none' };
  const own = STYLES[style].accessories.filter((a) => a !== 'none');
  return { style, acc: own[Math.min(own.length - 1, Math.floor(rng() * own.length))]! };
}
