...
| 운영 상태 (2026-09-13) | main c9221915 · games 648992d3 · 이미지 portal-fe:c922191 / content:d8db405 · 번들 `index-CjJOLYZk.js`. **Phase 2 + 공중 공격 + 봇 자멸 수정 + 악세서리 16종 + 더블탭 개편 + 전체화면 + 모바일 한 화면 + 온라인 준비·방 목록·슬롯 봇 + 방 규칙 셋 + 협동 시나리오**. 운영 실측: `tools/e2e-prod-{fullscreen,portrait,score,rules,air,bots,accex,rooms,coop}.mjs` + `e2e-online.mjs` + `e2e-room-rules.mjs` 전부 통과 |
...
- [x] **방 규칙 셋** (2026-09-13 소감 「스탯은 방 생성 옵션으로 — 오래 한 사람이 더 세지니까」) — 아이템전/노템전 · 악세전/맨손전 · 스탯 적용/없음. **명단 단계에서 못 박는다**: 악세서리·스탯은 방장이 만드는 roster 에 반영돼 cfg 로 나가므로 게스트가 뭘 골랐든 방 규칙이 이긴다(그래서 cfg 에 따로 안 싣는다). 아이템만 월드가 알아야 해서 `MatchConfig.items` 로 싣는다. 검사는 규칙 on/off 를 같은 테스트에서 비교해 무동작이면 반드시 빨간불 — `shared/test/roomrules.test.ts` 8/8, 실측 `tools/e2e-room-rules.mjs`
- [x] **협동 시나리오** (2026-09-13 소감 「친구와 한편으로 즐기는 봇 상대 시나리오」) — 모드 `coop`. 사람은 전부 레드, 봇은 전부 블루(온라인 명단과 연습 둘 다 못 박는다). 봇은 물결마다 목숨 하나, 다 누우면 다음 물결이 체력·공격 +1 로 온다(5물결). 다 막으면 사람 승, 사람이 목숨(3)을 다 쓰면 봇 승. **물결은 스냅샷 `w` 의 세 번째 칸** — 뒤에 붙이면 타이머 자르기가 깨진다. HUD 모드 칩이 「물결 n / 5」로 바뀐다. `shared/test/coop.test.ts` 6/6, 실측 `tools/e2e-coop.mjs` 7/7
- [x] **더블탭 개편** (2026-09-13 소감 「약간 오버밸런스」) — 약공은 총열로 후려치는 근접 2단(`gunWhip`/`gunWhip2`, 7/9·리치 1.5m·탄 안 씀), 강공은 두 줄 2연발(사거리 13→5m·경직 8→3). **다연발이 탄을 안 먹던 버그**를 함께 고쳤다(3발 연사·백롤 6발이 공짜였다) — 이제 `fanCount` 만큼 먹는다. 탄·분기 판정을 `acc.ranged` 에서 **그 무브가 투사체인지**로 옮겼다. 300판 1.10 → 0.99 (한 번 0.77 로 과하게 떨어져 되올림), 차크람 탄 7→10 으로 0.92
- [x] **모바일 한 화면** (2026-09-13 소감 「드래그 안 해도 되게」) — 폰 가로(844×390)에서 타이틀이 826px 라 436px 가 잘렸다. 폼을 두 묶음(`tf-a`/`tf-b`)으로 나눠 짧은 화면에서 옆으로 편다. **판정은 `@media (max-height)` 로 못 한다** — 세로 폰을 CSS 로 돌려도 뷰포트는 844px 세로다. `orient.ts` 가 실제 쓸 수 있는 높이를 재서 `#app.short` 를 붙인다. `vw` 도 같은 이유로 금지(회전 전 폭을 가리켜 폼이 374px 로 줄었다). 결과 넘침 0px
- [x] **온라인 셋** (2026-09-13 소감) — ① 준비/준비 해제(`Pick.ready`, 방장 시작 버튼에 `준비 n/m`, 안 된 사람이 있으면 한 번 경고 후 강행) ② **공개 방 목록**에서 코드 없이 입장(릴레이 `rooms` 명령 + `listed` 플래그, JVM `GameRelayRegistry.listRooms` 와 `tools/dev-relay.mjs` 양쪽) ③ 방장이 **빈 슬롯을 눌러 그 자리만 봇**(`RoomSettings.botSeats`). 실측 `tools/e2e-online-room.mjs` 5/5
- [x] **악세서리 직업당 셋** (2026-09-13 소감 「직업당 세 개 이상으로」) — 열 종을 더해 16종(맨손 포함). 파이터 부스터·너클·체인 / 그래플러 월·클로·앵커 / 스피드스타 더블탭·대거·차크람 / 헤비 브레이커·해머·캐논 / 마셜 스파이크·스태프·넌척. 무브 46개·투사체 6종·무기 모델 10종·아이콘 10종을 함께 넣었다. **무브와 `ACCESSORY_IDS` 는 맨 뒤에만 붙인다**(스냅샷 인덱스). 300판 조정: 차크람 1.37→0.88·캐논 1.34→0.92(탄이 크고 한 발이 세서 더블탭 경제에 맞췄다), 대거·클로·넌척·스태프 0.76~0.84 → 0.89~0.97, 앵커 1.31→1.19(평타 슈퍼아머와 가드 크러시 면역을 뺐다 — 면역은 월의 정체성). 전부 ±30% 안
- [x] **직업 버튼 이름이 잘리던 것** (2026-09-13) — 색 상점의 `.swatch`(min-width 64px)가 직업 버튼 안 색 칸에도 걸려 「파이터」가 「파이」로 잘렸다. `.stylebtn .swatch { min-width: 0 }` 한 줄. 악세서리 격자는 3열 고정에서 `auto-fit`(최소 88px)으로 — 넷이 3+1 로 갈라지지 않는다
- [x] **카탈로그에서 시작하면 바로 전체화면** (2026-09-13 소감) — 1440×900 창에서 무대는 1150×521 인데 대기실은 659px 라 캐릭터 선택이 138px 잘려 있었다(실측). `shouldAutoLandscape`(터치+세로만)를 `shouldEnterFullStage`(가로 전용 게임이면 데스크톱도)로 바꾸고, `.game-stage:fullscreen .game-stage-frame { height: 100% !important }` 로 iframe 인라인 높이를 덮는다 — 안 덮으면 전체화면으로 올려도 521px 그대로다. portal-fe 쪽 변경
...
node tools/e2e-room-rules.mjs $P http://127.0.0.1:5180 <outDir>   # 방 규칙(노템·맨손·스탯 없음)이 판에서 강제되는지 (릴레이 자체 기동)
node tools/e2e-coop.mjs $P http://127.0.0.1:5180 <outDir>         # 협동 물결 — 팀 배치·목숨·물결 넘김·봇 강화
node tools/e2e-prod-coop.mjs $P https://game.1989v.com/games/arena/index.html <outDir> # 운영 번들의 협동 (모드 노출·팀·물결)
