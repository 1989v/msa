...
class KafkaConfigTest : BehaviorSpec({
    ...
    fun extractErrorHandler(): DefaultErrorHandler {
        val config = ProductKafkaConfig().apply {
            val field = ProductKafkaConfig::class.java.getDeclaredField("bootstrapServers")
            field.isAccessible = true
            field.set(this, "localhost:9092")
        }
        val factory = config.productKafkaListenerContainerFactory(
            consumerFactory = config.productConsumerFactory(),
