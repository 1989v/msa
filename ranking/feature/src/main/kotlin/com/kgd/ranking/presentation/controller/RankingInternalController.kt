...
import com.kgd.ranking.application.gas.dto.GasStationBulkRequest
...
class RankingInternalController(
    private val syncGasStations: SyncGasStationsUseCase,
    private val rebuildGasBoards: RebuildGasBoardsUseCase,
) {

    @PostMapping("/stations/bulk")
    fun bulkUpsert(@Valid @RequestBody request: GasStationBulkRequest): ApiResponse<GasStationBulkResult> =
        ApiResponse.success(syncGasStations.execute(SyncGasStationsUseCase.Command(request.stations)))

    /** 적재분으로 시군구 × 유종 보드 스냅샷을 다시 만든다. 수집 직후 같은 잡이 부른다. */
    @PostMapping("/boards/rebuild")
    fun rebuildBoards(@RequestBody request: BoardRebuildRequest): ApiResponse<RebuildGasBoardsUseCase.Result> =
        ApiResponse.success(rebuildGasBoards.execute(RebuildGasBoardsUseCase.Command(request.sourceLabel)))
}
