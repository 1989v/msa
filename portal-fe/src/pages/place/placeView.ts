...
/* ─── 개요 본문 정리 ────────────────────────────────────────────────────────
 * 원천(TourAPI)의 텍스트는 평문이 아니라 `<br />`·`&rsquo;` 가 섞인 문자열이다.
 * 정리 규칙은 **`seo/copy.mjs` 가 단일 원본**이다 — 화면만 정리하고 프리렌더가 안 거치면
 * 크롤러가 보는 본문에 태그가 글자로 남는다 (2026-09-10 실측: 프리렌더 3/800 에
 * `It&rsquo;s` 가 그대로). 빌드 스크립트는 .ts 를 로드하지 못해 .mjs 에 둔다.
 */
export { sourceText } from '../../seo/copy.mjs';
