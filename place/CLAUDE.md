...
- **`regions`(GeoNames 지명 계층)과 `administrative_regions`(한국 행정구역)은 다른 것이다** (ADR-0071).
...
- **분류 코드에는 이름이 따로 있다** — `attraction_category_codes`(`lclsSystmCode2`, ko 315 · en 302).
  코드만으로는 필터 이름도 질의 연결도 못 만든다. **깊이는 받는 값이지 코드 길이로 유도하는 값이 아니다**:
  대부분 2/4/8 자지만 `C01`(추천코스) 계열만 3/5/9 자라(13건) 유도하면 그 13건에서 터진다.
  같은 코드의 국문·영문 이름이 짝으로 와서 **한영 동의어가 여기서 나온다**.
- **반려동물은 `detailIntro2` 가 아니라 `detailPetTour2`** 다. 전자의 `chkpet` 은 44,924건 중 3건뿐이고,
  후자는 contentId 없이 목록으로 9,691건을 준다 — 약 100콜이면 전량이라 예산 인자가 없다.
...
| GET | `/api/places/administrative-regions?level=&parent=` | public | **한국 행정구역**(법정동 코드) 시도/시군구 (ADR-0071) |
| POST | `/api/places/administrative-regions/bulk` | ADMIN | 법정동코드 자료 적재 |
