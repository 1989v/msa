...
- **시도 17개는 `SIDO_EN` 상수**다. 관광지 주소에서 뽑으면 원천 오류를 물려받는다 —
  `Jeonnam-Gwangju Special Metropolitan City` 568건이 전남과 광주를 뭉개서 둘 다 그 이름이 된다.
  코드로 잡는다(이름은 바뀐다: 강원도 → 강원특별자치도).
...
> **첫 실행 후 출력된 영문명을 한 번 훑을 것.** 원천이 **일관되게** 틀린 경우가 있어 최빈값으로도
> 안 걸러진다 — 실측으로 인천 서구가 137건 모두 `Seohae-gu` 로 온다(맞는 표기는 `Seo-gu`).
