...
> **첫 실행 후 출력된 영문명을 한 번 훑을 것.** 잡의 마지막에 시도별로 한 줄씩 찍는다.
>
> 다만 **낯선 이름을 원천 오류로 단정하지 말 것.** 이 작업에서 두 번 그렇게 판단했고 두 번 다
> 틀렸다 — `Jeonnam-Gwangju Special Metropolitan City`(전남·광주 통합)와 `Seohae-gu`(인천
> 서구 폐지 후 개편) 둘 다 실제 행정구역이다. 판정 기준은 기억이 아니라 **법정동 자료**다.

**순서가 있다.** 이 잡은 관광지에서 좌표·영문명을 뽑으므로 **관광지 재동기화(`--job=sync`) 뒤에**
한 번 더 돌려야 채워진다. 코드가 비어 있으면 묶을 키가 없어 0건이 나온다.
...
- 인천 서구(2826000000)는 **폐지**되고 제물포구·영종구·서해구·검단구로 개편됐다.
  TourAPI 의 `Seohae-gu` 도 오류가 아니다.
