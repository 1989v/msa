-- ADR-0029 PR-10 (Phase 4): fulfillment `processed_event` v0 / backup 테이블 정리.
--
...
--
-- PR-5 호출부 helper 이관 + 운영 1주 관찰 (graceful degrade missingId 메트릭 0) 후 본 PR 에서 두 테이블을
-- 영구 DROP 한다. 운영 DB 에 잔존하지 않는 환경 (PR-4 시점 이전에 합류한 cluster, 또는 dry-run 환경) 에서도
-- 스크립트가 안전하게 통과하도록 IF EXISTS 가드를 사용한다.
--
-- ## Rollback
