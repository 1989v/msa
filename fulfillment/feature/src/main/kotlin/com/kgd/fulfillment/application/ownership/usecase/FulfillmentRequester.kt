...
/**
 * 이행 REST 요청자 — 게이트웨이가 JWT 를 검증하고 주입한 `X-User-Id` · `X-User-Roles` 에서 온다.
 * 게이트웨이는 클라이언트가 붙인 같은 이름의 헤더를 지운다.
 */
data class FulfillmentRequester(val userId: String, val roles: Set<String>) {
    ...
    companion object {
        ...
        fun of(userId: String?, roles: String?): FulfillmentRequester {
            ...
            return FulfillmentRequester(id, roleSet)
        }
    }
}
