...
- K8s (Kubernetes) 에서 Redis 클러스터 모드 전환 시 5개 서비스 (gateway, product, gifticon, analytics, experiment) 영향
- Redisson 사용 여부
...
- Redis Stream 심화 (msa 에 도입 가능성?)
  - ✅ 결정: `14-stream-consumer-group.md` 에서 XADD/XREADGROUP/XACK + PEL + Kafka 비교까지 정리. improvements §7 (P1) 에서 Notification fanout 도입 검토 후보(△, 조건부) 등재.
- Redisson 사용 여부 확인 후 범위 결정
  - ✅ 결정: 단일 master Redisson `RLock` + 펜싱 토큰 패턴 권장 (preview 권장 운영값 표 + improvements §6 P1 분산 락 정책 ADR 후보). RedLock 은 Kleppmann 비판 + 운영 부담 대비 이득 작아 미사용.
- Cluster 모드 실습 범위
  - ✅ 결정: `09-cluster-slots.md` 에서 16384 슬롯/MOVED-ASK/hash tag/multi-key 한계까지 이론 + msa `k8s/infra/prod/redis/values.yaml` 결합. improvements §4 (P0) 에서 Hash tag 컨벤션 ADR 후보 등재.
- Lua Script 실전 예시 포함 여부
  - ✅ 결정: `15-pipeline-lua-pubsub.md` 에서 RTT 절감 + Lua 원자성 + Redis 7+ Function 비교. improvements §10 (P2) 에서 Lua → Function 마이그레이션 후보(△) 등재.
