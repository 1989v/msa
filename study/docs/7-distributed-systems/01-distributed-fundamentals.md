...
| 2 | Latency is zero | TCP RTT, TLS (Transport Layer Security, 전송 계층 보안) 핸드셰이크, AZ (Availability Zone, 가용 영역) 간 1ms, 리전 간 80-300ms |
...
- "죽은 건 아닌데 정상도 아닌" 상태 — GC (Garbage Collection, 가비지 컬렉션) pause, 디스크 I/O 폭주, CPU 100%
...
| 오류 | msa 적용 / 방어 |
|---|---|
| network reliable | order → payment / product **CB + retry** (ADR (Architecture Decision Record, 아키텍처 결정 기록)-0015), Kafka outbox 로 발행 보장 |
| latency zero | non-blocking I/O (suspend + WebClient), Redis fast-path |
| bandwidth ∞ | Kafka payload 최소화, ES bulk indexing |
| network secure | mTLS (mutual TLS, 양방향 TLS) (k8s + Istio 검토), JWT 검증 (gateway) |
