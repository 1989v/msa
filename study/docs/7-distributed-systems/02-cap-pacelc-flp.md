...
> "CAP 에서 뭘 버리셨어요?" 는 함정 질문이다. **CAP 는 분할 시점에만 적용** 되고, **평시엔 PACELC 의 L vs C** 가 더 본질이다.

## 1. CAP Theorem (Eric Brewer, 2000)

분산 시스템이 동시에 만족할 수 없는 3개 속성:
...
4. 모니터링: replica lag, kafka consumer lag SLI
...
| product (상품) | PA + EL | Kafka consumer 로 stock 동기화 (ADR-0013), eventual |
| analytics | PA + EL | ClickHouse, 대량 적재 우선 |
| auth | PC + EC | RBAC 정합성 우선, 토큰 검증은 stateless |
