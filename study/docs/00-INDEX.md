...
- **msa 적용**: search 서비스 4-모듈 (`domain/app/consumer/batch`) 직접 분석 + 강점 10개 / 점검 17개 / **ADR 후보 12건** (lag SLA / 변동성 필드 컨벤션 / ES vs OpenSearch 일원화 / Hybrid Search 도입 + 평가 메트릭·judgment 인프라 / function_score modifier 표준 LN2P / 자동완성 단계적 도입 + 인덱스 component template 분해 / 매핑 v2 (scaled_float·_routing·copy_to) / _msearch 통합 / data stream + downsampling / 벡터 quantization 표준) + Hybrid Search PoC 코드 + 평가/modifier/자동완성 deep file 3건 (#34~36) + 인덱스/매핑/운영/시계열/벡터 deep file 5건 (#37~41) + online learning/MAB 트랙 3건 (#42~44, ADR-0043) + **평가 파이프라인·bias 보정** (#45, Stage×Setting 2×2 / Position-Exposure-Feedback Loop 3종 / IPS / Shadow·Interleaving·Ramp-up).
