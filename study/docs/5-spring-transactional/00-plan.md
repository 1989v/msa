...
- ADR-0020 기반으로 진행 범위 한정 vs 일반 이론 중심
  - ✅ 결정: Top-down 으로 일반 이론 (Phase 1-2: AOP/롤백/전파/격리/readOnly) 먼저 + ADR-0020/0022/0012 매핑 (Phase 3-4) 결합. preview "핵심 5문장" 4번이 ADR-0020 의 외부 IO 분리 원칙.
- Saga / Outbox 패턴 깊이
  - ✅ 결정: `09-external-io-separation.md` 에서 TransactionalEventListener (AFTER_COMMIT) + Outbox 폴링 + Saga 개념 + `12-msa-outbox-saga.md` 에서 inventory/fulfillment 코드 분석. 분산 정합성 깊이는 #7 분산시스템 으로 분담.
- TransactionTemplate 실습 여부
  - ✅ 결정: `10-transaction-template.md` 에서 프로그래밍 방식 트랜잭션 + Reactive 환경 위치까지 다룸 (실측 lab 형태 아닌 패턴 문서로).
