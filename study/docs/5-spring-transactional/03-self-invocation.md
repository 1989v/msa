...
AspectJ (Load-Time Weaving 또는 Compile-Time Weaving) 를 쓰면 바이트코드 자체를 변환하므로 self-invocation 도 동작한다. 하지만 설정 복잡성 때문에 실무에서 거의 안 쓴다 — Spring 표준은 프록시 기반.
...
[fulfillment/]
  FulfillmentService.kt          ← @Transactional 클래스 레벨 (예외 케이스)
                                   외부 IO 가 outboxPort.save 만이라 분리 불필요
```
