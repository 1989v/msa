...
→ 다만 이 정책이 모든 서비스에 일관적이지 않다. [13-improvements.md](13-improvements.md) 의 stickiness ADR (Architecture Decision Record, 아키텍처 결정 기록) 후보 참고.
...
- **MySQL 8.x + RDS (Relational Database Service, 관계형 데이터베이스 서비스) Proxy / Aurora**: Connection.setReadOnly 기반 자동 라우팅 → application 코드는 라우팅 모름
