...
msa 프로젝트가 Kafka 를 SSOT 기반 이벤트 버스로 사용하므로 직접 연결된다. ADR (Architecture Decision Record, 아키텍처 결정 기록)-0012 (멱등 Consumer), ADR-0015 (Resilience) 와 연계.
...
- 분산 시스템 기본 (CAP (Consistency / Availability / Partition tolerance, 일관성·가용성·분할 내성))
...
- Rack Awareness (KIP-392) — AZ (Availability Zone, 가용 영역) 간 비용 최적화
...
- Kafka Streams 포함 여부 (msa 가 analytics 에서 사용)
  - 🔄 부분 결정: analytics Streams 코드 점검 + EOS 외부 write 한계 (improvements §9) 까지 다룸 / 추가 검토 필요: Streams 토폴로지/스토어 깊이 학습은 별도 토픽 후보 ("Kafka Streams 심화 + Iceberg 통합") 로 이관.
- KRaft 모드 심화 여부
  - ✅ 결정: `03-controller-kraft.md` 에서 메타데이터 관리 + ZK 마이그레이션 + msa 가 KRaft 사용 중인 사실 정리.
- Schema Registry 도입 검토 포함 여부
  - ✅ 결정: improvements §10 에서 Confluent Schema Registry/Apicurio + Avro 도입 ADR 후보(L3) 로 정식 등재.
- EOS 실습 포함 여부
  - 🔄 부분 결정: `09-exactly-once.md` 에서 transactional.id + read_committed + Consume-Transform-Produce 한계 정리 / 추가 검토 필요: 별도 lab 형태 재현은 미작성 (analytics Streams 자연 멱등 분석으로 대체).
