...
**그럼 fsync 안 했는데 데이터 손실은?** → **Replication 이 fsync 를 대신**. acks=all + RF=3 + min.ISR=2 → 두 개 broker 의 page cache 에 동시에 손실되어야 데이터 잃음. 매우 낮은 확률.
...
- TLS 활성화 시도 zero-copy 불가 (kernel 이 평문→암호문 변환을 user 공간에서 못 함)
...
- broker JVM heap 은 **6GB 이하** 로 잡아라 — 나머지는 OS page cache 에 양보
...
- **Q. broker 의 JVM heap 을 크게 잡으면 안 되는 이유?**
  > Kafka 는 Java 객체 캐싱을 거의 하지 않는다 (page cache 신뢰). 큰 heap → GC pause ↑. heap 작게 (4-6GB) + 나머지 OS 가 page cache 로 활용 → 최근 데이터 in-memory 효과. 보통 NewGen ↑ + G1GC.
