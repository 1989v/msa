...
Java 의 동시성 프리미티브 (synchronized, volatile, Atomic, Lock, Concurrent Collections) 의 내부 동작 원리부터, Kotlin 의 coroutine/Flow, 최신 Virtual Threads (Project Loom (Project Loom — 가상 스레드 프로젝트)) 까지 10년차가 면접에서 방어해야 할 동시성 전반을 학습한다.
...
- `ConcurrentHashMap` 의 내부 구조 (Segment → Bin + CAS (Compare-And-Swap, 비교-교환)) 를 설명할 수 있다
