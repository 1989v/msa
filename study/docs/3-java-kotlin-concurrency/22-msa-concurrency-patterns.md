...
# 검증 결과 (2026-05-01): zero hit. ThreadPoolTaskScheduler / TaskScheduler 명시 빈 없음.
```

→ 명시 빈 없으면 **여러 잡이 1개 스레드 공유** — Spring 의 default `TaskScheduler` 가 1개 스레드. msa 에서는 `ReservationExpiryService`, `InventoryReconciliationService`, `OutboxPollingPublisher` (×2 서비스) 등 다수의 `@Scheduled` 가 동일 풀을 공유. 한 잡이 길어지면 다른 잡이 늦음 → **개선 후보 (P1)**: `@Configuration` 에 `ThreadPoolTaskScheduler(poolSize=N)` 빈 명시.
