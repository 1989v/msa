...
OS (OpenSearch) 의 **Search Pipeline** = ES 의 ingest pipeline 의 **검색 버전**. request / response / search-phase processor 3계층으로 검색 요청·응답을 변환. **Hybrid query + normalization-processor** 가 ES 의 retrievers RRF 등가물이고, **Neural / Neural Sparse + ML Commons** 가 ES 의 Inference API + ELSER 등가물.
...
- **Neural query**: `neural` — 자동 임베딩 + kNN. text → ML model → vector → kNN
