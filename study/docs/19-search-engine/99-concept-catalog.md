...
## §G. Online Learning / MAB (2026-05-12 추가)

> §42~§44 의 online learning 트랙 신설로 catalog 에 §G 섹션 추가. 기존 §A~§F 와 직교 — 오프라인 IR/ML 가 아닌 실시간 클릭 기반 의사결정.

| 행ID | 영역 | 개념 | 상태 | 심화 파일 |
|---|---|---|---|---|
| §G.bayesian-update | online | Bayesian update (Prior + Likelihood → Posterior) | ✅ 커버 | §42 |
| §G.beta-bernoulli-conjugate | online | Beta-Bernoulli conjugate posterior | ✅ 커버 | §42 |
| §G.thompson-sampling | online | Thompson Sampling (확률적 샘플링 ranking) | ✅ 커버 | §42 §43 §44 |
| §G.epsilon-greedy | online | ε-greedy (균등 랜덤 탐색) | ✅ 커버 | §43 |
| §G.ucb1 | online | UCB1 (Upper Confidence Bound — 결정적 보너스) | ✅ 커버 | §43 |
| §G.linucb | online | LinUCB (Contextual Bandit, 선형) | △ 개요 | §43 |
| §G.neural-bandit | online | Neural Bandit (DNN posterior / NeuralUCB) | △ 개요 | §43 |
| §G.empirical-bayes-prior | online | 카테고리/지역 평균 CTR 기반 prior 산출 | ✅ 커버 | §42 §44 |
| §G.decay-temporal | online | `exp(−λ × age)` 시간 감쇠 가중 | ✅ 커버 | §42 §44 |
| §G.hybrid-score | online | `w·esNorm + (1−w)·sample` 결합 | ✅ 커버 | §43 §44 |
| §G.impression-threshold | online | 노출 N 미만 arm 의 prior-only fallback | ✅ 커버 | §44 |
| §G.session-cache | online | flicker 방지 — `(userId, query)` N초 같은 샘플 | ✅ 커버 | §43 §44 |
| §G.position-bias-ipw | online | 위치별 클릭률 보정 (Inverse Propensity Weighting) | ⏳ Phase 3 | §43 |
| §G.bandit-key-bucketing | online | `(categoryId, productId)` arm 키 / Redis state | ✅ 커버 | §44 |
| §G.bandit-event-stream | online | Kafka 토픽 3종 (impression/click/snapshot) | ✅ 커버 | §44 (ADR-0043) |
| §G.graceful-degradation | online | Redis 다운 시 prior-only 동작 | ✅ 커버 | §44 |
| §G.regret-bound | online | regret 시간복잡도 (Θ(T) / O(log T)) | △ 개요 | §43 |
| §G.ab-testing-mab | online | A/B vs Bandit 의사결정 (Welch vs allocation) | ⏳ Phase 2 | — |

### 후속 후보 (Phase 2~3)

- **§G.contextual-feature-eng** — user × time × device × geo feature design
- **§G.ope-doubly-robust** — Off-policy evaluation (DR estimator)
- **§G.slate-cascade-bandit** — top-K 전체를 한 slate 로 보는 bandit
- **§G.ts-non-bernoulli** — Gaussian / log-normal 보상의 TS 변형

## §H. Evaluation Pipeline · Bias Correction (2026-05-13 추가)

> §34 가 "메트릭 수식 + Stage 축" 까지라면, §45 는 **Setting 축 (Offline vs Online) + Bias 3종 + 3-Tier 평가 파이프라인** 을 다룬다. §G (online learning / MAB) 와 직교 — §H 는 "평가/측정 인프라", §G 는 "exploration 알고리즘".

| 행ID | 영역 | 개념 | 상태 | 심화 파일 |
|---|---|---|---|---|
| §H.online-offline-axis | evaluation | Stage × Setting 2×2 프레임워크 (Retrieval/Ranking × Offline/Online) | ✅ 커버 | §45 |
| §H.graded-relevance-5tier | evaluation | 노출/클릭/dwell/cart/구매 = 0~4 등급 매핑 | ✅ 커버 | §45 |
| §H.online-metrics-catalog | evaluation | CTR / CTR@1 / CVR / GMV / AOV / Bounce / Re-search / Retention / Diversity / Zero-Hit | ✅ 커버 | §45 |
| §H.north-star-guardrail | evaluation | North-Star 1개 + 가드레일 2~3개 의사결정 패턴 | ✅ 커버 | §45 |
| §H.position-bias | bias | 위치별 CTR 편향 (위가 본질적으로 클릭 ↑) | ✅ 커버 | §34 §45 |
| §H.exposure-bias | bias | 노출 자체가 안 된 doc 의 학습 부재 (cold-start / 신상품 사망) | ✅ 커버 | §45 |
| §H.feedback-loop | bias | Position + Exposure 의 self-reinforcing 사이클 | ✅ 커버 | §45 |
| §H.ips-counterfactual | bias | `weighted_click = click / propensity` + clipping (ε=0.01) | ✅ 커버 | §45 |
| §H.propensity-estimation | bias | position heuristic / EM / random swap / click model (PBM·DBN·UBM) | ✅ 커버 | §45 |
| §H.dr-estimator | bias | Doubly Robust — IPS + direct method | △ 개요 | §45 (소개만) |
| §H.shadow-traffic | pipeline | 새 모델 dual-execute, 응답 무시, diff 로깅 | ✅ 커버 | §45 |
| §H.interleaving-tdi | pipeline | Team-Draft Interleave — A/B 대비 10~100배 효율 | ✅ 커버 | §45 |
| §H.ab-stat-sample-size | pipeline | `n = 16(σ/δ)²` + Welch / chi-square / peeking bias / SPRT | ✅ 커버 | §45 |
| §H.ramp-up-pattern | pipeline | 1% → 10% → 25% → 50% → 100%, 단계당 ≥24h, 가드레일 자동 abort | ✅ 커버 | §45 |
| §H.random-exploration-5pct | bias | feedback loop 깨는 random 트래픽 (§G MAB 와 통합) | ✅ 커버 | §45 (§42~§44 연계) |
| §H.offline-overfitting-popular | anti-pattern | LTR 의 popularity feature 과적합 → long-tail 사망 | ✅ 커버 | §45 |
| §H.offline-overfitting-diversity | anti-pattern | nDCG 좋지만 intra-list diversity 붕괴 — MMR 후처리 후보 | ✅ 커버 | §45 |
| §H.offline-overfitting-leakage | anti-pattern | query-level split 안 한 train/test leak — nDCG 0.95 비현실 신호 | ✅ 커버 | §45 |
| §H.click-model-pbm-dbn | bias | Position-Based Model / Dynamic Bayesian Network click model | ⏳ 후보 | — |
| §H.ope-estimator-family | bias | IPS / DR / SNIPS / Switch / WIS — counterfactual estimator 카탈로그 | ⏳ 후보 | — |

### 후속 후보 (Phase 2)

- **§H.click-model-pbm-dbn** — PBM / Cascade / DBN / UBM 클릭 모델 상세
- **§H.ope-estimator-family** — IPS / DR / SNIPS / Switch / WIS 비교
- **§H.diversity-metric** — intra-list diversity, ILD, α-nDCG, ERR-IA
- **§H.fatigue-saturation** — 사용자 fatigue / saturation effect 모델링
