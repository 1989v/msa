...
| 3 (반기) | **ADR-0043** : 검색 결과 랭킹의 온라인 학습 트랙 (Thompson Sampling) | 1 sprint | 중간 | 신상품 노출률 ↑, CTR 자동 최적화 |
...
## ADR-0043: 검색 결과 랭킹의 온라인 학습 트랙 — Thompson Sampling MAB rerank

**Status**: Proposed (2026-05-12) — `docs/adr/ADR-0043-search-online-bandit-thompson.md` 로 승격됨
**Deciders**: backend-platform, search-team
**Related**: ADR-0025 (latency budget), ADR-0012 (idempotent consumer), 본 파일 ADR-XXXX-4 (Hybrid)

### Context

본 §19 의 ADR 4건 + 추가 12건은 **오프라인 트랙** (BM25 / function_score / Hybrid / LTR) 의 점검·신설 후보. 그러나 §42~§44 로 신설된 **온라인 학습 트랙** (MAB / Thompson Sampling) 은 별도 ADR 이 필요하다.

문제:
- `ProductEsDocument.ctr: Double` 단일 스칼라만 저장 → uncertainty 소실 (1/2 와 500/1000 의 차이 표현 ❌)
- deterministic function_score → exploration 메커니즘 부재 → 신상품 cold-start
- 글로벌 가중치 (`popularityWeight`, `ctrWeight`) 만 외부화 → 카테고리/세그먼트 sensitivity 학습 ❌

### Decision

ADR-0043 `docs/adr/ADR-0043-search-online-bandit-thompson.md` 채택. 핵심:

- arm 키: `(categoryId, productId)`
- state: Redis hash `bandit:state:{cat}:{pid} = {clicks, impressions, lastTs}`
- 알고리즘: Beta-Bernoulli conjugate posterior + Thompson Sampling
- rerank: ES function_score top-N → ThompsonReranker (top-K) hybrid (`w · esNorm + (1−w) · sample`)
- 새 Kafka 토픽 3종: `search.impression.logged`, `search.click.logged`, `analytics.bandit.state.snapshot`
- ProductEsDocument 에 `categoryId` keyword 필드 추가 → alias swap 1회 reindex
- 튜닝 노브 8종 (`BanditProperties`)

### Consequences

**Positive:**
- 신상품 노출률 SLA 정의 가능
- A/B 비교 (TS off vs on) + nDCG / CTR / CVR 단계 평가
- prior/decay/hybrid 가중치 hot reload
- 카테고리별 분리 학습

**Negative / Risk:**
- 추가 latency (~10ms, ADR-0025 안에서 운영)
- flicker (session cache mitigation)
- position bias (Phase 3 IPW 도입 후보)
- Redis 의존성 (graceful degradation 설계됨)

### Phases

| Phase | 범위 |
|---|---|
| P1 | 본 ADR + study 3 deep file + search:domain port + Redis adapter + ThompsonReranker + impression/click endpoint + analytics consumer + ProductEsDocument.categoryId |
| P2 | FE/gateway 자동 impression tracking, dashboard, A/B 토글, prior auto-tune |
| P3 | Contextual Bandit (LinUCB) + IPW position bias + LTR cascade 통합 |

---

## 다음 학습

- [42-bayesian-beta-thompson.md](42-bayesian-beta-thompson.md) — 본 ADR 의 수학적 토대
- [43-mab-algorithms.md](43-mab-algorithms.md) — TS 외 알고리즘 비교
- [44-msa-bandit-grounding.md](44-msa-bandit-grounding.md) — 본 ADR 의 코드 grounding
