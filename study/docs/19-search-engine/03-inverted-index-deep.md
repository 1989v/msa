...
RDB (Relational Database, 관계형 데이터베이스) 의 인덱스는 row → row, 검색엔진의 인덱스는 term → row set. 비교 대상이 비대칭. 이 비대칭성이 LIKE '%foo%' 가 안 되는 근본 이유.
...
- **term frequency** (".doc") — 각 doc 에서 term 등장 횟수 (BM25 (Best Match 25) 의 tf)
- **positions** (".pos") — 각 doc 내 term 의 위치 (phrase query 용)
- **payloads** (".pay") — term 별 부가 정보 (LTR (Learning to Rank, 랭킹 학습) feature 등에 활용)
