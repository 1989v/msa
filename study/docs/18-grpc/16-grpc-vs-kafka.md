...
> 면접 단골: "둘 차이가 뭐예요?" — 본질은 **시간 결합 / 위치 결합 / 영속성** 의 차이 (gRPC = 동기 RPC (Remote Procedure Call, 원격 프로시저 호출)).
...
ADR (Architecture Decision Record, 아키텍처 결정 기록) -0003 의 결정:
...
A: gRPC = 동기 RPC, 시간/위치 결합 강함, 영속성 없음, 1:1 호출. Kafka = 비동기 이벤트 로그, 시간/위치 결합 약함, 영속 + 재처리, 1:N pub/sub. 본질적으로 다른 도구이며 **둘 다 함께 사용** 이 표준 (CQRS (Command Query Responsibility Segregation, 명령-조회 책임 분리), Outbox, Saga).
