...
**한 줄**: REST (Representational State Transfer) 는 *리소스 표현*, gRPC 는 *함수 호출 추상화* (gRPC = gRPC Remote Procedure Call, Google 발 RPC (Remote Procedure Call, 원격 프로시저 호출) 프레임워크).
...
- DTO (Data Transfer Object, 데이터 전송 객체) / 스키마 / 문서가 모두 proto 에서 파생 → drift 0
...
**깊이**:
- 원인: HTTP/2 는 1 TCP 위 다수 stream. ClusterIP service 는 L4 (TCP) LB 라 connection 단위 분배만. 한 connection = 한 pod (K8s = Kubernetes).
- 해법:
...
- ADR (Architecture Decision Record, 아키텍처 결정 기록) -0025 의 fan-out tail 보호의 핵심 도구
...
- PERMISSION_DENIED (7): 인증은 OK 지만 권한 없음. RBAC (Role-Based Access Control, 역할 기반 접근 제어) 거부.
...
**한 줄**: mTLS (mutual TLS, 양방향 TLS) = 서비스 신원, JWT (JSON Web Token) = 사용자 신원.
...
4. 제외: 외부 API (PG, OAuth (Open Authorization, 인가 프로토콜)), 비동기 (Kafka 자리), 트래픽 적은 호출
...
**한 줄**: schema-first MSA (Microservices Architecture, 마이크로서비스 아키텍처) 내부 통신의 표준. 점진 도입, 외부는 REST 유지.
