...
> 학습자 수준: 중급(intermediate, gRPC 이론 +α / 실 운영 미경험) · 전체 예상 시간: 14h · 목표: 면접 대비 + msa 도입 ADR 초안 작성
...
3. **K8s 에서 gRPC LB 가 안 되는 이유** = ClusterIP 는 L4 → 단일 TCP 위 다중 stream 을 한 pod 가 독식.
