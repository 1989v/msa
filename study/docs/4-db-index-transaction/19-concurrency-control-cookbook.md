...
| **MSA Outbox 표준** | (event_id) UNIQUE 가 inbox 패턴의 핵심. Eventuate / Debezium / Axon 모두 동일. |
...
- lock 보유 시간 = 트랜잭션 길이 → **외부 IO 절대 금지** (ADR-0020).
...
**대안**:
- K8s Deployment replicas=1 — 가장 단순. **가용성 손실** trade-off.
...
- Pessimistic Lock — 직렬화 → throughput 1 TPS 수준 → 사용자 timeout 폭발.
...
**대안**:
- 자연키 (orderId) UNIQUE 만 — 사용자가 매번 같은 orderId 보낼 수 있어야 함 (UI/UX 제약).
...
- Debezium CDC — outbox 테이블의 binlog 를 Kafka 로 직접 → polling 자체 불필요.
