...
RDBMS (특히 MySQL/InnoDB) 의 인덱스 구조, 실행 계획 분석, 트랜잭션 격리 수준과 MVCC (Multi-Version Concurrency Control, 다중 버전 동시성 제어), Lock 매커니즘을 10년차 수준으로 학습한다. 거의 모든 백엔드 면접의 단골 주제이며, 실무에서도 성능/장애의 70% 이상이 DB 에서 발생한다.
...
- ACID (Atomicity / Consistency / Isolation / Durability, 원자성·일관성·격리성·내구성), 4가지 격리 수준 (READ UNCOMMITTED, READ COMMITTED, REPEATABLE READ, SERIALIZABLE) 차이
- MVCC 동작 원리 (Undo log, Read View)
- Lock 단위 (Row Lock vs Table Lock) 와 InnoDB 가 Row Lock 을 기본으로 쓰는 이유
- Lock 모드 (S-Lock / X-Lock / IS / IX) 와 호환성 매트릭스
- InnoDB Lock 종류 (Record Lock, Gap Lock, Next-Key Lock, Insert Intention Lock)
- 메타데이터 락 (MDL (Metadata Lock, 메타데이터 락)) 과 DDL (Data Definition Language) 이 트래픽에 끼치는 영향
...
- Lock 의 기본 모드: Shared Lock (S-Lock, 읽기 공유) vs Exclusive Lock (X-Lock, 쓰기 배타)
- Lock 단위: Row Lock vs Table Lock vs Page Lock — 엔진별 차이 (InnoDB=Row, MyISAM=Table)
- 명시적 잠금 구문: `SELECT ... FOR UPDATE` (X-Lock), `SELECT ... LOCK IN SHARE MODE` / `FOR SHARE` (S-Lock)
...
- Intention Lock (IS / IX): Row-level Lock 과 Table-level Lock 을 공존시키는 멀티-그래뉼래리티 메커니즘
- Lock 호환성 매트릭스 (S/X/IS/IX 의 충돌 여부)
- Auto-Increment Lock (AUTO-INC), `innodb_autoinc_lock_mode` 0/1/2 차이
- Metadata Lock (MDL): DDL 실행 시 long-running TX 가 만드는 잠금 사슬
- `SHOW ENGINE INNODB STATUS`, `performance_schema.data_locks` / `data_lock_waits` 로 Lock 추적
...
- "S-Lock 과 X-Lock 의 차이? `SELECT ... FOR UPDATE` 는 어떤 락인가요?"
- "Intention Lock (IS/IX) 은 왜 필요한가요? 없으면 어떤 문제가 생기나요?"
- "Row Lock 과 Table Lock 은 언제 각각 사용되나요? Lock Escalation 이 InnoDB 에 있나요?"
- "MDL (Metadata Lock) 때문에 DDL 이 행 (block) 되는 시나리오와 회피 방법?"
...
- DB 범위: MySQL 집중 vs PostgreSQL/Oracle 도 비교?
  - ✅ 결정: MySQL/InnoDB 집중. 16KiB 페이지/Gap-Lock/Next-key/MVCC 가 전 deep file 의 표준 (Phase 1-2 전체).
- 실습: 실제 테이블에 인덱스 추가/EXPLAIN 실행?
  - ✅ 결정: msa 11개 서비스 엔티티 + Flyway DDL 전수 분석 (`14-msa-entities.md`) + slow 후보 도출 (`15-msa-queries.md`). EXPLAIN 사용 패턴은 `09-explain.md` 에서 type/key/rows/Extra 표준화.
- Deadlock 재현 실습 포함 여부
  - ✅ 결정: `08-deadlock-mdl.md` 에서 wait-for graph + SHOW ENGINE INNODB STATUS LATEST DETECTED DEADLOCK + 2 세션 재현 SQL 까지 정리.
- JPA + N+1 문제 심화 포함 여부
  - ⏸️ 미결 (이유: 본 학습은 InnoDB 물리 계층 + lock/MVCC 에 집중. JPA 의 fetch 전략 / N+1 은 Hibernate 영역으로 별도 학습 토픽 후보. preview 의 "Kotlin/JPA 특유 함정" 섹션에 OIV / LazyLoading 함정만 메타 수준으로 언급).
