...
> 시스템 설계 시나리오 중 **가장 동시성이 극한**인 케이스. **대기열, 좌석 선점 TTL (Time To Live, 생존 시간), 오버셀링 방지**가 3대 난제. 한국 인터파크/티켓링크 폭주 트래픽 사례 단골.
...
피크 QPS (Queries Per Second, 초당 쿼리 수):
  - 좌석 조회: 100만 × 1 req/s = 1M RPS (Requests Per Second, 초당 요청 수)
...
> 100만 동시는 **단일 서버 절대 불가**. CDN (Content Delivery Network, 콘텐츠 전송 네트워크) + 대기열 + 분산 락이 필수.
