...
생성자에서 reflection 호출이나 외부 IO (Input/Output, 입출력) 가 일어나면 역직렬화만으로 부작용 발생. 도메인 객체 생성은 가능한 한 단순하게.
...
→ 파서가 재귀 호출하다 StackOverflow 또는 OOM (Out Of Memory, 메모리 부족). Jackson 은 기본 `MAX_DEPTH` 제한이 없어서 명시 필요.
