...
Shenandoah 는 **Red Hat OpenJDK** 가 만든 저지연 GC (Garbage Collection, 가비지 컬렉션). ZGC (Z Garbage Collector) 와 비슷한 목표(짧은 pause, concurrent compact) 지만 **Brooks Pointer** 라는 다른 방식. 객체 헤더 앞에 1 word(8 byte) 의 **forwarding pointer** 를 두어 객체 이동 시 여기를 갱신. ZGC 와 비교한 강점은 **Compressed Oops 호환** 과 **작은 힙(2-4GB) 에서도 효율**. 단점은 객체당 1 word 추가 메타. JDK 17+ 부터 OpenJDK 본가 포함, **배포본별 가용성 차이**가 채택 시 검토 포인트.
...
Shenandoah 도 마킹용 SATB write barrier 가 있다. ZGC 와 G1 (Garbage-First Collector) 의 중간.
