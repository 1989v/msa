...
- 6 영역 × 8문항 + 영역 7 (Cookbook) 3문항 = **51 카드** + 함정 4 = **55** (50문항 목표 초과)
...
## 영역 7: G1GC 튜닝 Cookbook (23번 추가)

### Q-NEW-1. **G1 에서 Young generation 이 핵심인 이유?**

**A**: Pause time 의 80%+ 가 Young GC 에서 발생. Mixed GC 도 Young + 일부 Old 라서 Young evacuation 시간이 그대로 들어감. 약한 세대 가설로 99% 가 Young 에서 죽기 때문에 Young 사이즈 / 효율 = G1 전체 성능. Old 는 Mixed 로 incremental 하게 돌아 직접 부담은 작지만, Young 이 잘못 잡히면 promotion 폭증 + 빈도 ↑로 Full GC 까지 갈 수 있다.

- Q-1: Young 사이즈가 작으면 / 크면?
  - 작음: pause ↓ but GC 빈발, throughput ↓, promotion 폭증
  - 큼: throughput ↑ but pause 길어짐, promotion 압박
- Q-2: G1 의 트릭 핵심은?
  - "Young 에서 죽이고 Old 로 promote 안 시키기" — Eden 비율 / age threshold / Survivor 크기 모두 이 목적
- 함정: "Old GC 가 더 느리니 Old 만 잘 튜닝" — Young 이 먼저. 23번 시나리오 3 참조

### Q-NEW-2. **System.gc() 처리 옵션 차이?**

**A**: 3가지:
- `-XX:+ExplicitGCInvokesConcurrent` — `System.gc()` 를 G1 concurrent mark cycle 로 변환. STW 거의 없음. **운영 default**
- `-XX:+DisableExplicitGC` — `System.gc()` no-op. 단 NIO DirectBuffer 정리 의존 라이브러리에서 native 메모리 누수 가능
- `-Dsun.rmi.dgc.server.gcInterval=86400000` — RMI distributed GC 주기 1h → 24h (RMI 사용 시만)

- Q-1: msa 에서 어떤 게 안전?
  - DirectBuffer 사용 (Netty / Kafka client) 있으니 ExplicitGCInvokesConcurrent. DisableExplicitGC 는 risk
- Q-2: 호출 stack 추적?
  - JFR `jdk.SystemGC` 이벤트로 호출 위치 확인
- 함정: "DisableExplicitGC 가 가장 안전" — NIO 호환성 risk. ExplicitGCInvokesConcurrent 가 default

### Q-NEW-3. **GC 로그의 Sys time 이 User time 보다 클 때?**

**A**: `User=0.10s Sys=0.30s Real=0.05s` 같이 Sys >> User 면 OS kernel 작업이 GC 시간 dominant. 원인 6가지:
1. swap (보통 K8s 노드는 off)
2. page fault (lazy alloc)
3. NUMA migration
4. THP defrag (transparent hugepages 의 khugepaged)
5. 디스크 IO 경합 (GC 로그 + heap dump 같은 디스크)
6. JIT compile burst

대응:
- `-XX:+AlwaysPreTouch` — runtime page fault 0
- `-XX:+UseLargePages` + `hugepages-2Mi` resource — TLB miss ↓
- THP off (노드 sysctl)
- `-XX:+UseNUMA` 또는 `numactl` 핀
- `-XX:CICompilerCount=2` — JIT 스레드 ↓
- IO 분리 (NVMe / tmpfs)

- Q-1: AlwaysPreTouch 의 trade?
  - 부팅 시간 ↑ (큰 힙 수십 초). K8s readinessProbe initialDelaySeconds 로 흡수
- Q-2: msa 에서 우선 검토?
  - quant 같은 latency 민감 서비스만 AlwaysPreTouch + UseLargePages. 일반 API 는 무가치
- 함정: "Sys 크면 무조건 swap" — THP / NUMA / 디스크 IO / JIT 도 원인. /proc/meminfo + numactl 같이 봐야
