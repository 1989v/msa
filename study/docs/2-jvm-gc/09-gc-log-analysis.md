...
GC (Garbage Collection, 가비지 컬렉션) 로그는 **모든 튜닝의 출발점**. JDK 9+ 부터 통합 로그(`-Xlog:gc*`) 가 기본. 핵심 지표는 **Pause Time / Throughput / Allocation Rate / Promotion Rate / Heap Occupancy 추이** 5가지. 면접에서 "GC 로그 어떻게 봤냐" 질문은 이 5가지를 순서대로 답하면 된다. 도구는 **GCViewer** (로컬 GUI), **GCEasy.io** (웹, 자동 분석 리포트), **gceasy CLI** (배치). 로그를 안 남기고 운영하면 장애 후 원인 진단이 거의 불가능 — 무조건 켜야 하는 운영 옵션.
...
mutator 가 ZGC (Z Garbage Collector) 를 못 기다림. 자주 보이면 ConcGCThreads 증가 또는 Xmx 증가.
...
- **JVM (Java Virtual Machine, 자바 가상 머신) Memory Size** — Young/Old/Metaspace 변화
...
A:
1. **G1 (Garbage-First Collector) 면 `-Xlog:gc*:file=gc.log` 로 수집** 합니다 — JDK 9 통합 로그.
