...
Route 53 은 AWS 의 **관리형 DNS (Domain Name System, 도메인 이름 시스템) 서비스**. 레코드 관리 + 라우팅 정책 + Health Check 통합. Private Hosted Zone 으로 VPC (Virtual Private Cloud, 가상 사설 클라우드) 내부 DNS 도 제공.
...
**Private Hosted Zone**:
- VPC 내부 전용
- 여러 VPC 공유 가능 (Peering/TGW (Transit Gateway) 불필요)
...
- VPC 의 기본 DNS 서버: **VPC CIDR (Classless Inter-Domain Routing) +2** (예: `10.0.0.2`)
- EC2 (Elastic Compute Cloud, 가상 머신 서비스)/Pod 내부에서 이 DNS 사용
...
EKS 환경에서는 K8s (Kubernetes) 자체 DNS (`.svc.cluster.local`) 가 있지만, **클러스터 외부**(Lambda, 다른 VPC)에서 접근 시 Route 53 Private Zone 필요.
...
- EKS 이전 시 `api.commerce.example.com` → Public Alias → ALB (Application Load Balancer, 애플리케이션 로드 밸런서)
...
**Q15 답변**: Alias 는 Route 53 전용으로 AWS 리소스 (ALB/NLB (Network Load Balancer, 네트워크 로드 밸런서)/CloudFront/S3 (Simple Storage Service, 객체 스토리지)) 를 지정할 때 사용. 루트 도메인에도 쓸 수 있고 쿼리 비용이 무료.
