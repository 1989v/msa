...
- 연결 대상: EC2 (Elastic Compute Cloud, 가상 머신 서비스) ENI, NAT (Network Address Translation, 네트워크 주소 변환) Gateway, ALB (Application Load Balancer, 애플리케이션 로드 밸런서)/NLB (Network Load Balancer, 네트워크 로드 밸런서) (간접), Direct Connect Gateway
...
- EIP 는 **1:1 NAT** 로 동작 (IGW (Internet Gateway) 에서 수행되는 NAT 와 같음)
...
- 라우팅/CIDR (Classless Inter-Domain Routing) 관리 복잡, AS (Autonomous System) 번호 필요
...
- 현재 msa 는 EIP 미사용 (로컬 K8s (Kubernetes))
- EKS 이전 시:
  - NAT GW: EIP 필수 (AZ (Availability Zone, 가용 영역) 별 3개)
...
**Q8-3-1 답변**: Duo-stack 부터 시작 (IPv4 + IPv6 병행), 클라이언트 지원 확인, Route 53 AAAA 레코드 추가, SG (Security Group, 보안 그룹)/NACL (Network Access Control List, 네트워크 ACL) 규칙 IPv6 버전 추가. 완전 IPv6 전환은 드묾.
