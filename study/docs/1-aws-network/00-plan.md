...
AWS 환경에서 서비스를 운영하기 위한 네트워크 인프라 전반을 학습한다. VPC (Virtual Private Cloud, 가상 사설 클라우드) 설계부터 서브넷 분리, 보안 그룹/NACL (Network Access Control List, 네트워크 ACL)을 통한 방화벽 구성, NAT (Network Address Translation, 네트워크 주소 변환) Gateway를 통한 아웃바운드 트래픽 관리, ALB (Application Load Balancer, 애플리케이션 로드 밸런서)/NLB (Network Load Balancer, 네트워크 로드 밸런서)를 통한 로드 밸런싱까지 프로덕션 환경에서 필수적인 네트워크 아키텍처를 다룬다.

현재 msa 프로젝트가 K8s (Kubernetes)(k3d 로컬 / managed K8s 프로덕션) 기반으로 운영되고 있어, AWS managed K8s(EKS) 환경에서의 네트워크 설계와 직접적으로 연결된다.
...
- **현재 수준**: 기초 (VPC/서브넷 개념은 들어봤지만 직접 설계 경험 없음, SG (Security Group, 보안 그룹) vs NACL 혼동)
...
- **Phase 2 EKS 영역**: VPC CNI (Container Network Interface, 컨테이너 네트워크 인터페이스) 플러그인, Pod/Service 네트워킹, K8s Service type → AWS LB 매핑, Ingress Controller
