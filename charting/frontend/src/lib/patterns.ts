...
export const PATTERNS: PatternDefinition[] = [
  ...
  },
  {
    ...
    signal: 'bearish',
    color: '#f97316',
    description: '상승 추세에서 고점과 저점이 수렴하며 올라가는 패턴. 상단 돌파 실패 시 하락 전환.',
    prediction: '수렴 후 하방 이탈 가능성',
    accuracy: 68,
    keyPoints: ['고점과 저점이 동시에 상승', '추세선이 수렴', '거래량 감소 동반'],
    curve: [
      { x: 0.00, y: 0.20 }, { x: 0.10, y: 0.38 }, { x: 0.18, y: 0.30 },
      { x: 0.30, y: 0.52 }, { x: 0.40, y: 0.46 }, { x: 0.52, y: 0.64 },
      { x: 0.62, y: 0.60 }, { x: 0.74, y: 0.72 }, { x: 0.84, y: 0.70 },
      { x: 1.00, y: 0.78 },
    ],
    projection: [
      { x: 1.00, y: 0.78 }, { x: 1.08, y: 0.65 }, { x: 1.16, y: 0.50 },
      { x: 1.25, y: 0.36 }, { x: 1.33, y: 0.25 },
    ],
  },
  {
    id: 'falling_wedge',
    ...
    signal: 'bullish',
    color: '#22c55e',
    description: '하락 추세에서 고점과 저점이 수렴하며 내려가는 패턴. 하단 이탈 실패 시 상승 전환.',
    prediction: '수렴 후 상방 이탈 가능성',
    accuracy: 72,
    keyPoints: ['고점과 저점이 동시에 하락', '추세선이 수렴', '거래량 감소 동반'],
    curve: [
      { x: 0.00, y: 0.80 }, { x: 0.10, y: 0.62 }, { x: 0.18, y: 0.70 },
      { x: 0.30, y: 0.52 }, { x: 0.40, y: 0.58 }, { x: 0.52, y: 0.44 },
      { x: 0.62, y: 0.48 }, { x: 0.74, y: 0.36 }, { x: 0.84, y: 0.38 },
      { x: 1.00, y: 0.28 },
    ],
    projection: [
      { x: 1.00, y: 0.28 }, { x: 1.08, y: 0.40 }, { x: 1.16, y: 0.54 },
      { x: 1.25, y: 0.68 }, { x: 1.33, y: 0.80 },
    ],
  },
  {
    id: 'bull_pennant',
    name: '상승 페넌트',
    nameEn: 'Bullish Pennant',
    signal: 'bullish',
    color: '#06b6d4',
    description: '급등 후 삼각형 수렴 구간을 거쳐 추가 상승하는 패턴.',
    prediction: '깃발 후 추가 상승 가능',
    accuracy: 70,
    keyPoints: ['선행 급등(깃대)', '삼각 수렴(페넌트)', '거래량 감소 후 돌파'],
    curve: [
      { x: 0.00, y: 0.10 }, { x: 0.08, y: 0.30 }, { x: 0.15, y: 0.55 },
      { x: 0.22, y: 0.80 }, { x: 0.32, y: 0.72 }, { x: 0.42, y: 0.66 },
      { x: 0.52, y: 0.62 }, { x: 0.62, y: 0.65 }, { x: 0.72, y: 0.61 },
      { x: 0.82, y: 0.63 }, { x: 1.00, y: 0.62 },
    ],
    projection: [
      { x: 1.00, y: 0.62 }, { x: 1.08, y: 0.74 }, { x: 1.16, y: 0.86 },
      { x: 1.25, y: 0.96 }, { x: 1.33, y: 1.05 },
    ],
  },
  {
    id: 'bear_pennant',
    name: '하락 페넌트',
    nameEn: 'Bearish Pennant',
    signal: 'bearish',
    color: '#dc2626',
    description: '급락 후 삼각형 수렴 구간을 거쳐 추가 하락하는 패턴.',
    prediction: '깃발 후 추가 하락 가능',
    accuracy: 68,
    keyPoints: ['선행 급락(깃대)', '삼각 수렴(페넌트)', '거래량 감소 후 이탈'],
    curve: [
      { x: 0.00, y: 0.90 }, { x: 0.08, y: 0.72 }, { x: 0.15, y: 0.48 },
      { x: 0.22, y: 0.22 }, { x: 0.32, y: 0.30 }, { x: 0.42, y: 0.36 },
      { x: 0.52, y: 0.40 }, { x: 0.62, y: 0.36 }, { x: 0.72, y: 0.38 },
      { x: 0.82, y: 0.36 }, { x: 1.00, y: 0.37 },
    ],
    projection: [
      { x: 1.00, y: 0.37 }, { x: 1.08, y: 0.26 }, { x: 1.16, y: 0.16 },
      { x: 1.25, y: 0.08 }, { x: 1.33, y: 0.04 },
    ],
  },
  {
    id: 'rounding_bottom',
    ...
    signal: 'bullish',
    color: '#8b5cf6',
    description: '완만한 U자 곡선을 그리며 바닥을 형성하는 장기 전환 패턴.',
    prediction: '완만한 상승 전환 예상',
    accuracy: 75,
    keyPoints: ['긴 기간 완만한 하락', '바닥에서 횡보', '완만한 상승 전환'],
    curve: [
      { x: 0.00, y: 0.75 }, { x: 0.10, y: 0.62 }, { x: 0.20, y: 0.50 },
      { x: 0.30, y: 0.38 }, { x: 0.40, y: 0.28 }, { x: 0.50, y: 0.22 },
      { x: 0.60, y: 0.24 }, { x: 0.70, y: 0.32 }, { x: 0.80, y: 0.44 },
      { x: 0.90, y: 0.58 }, { x: 1.00, y: 0.72 },
    ],
    projection: [
      { x: 1.00, y: 0.72 }, { x: 1.08, y: 0.82 }, { x: 1.16, y: 0.90 },
      { x: 1.25, y: 0.97 }, { x: 1.33, y: 1.03 },
    ],
  },
  {
    id: 'triple_top',
    ...
    signal: 'bearish',
    color: '#ef4444',
    description: '비슷한 가격대에서 3번 저항을 받고 하락하는 강력한 반전 패턴.',
    prediction: '3중 저항 후 하락 전환',
    accuracy: 78,
    keyPoints: ['3번의 유사한 고점', '지지선 하향 돌파', '거래량 패턴 확인'],
    curve: [
      { x: 0.00, y: 0.35 }, { x: 0.12, y: 0.82 }, { x: 0.22, y: 0.55 },
      { x: 0.32, y: 0.48 }, { x: 0.45, y: 0.80 }, { x: 0.55, y: 0.52 },
      { x: 0.65, y: 0.46 }, { x: 0.75, y: 0.79 }, { x: 0.85, y: 0.58 },
      { x: 1.00, y: 0.52 },
    ],
    projection: [
      { x: 1.00, y: 0.52 }, { x: 1.08, y: 0.40 }, { x: 1.16, y: 0.28 },
      { x: 1.25, y: 0.18 }, { x: 1.33, y: 0.12 },
    ],
  },
  {
    id: 'triple_bottom',
    ...
    signal: 'bullish',
    color: '#3b82f6',
    description: '비슷한 가격대에서 3번 지지를 받고 상승하는 강력한 반전 패턴.',
    prediction: '3중 지지 후 상승 전환',
    accuracy: 80,
    keyPoints: ['3번의 유사한 저점', '저항선 상향 돌파', '거래량 증가 동반'],
    curve: [
      { x: 0.00, y: 0.65 }, { x: 0.12, y: 0.22 }, { x: 0.22, y: 0.48 },
      { x: 0.32, y: 0.55 }, { x: 0.45, y: 0.20 }, { x: 0.55, y: 0.46 },
      { x: 0.65, y: 0.52 }, { x: 0.75, y: 0.22 }, { x: 0.85, y: 0.44 },
      { x: 1.00, y: 0.50 },
    ],
    projection: [
      { x: 1.00, y: 0.50 }, { x: 1.08, y: 0.62 }, { x: 1.16, y: 0.75 },
      { x: 1.25, y: 0.86 }, { x: 1.33, y: 0.95 },
    ],
  },
  {
    ...
    signal: 'bullish',
    color: '#84cc16',
    description: '평행한 상승 추세선 안에서 가격이 움직이는 패턴.',
    prediction: '채널 내 상승 추세 지속',
    accuracy: 66,
    keyPoints: ['평행한 상승 추세선', '채널 하단 지지 확인', '채널 상단 돌파 시 가속'],
    curve: [
      { x: 0.00, y: 0.20 }, { x: 0.10, y: 0.42 }, { x: 0.20, y: 0.30 },
      { x: 0.30, y: 0.54 }, { x: 0.42, y: 0.44 }, { x: 0.52, y: 0.66 },
      { x: 0.62, y: 0.56 }, { x: 0.74, y: 0.76 }, { x: 0.84, y: 0.68 },
      { x: 1.00, y: 0.88 },
    ],
    projection: [
      { x: 1.00, y: 0.88 }, { x: 1.08, y: 0.96 }, { x: 1.16, y: 1.02 },
      { x: 1.25, y: 1.08 }, { x: 1.33, y: 1.12 },
    ],
  },
]
