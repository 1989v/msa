 ...
 *
 * `spring.datasource.blog.url` 이 있을 때만 켜진다 — 전환 기간에는 code-dictionary 와
 * content 가 동시에 blog 를 서빙해야 게이트웨이 전환 한 시점만 전환점이 된다. ③-C 에서 걷는다.
 *
 ...
 * 전환 기간에는 code-dictionary 도 그 이름을 알아야 해서 호스트 TM 을 같은 이름으로 한 번 더
 * 노출한다(CodeDictionaryJpaConfig, ③-C 에서 제거).
 */
@Configuration
@org.springframework.boot.autoconfigure.condition.ConditionalOnProperty(name = ["spring.datasource.blog.url"])
