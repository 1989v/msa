...
class BlogProfileService(
    ...
    fun find(identity: BlogIdentity): BlogProfileJpaEntity? =
    ...
    @Transactional
    fun requireWritableProfile(identity: BlogIdentity): BlogProfileJpaEntity {
        val memberId = requireMemberId(identity)
        val existing = profileRepository.findByMemberId(memberId)
        if (existing != null) {
            if (identity.isAdmin) {
                existing.toDomain().requireCanInteract()
                return existing
            }
            existing.toDomain().requireCanWrite()
            return existing
        }
        if (!identity.isAdmin) {
    ...
    @Transactional
    fun requireInteractiveProfile(identity: BlogIdentity, displayName: String?): BlogProfileJpaEntity {
        val memberId = requireMemberId(identity)
        val existing = profileRepository.findByMemberId(memberId)
        if (existing != null) {
            existing.toDomain().requireCanInteract()
        ...
        return profileRepository.save(
            BlogProfileJpaEntity(
                memberId = memberId,
                displayName = BlogProfile.newReader(memberId, name).displayName,
            ),
        )
    }
    ...
    fun response(profile: BlogProfileJpaEntity) = BlogProfileAdminResponse(
        id = profile.id ?: 0,
        ...
        postCount = profile.id?.let { postRepository.findAllByAuthorProfileIdOrderByIdDesc(it, COUNT_PROBE).totalElements } ?: 0,
    ...
    private companion object {
        ...
        val COUNT_PROBE = org.springframework.data.domain.PageRequest.of(0, 1)
    }
}
