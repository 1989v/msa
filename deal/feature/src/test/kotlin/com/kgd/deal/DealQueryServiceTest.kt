...
import com.kgd.deal.application.offer.service.DealQueryService
...
class DealQueryServiceTest : BehaviorSpec({
    ...
    fun category(id: Long, code: String, order: Int, status: DisplayStatus = DisplayStatus.OPEN) = DealCategory(
        id = id, code = code, label = code, tagline = null, status = status, orderNo = order,
    )

    fun offer(categoryId: Long, slug: String, revenueType: RevenueType, network: String?) = Offer(
    ...
    given("허브 한 화면을 그릴 때") {
        val categoryRepository = mockk<DealCategoryRepositoryPort>()
        val offerRepository = mockk<DealOfferRepositoryPort>()
        val service = DealQueryService(categoryRepository, offerRepository)

        every { categoryRepository.findAllByStatus(DisplayStatus.OPEN) } returns listOf(
        ...
        val sections = service.execute(now)
    ...
    given("이름·제공처로 검색할 때") {
        ...
        val sections = service.execute(SearchDealOffersUseCase.Query("트립", now))
    ...
    given("검색어가 비어 있으면") {
        ...
        then("저장소를 부르지 않는다 — 빈 질의로 전량 스캔이 돌면 안 된다") {
            service.execute(SearchDealOffersUseCase.Query("   ", now)) shouldBe emptyList()
    ...
    given("검색어에 LIKE 와일드카드가 섞여 들어오면") {
        ...
        every { categoryRepository.findAllByStatus(DisplayStatus.OPEN) } returns emptyList()
        every { offerRepository.searchVisible(capture(pattern), any()) } returns emptyList()

        service.execute(SearchDealOffersUseCase.Query("50%_할인", now))
    ...
    given("카테고리가 내려가 있으면") {
        ...
        every { categoryRepository.findByCode("travel") } returns category(1, "travel", 10, status = DisplayStatus.HOLD)

        then("오퍼를 조회조차 하지 않는다") {
            service.execute(GetDealOffersUseCase.Query("travel", now)) shouldBe emptyList()
