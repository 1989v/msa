 ...
 * 조건(`payment.pg`)을 지우면 기본 프로필에서도 웹훅 컨트롤러가 생겨 404 가 401 로 바뀐다.
 */
class PaymentPgSelectionSpec : BehaviorSpec({
    ...
    given("payment.pg=toss") {
        then("TOSS_SECRET_KEY 가 없으면 기동하지 않는다") {
            runner("PAYMENT_PG" to "toss", "TOSS_WEBHOOK_SECRET" to "whsec").run { ctx ->
                failureMessages(ctx.startupFailure.shouldNotBeNull()) shouldContain "TOSS_SECRET_KEY"
            }
        }
        then("TOSS_WEBHOOK_SECRET 가 없으면 기동하지 않는다") {
            runner("PAYMENT_PG" to "toss", "TOSS_SECRET_KEY" to "test_sk").run { ctx ->
                failureMessages(ctx.startupFailure.shouldNotBeNull()) shouldContain "TOSS_WEBHOOK_SECRET"
            }
        }
        then("키가 비어 있어도 기동하지 않는다") {
            runner("PAYMENT_PG" to "toss", "TOSS_SECRET_KEY" to " ", "TOSS_WEBHOOK_SECRET" to "whsec").run { ctx ->
                ctx.startupFailure.shouldNotBeNull()
            }
        }
        then("두 키가 있으면 토스 빈이 생기고 모의 PG 기록기는 없다 — 웹훅은 비밀 없이 401") {
            runner("PAYMENT_PG" to "toss", "TOSS_SECRET_KEY" to "test_sk", "TOSS_WEBHOOK_SECRET" to "whsec").run { ctx ->
                ctx.startupFailure shouldBe null
                ...
                webhookStatus(ctx) shouldBe 401
            }
        }
    }
}) {
    ...
    class WebhookProbe {
        @Bean
        fun resolvePaymentUseCase(): ResolvePaymentUseCase = mockk(relaxed = true)
