...
> **디자인 작업 전 [`docs/design/k-heritage.html`](docs/design/k-heritage.html) 를 먼저 연다.**
> 재료·표면·활자·여백·형태·상태·프리미티브가 **살아 있는 견본**으로 들어 있고, 원본 시안
> 13장이 함께 있다. 이 절은 규칙의 요약이고, 무엇이 어떻게 보이는지는 그 문서가 원본이다.

`1989v.com` 브랜드 면은 **고대비 기술 미니멀리즘과 한국 전통 공간 감각의 합**이다.
정서적 목표는 *따뜻한 정밀함* — 기술적으로 단단하되 장인의 손이 닿은 느낌.

**세 정경** — 라이트가 기본이고 다크가 파생인 구조가 **아니다.** 각각 별도 설계다.

| 정경 | 바탕 | 액션 | 구조 | 쓰는 곳 |
|---|---|---|---|---|
| 라이트 (전통) | 한지 `#F9F8F2` | 소나무 `#1A472A` | 기와 먹빛 | 브랜드·제품 면 |
| 다크 (Heritage Tech) | `#131313` | **청자 `#2B4B63`** | 황토 `#B38B6D` | 같은 화면의 야간 |
| 아케이드 | 송연 `#0A0A0A` | **연지 `#A2231D`** | 황토 | `/games/**` |
...
- 토큰 정의는 `portal-fe/src/styles/k-heritage.css` 한 곳. **`packages/design-system` 은
  건드리지 않는다** — 공유 토큰을 바꾸면 트레이딩·백오피스까지 따라 바뀐다.
- 켜는 방법은 `useHeritageSurface()`. `:root` 에 속성을 걸고 언마운트 시 되돌리며
  참조 카운트를 쓴다 — 라우트 전환에서 해제가 나중에 돌면 깜빡인다.
- 사용자 선택을 저장하고 없으면 시스템 설정을 따른다 (`useHeritageTheme`).

**브랜드 상수** — 팔레트에서 근사하면 그냥 다른 색이 된다. hex 로 고정한다.

| 토큰 | 값 | 이름 | 역할 |
|---|---|---|---|
| `--kh-hanji` | `#F9F8F2` | 한지 | 라이트 바탕·판 위 글씨 |
| `--kh-giwa` | `#1D1D1F` | 기와 | 먹빛 구조·머리띠·다크 판 |
| `--kh-ink` | `#0A0A0A` | 송연 | 아케이드 바탕 |
| `--kh-pine` | `#1A472A` | 소나무 | 라이트 액션 |
| `--kh-celadon` | `#2B4B63` | 청자 | **다크 액션** — 기와 유약에서 왔다 |
| `--kh-yeonji` | `#A2231D` | 연지 | 인장·라이트 강조 낱말·아케이드 액션 |
| `--kh-ocher` | `#B38B6D` | 황토 | 테두리·라벨·구조 강조 |
...
| `--kh-radius` | `4px` | 기본 |
| `--kh-radius-asym` | `0 8px 0 8px` | 좌상/우하 각짐, 우상/좌하 둥긂 |
| `--kh-display` | `clamp(2rem, 5vw, 3rem)` | 디스플레이 |
| `--kh-font-mono` | JetBrains Mono → 시스템 모노 | 기술 메타데이터 전용 |
| `--kh-grain-ink` | 면이 정한다 (밝은 면 먹, 판 위 흰선) | 결의 색 |

**프리미티브** (`.kh-*`)

| 클래스 | 무엇 |
|---|---|
| `.kh-slab` / `.kh-slab-offset` | 판 — 라이트에서도 어두운 카드 / 어긋난 판으로 만든 깊이 |
| `.kh-seal` / `.kh-seal-ink` | 인장 — 연지 낙관 / 먹빛 기본형 |
| `.kh-section-head` / `.kh-index` | 순번(`01_`)이 제목 앞, 아래로 전폭 괘선 |
| `.kh-section-label` | 황토색 모노 섹션 라벨 |
| `.kh-caps` | 대문자 라벨 — 본문 서체 캡스. 카드 모서리 표식 |
| `.kh-icon-box` | 아이콘 함 — 얇은 테두리 안의 작은 사각 |
| `.kh-display` / `.kh-display-accent` | 디스플레이 + **낱말 하나만** 연지 (다크는 색 없음) |
| `.kh-button` / `.kh-button-ghost` | 채움(라이트 먹빛 / 다크 청자) / 테두리만 |
| `.kh-field` · `.kh-status` · `.kh-table` | 입력 · 상태 · 표 |
...
- **한글에 대문자 변환·넓은 자간을 걸지 않는다.** 효과가 없고 자간만 벌어진다.
- **연지 강조 낱말은 라이트 전용.** 어두운 바탕의 붉은 글자는 강조가 아니라 경고로
  읽힌다. 다크의 강조 자리는 청자색 액션이 갖는다.
- 그림자를 쓰지 않는다. 깊이는 면의 색 단차와 어긋난 판으로 만든다.
- **처마 곡률은 폭을 따라 커진다.** 넓은 머리띠에 걸면 활처럼 휘므로 폭이 제한된 구획선에만 쓴다.
...
- **디스펜서는 터치 기기에서 스크롤로 돌리지 않는다.** 엄지 아래에서 판이 계속 움직이면 읽을 수 없고 관성 스크롤과
  ...
  **두 단계** — 판이 움직이는 동안(스핀·드래그·스크롤)에는 정면 부근의 카드가 덱에서 살짝(18px) 올라왔다 내려가기만 하고,
  멈춘 뒤(스핀 종료·스냅·스크롤 260ms 정지)에야 한 장이 완전히 나와 얼굴을 보인다. 돌 때마다 카드가 튀어나오면 움직임만 보이지 뽑힘이 안 보인다.
  **올라오는 것은 한 장이 아니라 정면 ±2.4칸의 무리다** — 한 장만 올리면 스핀이 한 칸을 수십 ms 만에 지나가 눈에 안 보인다
  (실측: 올라온 카드 중앙값 1장 → 4장). 물결이 판을 도는 것으로 읽혀야 뽑히는 과정이 보인다.
  **그림은 정면 ±2칸에, 판이 멈춘 뒤에 붙는다** — 판이 돌면 모든 카드가 정면을 지나므로 주소를 얼굴에 그대로 박으면
  스핀 한 번에 전부 내려받는다(실측 18장 → 49장, 지금은 10장 → 15장). 옆으로 선 카드의 그림은 어차피 안 보인다.
  **터치에서는 40장까지만 꽂고 먼 카드는 요소 둘(보이는 면 하나·옆면 하나)로 줄인다** — 카드 한 장이 컴포지터 레이어
  다섯 개라 80장짜리 판이 폰에서 400 레이어를 넘겨 스크롤이 버벅였다. 같은 이유로 **페이지 고정 캔버스를 콘텐츠 아래에
  깔지 않는다** — 그 위에 그려지는 것 전부가 별도 레이어로 승격된다. 터치의 먹 캔버스는 히어로 안에만 둔다.
