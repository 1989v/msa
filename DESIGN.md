...
| `/games/**` · `game.1989v.com` | `k-heritage` — 게임 카드만 판(어두움) 고정 |
...
| `.kh-mono` | 기술 메타데이터 표기 |
| `.kh-eave-edge` | 처마 곡선 구획선 — 처진 가운데, 들린 끝 |
| `.kh-grain` | 목재 결 텍스처 5% — 이미지 없이 CSS 로만 |
| `.kh-field` | 입력 — 테두리 없이 아래 선 하나. 포커스에 상호작용색 |
| `.kh-status` | 빈 상태 / 불러오는 중 / 실패 |
| `.kh-table` | 표 — 황토색 모노 머리줄, 세로선 없음 |
...
- **처마 곡률과 결 농도를 키우지 않는다.** 곡선은 20px 안쪽, 결은 5%. 넘기면 텍스처가
  아니라 무늬가 되고 즉시 유치해진다.
