...
class MemberTest : BehaviorSpec({
    Given("회원 생성") {
        When("소셜 식별값만으로 생성하면") {
            ...
            Then("표시 이름이 자동으로 붙은 회원이 생성된다") {
                member.name.shouldNotBeBlank()
        ...
        When("소셜 식별값이 비어있으면") {
            Then("예외가 발생한다") {
                // 식별값이 없으면 같은 사람을 다시 찾을 수 없다 — 유일한 조회 키다
                shouldThrow<IllegalArgumentException> {
                    Member.create(ssoProvider = SsoProvider.KAKAO, ssoProviderId = "")
                }
            }
        }
        ...
        When("표시 이름을 비워 넘기면") {
            Then("예외가 발생한다") {
                shouldThrow<IllegalArgumentException> {
                    Member.create(ssoProvider = SsoProvider.KAKAO, ssoProviderId = "123", name = "")
                }
            }
        }
    }
    ...
    Given("이름 수정") {
        val member = Member.create(ssoProvider = SsoProvider.KAKAO, ssoProviderId = "123")
