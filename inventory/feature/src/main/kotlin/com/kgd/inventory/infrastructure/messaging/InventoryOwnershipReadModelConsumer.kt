...
/**
 * `product.item.*` · `seller.seller.*` → 재고 REST 소유 판정 읽기 모델(`product_owner` · `owner_seller`).
 *
 * 같은 이벤트 재전달은 멱등 원장(`inventory-ownership` 그룹)이, 순서 역전은 `occurredAt` 비교가 거른다.
 * 판매자 컬럼 이전의 상품 이벤트에는 sellerId 가 없다 — 그때 상품은 전부 플랫폼 판매자(1)로 백필됐으므로 1 로 읽는다.
 */
@Component
class InventoryOwnershipReadModelConsumer(
    ...
    companion object {
        const val GROUP = "inventory-ownership"
