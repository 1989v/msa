...
import com.kgd.recommendation.application.recommendation.port.RankingPort
import com.kgd.recommendation.application.recommendation.port.UserMetadataPort
...
import com.kgd.recommendation.application.recommendation.usecase.GetPersonalizedUseCase.Companion.DEFAULT_VARIANT
import com.kgd.recommendation.application.recommendation.usecase.GetPersonalizedUseCase.Companion.MAX_LIMIT
import com.kgd.recommendation.application.recommendation.usecase.GetPersonalizedUseCase.Companion.MIN_ACTIONS_FOR_PERSONALIZED
import com.kgd.recommendation.application.recommendation.usecase.GetPersonalizedUseCase.Companion.RETRIEVAL_K_DEFAULT
...
class PersonalizedService(
    ...
    private val experimentClient: ExperimentVariantPort,
    private val banditPolicy: BanditPort,
    private val impressionPublisher: ImpressionPublisherPort,
    @Value("\${recommendation.experiment.id:0}") private val experimentId: Long,
) : GetPersonalizedUseCase {
    private val logger = KotlinLogging.logger {}

    override fun execute(query: GetPersonalizedUseCase.Query): Recommendation {
        val (userId, limit, defaultCityId, defaultCategoryId) = query
    ...
    private fun mergeWithCb(
        ...
        val cb = if (preferred != null) {
            categoryBest.execute(GetCategoryBestUseCase.Query(preferred.cityId, preferred.categoryId, limit))
        } else {
            categoryBest.execute(GetCategoryBestUseCase.Query(defaultCityId, defaultCategoryId, limit))
        }
