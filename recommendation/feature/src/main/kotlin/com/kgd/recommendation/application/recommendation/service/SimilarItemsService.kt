...
import com.kgd.recommendation.application.recommendation.usecase.GetSimilarItemsUseCase.Companion.MAX_LIMIT
...
class SimilarItemsService(
    ...
    override fun execute(query: GetSimilarItemsUseCase.Query): Recommendation {
        val (itemId, limit) = query
        ...
        if (metadata == null) {
            logger.warn { "SimilarItemsService: itemId=$itemId metadata 없음 — similar 결과만 반환 (size=${similar.size})" }
            return Recommendation(
                type = RecommendationType.SIMILAR_ITEMS,
                userId = null,
                context = RecommendationContext(sourceItemId = itemId),
