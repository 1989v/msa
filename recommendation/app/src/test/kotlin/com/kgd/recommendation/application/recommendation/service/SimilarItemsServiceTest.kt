...
class SimilarItemsServiceTest : BehaviorSpec({
    ...
    given("similar items 가 limit 의 50% 이상 (충분)") {
        ...
        val useCase = SimilarItemsService(similarity, metadata, repo)

        every { similarity.findSimilar(1001, 10) } returns (1..6).map { cfItem(2000L + it, 1.0 - it * 0.1) }

        `when`("execute(itemId=1001, limit=10)") {
            val result = useCase.execute(GetSimilarItemsUseCase.Query(1001, 10))
    ...
    given("similar items 가 부족 (sparse), metadata 있음") {
        ...
        `when`("execute(itemId=1002, limit=10)") {
            val result = useCase.execute(GetSimilarItemsUseCase.Query(1002, 10))
    ...
    given("similar items 부족 + CB 결과에 source itemId 자체가 포함") {
        ...
        `when`("execute") {
            val result = useCase.execute(GetSimilarItemsUseCase.Query(5005, 10))
    ...
    given("similar items 부족 + metadata 없음 (newer item)") {
        ...
        `when`("execute") {
            val result = useCase.execute(GetSimilarItemsUseCase.Query(9999, 10))
    ...
    given("invalid input") {
        val useCase = SimilarItemsService(mockk(), mockk(), mockk())
        `when`("itemId=0") {
            then("IllegalArgumentException") {
                shouldThrow<IllegalArgumentException> { useCase.execute(GetSimilarItemsUseCase.Query(0, 10)) }
            }
        }
        `when`("limit=0 또는 limit>100") {
            then("IllegalArgumentException") {
                shouldThrow<IllegalArgumentException> { useCase.execute(GetSimilarItemsUseCase.Query(1, 0)) }
                shouldThrow<IllegalArgumentException> { useCase.execute(GetSimilarItemsUseCase.Query(1, 101)) }
            }
        }
    }
})
