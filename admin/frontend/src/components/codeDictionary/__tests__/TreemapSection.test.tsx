...
import type { TreemapDataDto, Concept } from '@/api/codeDictionary';
...
vi.mock('@/api/codeDictionary', async () => {
  const actual = await vi.importActual<typeof import('@/api/codeDictionary')>(
    '@/api/codeDictionary',
  );
  return {
    ...actual,
...
import { fetchTreemapStats, fetchConceptByConceptId } from '@/api/codeDictionary';
