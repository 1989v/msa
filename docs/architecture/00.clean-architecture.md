...
위치: Application (`application/{entity}/usecase`). **단일 구현이라도 인터페이스를 생략하지 않는다** (ADR-0083).
...
위치: **Application** (`application/{entity}/port`) — ADR-0083 으로 확정. domain 모듈에 두지 않는다.
유일한 예외는 `search:domain` (`Page`/`Pageable` — `docs/conventions/package-structure.md`).
...
이 프로젝트는 Domain 레이어를 물리적으로 분리한 **Nested Submodule 구조**를 사용한다.
...
- Application → Infrastructure 직접 참조 0건 — 루트 `verifyLayerDependencies` 게이트가 빌드에서 검사 (ADR-0083)
