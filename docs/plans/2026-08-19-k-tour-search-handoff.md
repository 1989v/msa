...
| `궁궐` | 경복궁 없음 | 변화 없음 | **여전히 없음** — 2순위(유사어) 몫 |

세 신호가 각자 제 몫을 했다. 분류 가중치가 `한옥`을, 이름 접두가 `경복`을, 자모가 `경보`를
해결했다. **어느 하나로 다 되지 않았다** — 분류 체계가 새는 자리(`한복남 경복궁점`이 culture)는
분류로 못 가르고, 조합 중간 상태는 형태소로 못 잡는다.
