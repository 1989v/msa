...
> [!] **자동 스케줄을 임의로 걸지 않는다** (2026-08-17 사용자 결정). 클러스터는 엣지 하드닝으로
> 외부 API 를 못 부르고, 예외를 뚫는 비용 대비 급하지 않다. 로컬 수동 실행으로 둔다.
> 이어서 돌릴 때는 **사용자에게 실행 시점을 먼저 확인**한다.
...
- 주기 동기화 CronJob 화 — egress 정책 예외가 필요해 비용 판단이 선행
