...
| 언어 | 레코드 | 개요 보유 | 이미지 보유 |
|---|---|---|---|
| ko | 44,922 | 1,702 | 39,304 |
| en | 14,663 | 1,971 | 13,335 |
| **합계** | **59,585** | 3,673 | 52,639 |

**2026-08-20 갱신** — 전 분류 재동기화 후 법정동 코드 **99.9%** 채움(ko 44,889 · en 14,647).
행정구역 `administrative_regions` 285건(시도 16 · 시군구 269), 좌표 283 · 영문명 282.
개요는 재동기화 전후 **1,702 / 1,971 로 동일** — `syncFrom` 보호를 실측으로 확인했다.
지역 축은 `ldong_regn_cd`/`ldong_signgu_cd` 다. `area_code`/`sigungu_code` 는 두 체계가
섞여 있어 **필터 축으로 쓰지 않는다** (ADR-0071).
...
### 자모 자동완성 (2026-08-20 추가)

조합 중간 상태로도 맞아야 한다 — `경보`까지 친 상태에서 `경복궁`이 나와야 자동완성이다.
형태소 분석기는 완성 음절만 다루므로 nori 로는 안 된다.

`titleJamo` 필드에 자모로 편 이름을 색인하고(edge_ngram), 질의도 같은 `Jamo.decompose` 로 편다.
**색인과 질의가 같은 함수를 써야 한다** — 한쪽만 바뀌면 조용히 아무것도 안 맞는다.
플러그인을 쓰지 않은 이유: 관리형 이미지에 커뮤니티 플러그인을 넣으면 그게 배포의 제약이 된다.

가중치는 눌러서 섞는다.

| 신호 | 가중치 | 예 |
|---|---|---|
| 이름이 입력으로 시작 (`title.keyword`) | ×6 | `경복궁` vs `한복남 경복궁점` |
| 형태소 기준 일반 매칭 | ×1 | 기존 동작 |
| 자모(조합 중간 상태) | ×0.3 | `경보` → `ㄱㅕㅇㅂㅗ` |

`must` 가 아니라 `should` + `minimumShouldMatch(1)` 이다 — 자모로만 맞는 입력을 `must` 가
걸러버리면 자모 색인이 아무 일도 하지 못한다. 자모를 낮게 두는 건 그게 가장 헐거운 신호라서고,
같은 무게로 두면 자모로만 스친 문서가 이름이 정확히 맞는 문서를 밀어낸다.

### 재색인 후 최종 실측 (2026-08-20, 전체 재색인 59,585 docs)

| 쿼리 | 처음 | 분류 가중치 후 | 접두+자모 후 (현재) |
|---|---|---|---|
| `경복` | 한복남 경복궁점 1위 | 변화 없음 | **경복궁 1위** ✅ |
| `경보` (조합 중간) | — | — | **경복궁 1위** ✅ |
| `한옥` | 식당 4개 상위 | 관광지로 뒤집힘 | 유지 ✅ |
| `해운` | — | — | 해운대구(지역) + 해운대해수욕장 등 ✅ |
| `궁궐` | 경복궁 없음 | 변화 없음 | **유사어 재색인 후 해결** (2026-08-20, 6/6) |

네 신호가 각자 제 몫을 했다. 분류 가중치가 `한옥`을, 이름 접두가 `경복`을, 자모가 `경보`를,
유사어가 `궁궐`을 해결했다. **어느 하나로 다 되지 않았다** — 분류 체계가 새는 자리
(`한복남 경복궁점`이 culture)는 분류로 못 가르고, 조합 중간 상태는 형태소로 못 잡고,
문자열이 안 겹치는 상하위어는 사전 없이는 안 잡힌다.

회귀는 `scripts/attractions-search-check.py` 6케이스가 지킨다 (2026-08-20 라이브 6/6).

`궁궐`↔`경복궁`은 분류 가중치로도 접두 부스트로도 해결되지 않는다. 문자열이 겹치지 않아
애초에 후보에 없다 — 2순위(유사어)의 몫이다.

### 2순위 — 유사어 사전 — **1차(수기 시드) 구현 (2026-08-20)**

**쿼리 타임 `synonym_graph`** (`nori_search` 애널라이저, ko 필드의 search_analyzer).
문서는 그대로 두고 질의만 확장한다 — 색인이 안 부풀고, 사전 수정도 재색인 한 번이면 반영된다.

시드는 관광 도메인 수기 10줄이다 (`attractions-index.json` 의 `tourism_synonyms`).
`궁궐, 고궁, 왕궁 => …, 경복궁, 창덕궁, 덕수궁, 창경궁, 경희궁` 한 줄이 4장의 난제를 푼다.

운영 스크래치 인덱스로 검증했다(2026-08-20): 질의 `궁궐` 이 `경복`+`궁` 으로 확장되고
색인 쪽 `경복궁` 이 정확히 `경복`+`궁` 으로 잘려 매칭이 성립한다.

TourAPI 국영문 타이틀 쌍에서 사전을 **유도**하는 것(OQ-3)은 그대로 후속 과제다 — 수기 시드가
효과를 증명하면 그때 투자한다.
