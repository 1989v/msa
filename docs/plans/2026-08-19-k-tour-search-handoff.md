...
### 1순위 — 분류 가중치 — **구현 완료 (2026-08-20), 배포 후 재측정 필요**

`function_score` 로 관광 분류(nature/history/culture/leisure) ×3.0, 상점·식당 ×0.35.
검색과 **자동완성 양쪽**에 건다 — "경복"이 밀린 곳이 자동완성이었다.
설정은 `search.attraction-ranking` (`AttractionRankingProperties`), 둘 다 `1.0` 이면
`function_score` 자체를 감싸지 않아 이전 동작으로 돌아간다.

상점·식당을 **지우지 않고 내린다.** 근처 검색에서는 여전히 유효한 결과이고, 이 서비스가
보여주려는 것이 관광지일 뿐이다.

**바꾸기 전 (2026-08-19/20 운영 실측)** — 배포 후 같은 쿼리로 다시 재서 아래에 나란히 적는다.

| 쿼리 | 상위 결과 |
|---|---|
| `경복` (자동완성) | 한복남 경복궁점 · **경복궁** · 다이소 경복궁역점 · 앤더슨벨 · 올리브영 경복궁역점 · 경복궁(food) |
| `한옥` (60건) | 한옥 생고기 · 스미스가 좋아하는 한옥 · 한옥달 · 이태리한옥 · 전주 한옥 마을 역사관 |
| `궁궐` (13건) | 창경궁 · 덕수궁 · 진도 용장성 · 광화문 — **경복궁 없음** |
| `해수욕장` (2,236건) | 전부 nature — 정상 |

**바꾼 뒤 (2026-08-20 배포 후 같은 쿼리)**

| 쿼리 | 결과 | 판정 |
|---|---|---|
| `경복` (자동완성) | 한복남 경복궁점 · 경복궁 · 다이소 · 앤더슨벨 · 올리브영 | **변화 없음** |
| `한옥` (60건) | 전주 한옥 마을 역사관 · 북촌한옥마을 · 한옥글방 · 북촌한옥마을 · 송도 한옥마을 | **해결** |
| `궁궐` (13건) | 창경궁 · 덕수궁 · 진도 용장성 · 광화문 | 변화 없음 (예상대로) |
| `해수욕장` | 전부 nature | 유지 |

`한옥`은 식당 4개가 상위였던 것이 통째로 뒤집혔다.

**`경복`이 안 움직인 이유 — 분류 체계가 샌다.** `한복남 경복궁점`의 TourAPI 분류가 **`culture`**
라 경복궁과 똑같이 ×3.0 을 받았다. 한복 대여점이 문화시설로 분류돼 있고, `한옥글방`도 같다.
분류로 가를 수 없는 것을 분류로 가르려 한 셈이다.

→ **이름의 모양을 신호로 추가했다.** 자동완성에 `title.keyword` 접두 일치 부스트(×6). `경복궁`은
"경복"으로 **시작**하고 `한복남 경복궁점`은 포함만 한다. `attractions-index.json` 에
`title.keyword` 서브필드를 추가했으므로 **다음 전체 재색인(매일 KST 04:30) 이후 적용**된다.
없는 필드에 걸린 prefix 절은 매칭 0으로 조용히 지나가므로 그 전까지는 이전과 같이 동작한다.
적용 후 `경복`을 다시 재서 위 표에 한 줄 더 적는다.

`궁궐`↔`경복궁`은 분류 가중치로도 접두 부스트로도 해결되지 않는다. 문자열이 겹치지 않아
애초에 후보에 없다 — 2순위(유사어)의 몫이다.
