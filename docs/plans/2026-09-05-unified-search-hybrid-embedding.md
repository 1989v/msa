...
| 모델 | **`Qwen/Qwen3-Embedding-4B` 확정**(4B · native 2560 → MRL 1024 · Apache-2.0) — 전 코퍼스 nDCG@10 **0.7764**, 후보 7종 중 1위 (2026-09-07 재측정) | §8.11 — 서버 비용이 0 이라 크기가 아니라 **판정 세트 nDCG 와 k-NN 메모리**로 골랐다. 초판의 arctic-ko 는 근거 부족으로 뒤집혔다(ADR-0090 D5 개정) |
...
- **이미지 임베딩.** 질의가 전부 텍스트이고 사진은 표시용이라 텍스트 규칙(이름·분류·주소·개요)이 뜻을 이미 담는다. 이미지↔텍스트를 한 공간에 놓는 멀티모달
  모델은 다른 계열이라 지금 벡터와 섞이지 않고, 사진 6만 장 수집과 TourAPI 저작권 구분(`copyrightDivCd`)이 따라온다. "이 사진과 비슷한 곳" 같은 기능이
  생기면 별도 인덱스로 다시 본다.
...
| P0-2 | ✅ **872건 전부 판정**(사람 113 + LLM 초안 759) — 애매한 79건은 규칙 10개로 묶어 검토 대기 | `judgments.yml`(by: human/llm 출처 기록), `doubts.md`(규칙 10개), `review-*.md`, `intents.yml` |
| P0-3 | ✅ 5개 모델 후보 재순위 비교 완료(§8.9) — **판정 후 nDCG 확정** | `rerank-ko.md`, `tools/embed/`, `notebooks/bakeoff.ipynb` |
...
1. ~~모델~~ → **qwen3-4b 확정**(2026-09-07 재측정, §8.11-1). 남은 것은 **차원 512 vs 1024** 뿐 — 메모리는 둘 다 되므로(§8.4) nDCG 차이로만 정한다. P1 첫 채움 때 같이 잰다.
...
### 8.4-1 차원 재측정 — 1024 vs 4096 (2026-09-06, 같은 프로브)

8B 를 **원래 차원(4096)으로 쓸 수 있나**를 가리려고 다시 쟀다. 같은 이미지·heap 512m·한도 1536Mi, 문서 60,000 + 사전 80,000.

| 차원 | 문서 store | 사전 store | 컨테이너 (idle 920MiB →) | knn p50/p95/p99 | 사전 GET p50 | heap used | 결과 |
|---|---|---|---|---|---|---|---|
| 1024 | 478MB | 1,287MB | **1.169GiB** | 5.1 / 6.3 / 7.1ms | 2.24ms | 266/512MB | 통과 (색인 84s · 병합 49s) |
| 4096 | — | — | — | — | — | — | **실패 — `_bulk` 가 429** |

**디스크 크기는 상주 메모리가 아니다.** 1024차원에서 store 합이 1,765MB 인데 컨테이너는 1.169GiB 였다.
`segments_memory_mb: 0` 이고 heap 은 266MB 다 — `engine: lucene` 이라 HNSW 가 세그먼트 파일에 있고 mmap 으로 읽히며,
cgroup 통계에는 회수 가능한 페이지 캐시로 잡힌다. (faiss·nmslib 이었다면 전용 오프힙 캐시라 상주가 맞다.)
**이전 판의 "4096은 한도를 넘는다"는 디스크와 메모리를 섞은 잘못된 추정이었고, 여기서 취소한다.**

4096 이 실패한 방식도 OOM 이 아니다: 컨테이너는 살아 있었고(`running=true oom=false`) 색인 bulk 가 **429(거부)** 를 받았다.
차원이 4배가 되면 페이로드도 HNSW 구성 비용도 함께 커져 쓰기 대기열이 밀린 것으로 보이나 **원인은 격리하지 않았다** —
배치 크기를 줄이거나 heap 을 올리면 통과할 수도 있다. 지금 말할 수 있는 것은 **같은 조건에서 1024는 되고 4096은 안 됐다**는 것뿐이다.
`os_mem_used_pct` 는 1024에서 이미 97% 였다.

**판단**: 8B 를 4096으로 쓰는 길은 이 티어에서 열려 있지 않다. 8B 를 쓴다면 1024로 잘라야 하고, 그러면 §8.3 의
후보 재순위에서 8B(0.7635)가 4B(0.7875)보다 낮았던 결과를 그대로 안고 가는 셈이다.

### 8.5 hybrid 스파이크 (2026-09-05, OpenSearch 3.3.0, 로컬 Docker)

> **2026-09-06 재실행에서 케이스 3 이 구현 결함을 잡았다.** `hybrid` 질의에 `_score` 정렬과 tiebreaker(`idSort`·`id`)를
> 함께 주면 OpenSearch 가 거부한다 — `"_score sort criteria cannot be applied with any other criteria."`
> P1-5 구현이 그 조합을 보내고 있었고, 단위 검사는 **요청 모양만 보므로 못 잡았다**(서버 규칙이라 클라이언트에서 안 보인다).
> 고침: 하이브리드 경로는 정렬을 전혀 걸지 않는다(기본이 점수 내림차순). 대가는 **동점 시 순서 보장이 없어지는 것**이고,
> RRF 점수가 `1/(60+rank)` 의 합이라 동점은 실제로 생긴다. 같은 실행에서 케이스 1·4·7 이 정렬 없는 하이브리드로 통과했다.
...
### 8.7 판정 후보 풀링과 fp16 NaN (2026-09-05 저녁, M4 Pro MPS)

BM25 상위 10 만 판정하면 벡터가 새로 찾은 문서가 "판정 없음"으로 빠져 벡터 레그가 과소평가된다. `embed.pool` 로 후보 모델의 벡터 상위 10 중
BM25 후보에 없던 문서를 `vector_candidates` 로 덧붙였다 — 판정 대상 300 → **872건**(모델 3종). **BM25 와 벡터의 겹침은 질의당 평균 0.0건** —
벡터가 낸 572건이 전부 지금 검색이 상위 10 에 못 올리던 문서다.

| 모델 | 코퍼스 | 처리량 | 결과 |
|---|---|---|---|
| `multilingual-e5-small` fp16 | ko 44,924 / en 14,811 | 604 / 344 docs/s | ✅ 벡터 후보 250건 |
| `snowflake-arctic-embed-l-v2.0-ko` fp16 | ko 44,924 / en 14,811 | 58 / 50 docs/s (ko 778초) | ✅ 237건 |
| `harrier-oss-v1-270m` **fp32** | ko 44,924 / en 14,811 | 85 / 81 docs/s (ko 531초) | ✅ 235건 |
| `harrier-oss-v1-270m` fp16 (첫 시도) | ko / en | 60~80 docs/s | ❌ **전 벡터 NaN** — 폐기, 아래 경고 |

> [!warning] fp16 NaN 은 그럴듯한 결과로 위장한다
> harrier 풀 파일은 정상처럼 생겼는데 **모든 질의의 상위 10 이 같은 문서 하나**였다(NaN 비교의 argsort 결과). 파일 크기·형식·건수는 전부 정상이라
> 눈으로 봐야 잡혔다. 재현: `mps/fp16` NaN, `mps/fp32`·`cpu/fp32` 정상(코사인 0.544). 대응 둘 — `ModelSpec.fp16_ok=False`(harrier 계열),
> 그리고 `bakeoff.encode` 가 **NaN/Inf 를 만나면 던진다**. 조용한 실패를 막는 쪽이 본질이다.

**모델별 벡터 상위 5 (ko 전 코퍼스 44,924건, 판정 전 참고용)**

| 질의 | `arctic-embed-l-v2.0-ko` | `multilingual-e5-small` |
|---|---|---|
| `궁궐` | 경복궁 · 건청궁 · 창경궁 · 경희궁 · 덕수궁 | 경복궁 · 창덕궁과 후원 · 영휘원 · 궁집 · 창덕궁 인정문 |
| `한옥` | 한티옥 · 은평한옥마을 · 화천한옥학교 · 한일옥 · 김용학가옥 | 화천한옥학교 · **한옥 생고기(식당)** · 원당마을한옥도서관 · 남산골한옥마을 |
| `바다가 보이는 곳` | 바다보는날 · 영도 하늘전망대 · 바다캠핑장 · 영뜰해변 · 안목해변 | 중리노을전망대 · 원포리해변 · 학암포해수욕장 · 함평항 |
| `아이와 갈만한 곳` | 구봉산근린공원 · 아빠랑놀자 키즈글램핑 · 인천어린이천문대 · 수성패밀리파크 | 청주랜드 · 금강습지생태공원 · 루덴시아 · 몬스터리움 |
| `조용한 사찰` | 옥련선원 · 서광사 · 달마사 · 수암사 · 남원사 | 남원사 · 반야사 · 달마사 · **한국사찰음식문화체험관** |
| `비 오는 날 갈만한 곳` | 물안개공원 · 보광사계곡 · 와온공원 · 태종대 | **비선대 · 미포항 · 비손농장 · 비양도선착장** |

판정 전이지만 방향은 보인다 — arctic-ko 가 `궁궐`에서 궁만 다섯을, `한옥`에서 식당을 빼고 마을만, `아이와 갈만한 곳`에서 키즈글램핑·어린이천문대를 낸다.
e5-small 은 `비 오는 날`에서 여전히 **`비` 한 글자에 반응**하는데(비선대·비손농장) arctic-ko 는 그러지 않는다. 다만 둘 다 그 질의의 정답(실내 시설)은 못 찾았다 —
**의도 사전에 수기로 넣어야 하는 종류**다. 처리량은 arctic-ko 58 docs/s vs e5-small 604 docs/s 로 10배 차이지만, 오프라인이라 비용이 아니라 시간일 뿐이다(6만 건 13분).

e5-small 이 전 코퍼스에서 낸 벡터 상위(참고): `바다가 보이는 곳`→중리노을전망대·원포리해변·학암포해수욕장, `아이와 갈만한 곳`→청주랜드·금강습지생태공원·루덴시아,
`조용한 사찰`→남원사·반야사·달마사, `궁궐`→경복궁·창덕궁·영휘원. BM25 가 못 찾던 의도 질의를 잡는다(§1.3 과 나란히 보라).
반면 `비 오는 날 갈만한 곳`→비선대·미포항(여전히 `비` 한 글자), `서울 근교 드라이브`→서울둘레길 코스들은 벡터도 못 맞혔다.

### 8.8 큰 모델의 로컬 처리량 — 전 코퍼스 풀링의 한계 (2026-09-05, M4 Pro MPS fp16)

`embed.pool` 은 전 코퍼스를 인코딩해 상위 10 을 뽑는다. 큰 모델은 그 비용이 감당 안 된다(200건 실측 → 44,924건 환산).

| 모델 | 로드 | 처리량 | ko 44,924건 환산 |
|---|---|---|---|
| `multilingual-e5-small` 118M | 27초 | 604 docs/s | 1.2분 |
| `harrier-oss-v1-270m` (fp32) | — | 85 docs/s | 8.8분 |
| `arctic-embed-l-v2.0-ko` 568M | — | 58 docs/s | 12.9분 |
| `Qwen3-Embedding-4B` | 210초 | **2.2 docs/s** | **5.6시간** |
| `Qwen3-Embedding-8B` | 383초 | **1.3 docs/s** | **9.5시간** |

8bit 양자화는 bitsandbytes 라 **CUDA 에서만** 된다 — MPS 에서는 fp16 으로 내려간다(8B fp16 ≈ 16GB 가중치, 48GB 통합 메모리에서 동작 확인).
`load_model` 이 디바이스를 보고 갈라야 한다(그 전에는 MPS 에서 8bit 경로를 타 실패했다).

그래서 큰 모델은 **후보 재순위**(`embed.rerank`)로 비교한다 — 판정 세트에 모인 후보(BM25 + 다른 모델의 벡터 상위)만 인코딩해 줄 세운다.
**새로 찾는 문서는 못 보지만** "이미 모인 후보 중 좋은 것을 위로 올리는가"는 같은 잣대로 잴 수 있고, 판정이 붙으면 그대로 nDCG@10 이 된다.
전 코퍼스 풀링이 필요하면 Colab GPU 로 간다(노트북 준비돼 있다).

### 8.9 후보 재순위 비교 — 5개 모델 (2026-09-05, ko 24질의 × 후보 678건)

전 목록은 `docs/specs/2026-09-05-unified-search/rerank-ko.md`. 판정 전이라 **순위는 눈으로 본 인상**이고, nDCG 는 grade 가 채워져야 나온다.

| 모델 | 678건 인코딩 | 인상 |
|---|---|---|
| `Qwen3-Embedding-8B` | 656초 | `조용한 사찰`에 절만 넷(무상사·수암사·옥련선원·정혜사), `실내 놀거리`에 실내 시설만. **`궁궐`에서 청와대칠궁·광화문을 경복궁보다 위로** |
| `Qwen3-Embedding-4B` | 351초 | `궁궐` 덕수궁·경희궁·경복궁·창경궁으로 안정. `한복 대여`는 한복남 지점만 다섯. `조용한 사찰` 1위가 사찰음식체험관 |
| `arctic-embed-l-v2.0-ko` | 27초 | `궁궐` 경복궁 1위, `한옥`에 식당 없음. `비 오는 날`에 `비` 글자 반응 없음. **속도 대비 가장 안정** |
| `harrier-oss-v1-270m` | 21초 | `야경 명소`에 N서울타워, `실내 놀거리` 준수. **`궁궐`에서 국립고궁박물관·조선왕가를 궁보다 위로** |
| `multilingual-e5-small` | 10초 | `한옥`에 **한옥 생고기(식당)**, `비 오는 날`에 **비선대·비손농장**(`비` 한 글자). 기준선다운 실패 |

관통하는 것 둘. **크다고 다 좋지는 않다** — 8B 가 `궁궐`에서 청와대칠궁·광화문을 경복궁 위에 올렸는데 이건 "궁 이름이 든 것"과
"조선 궁궐"을 못 가른 것이다. 반대로 `조용한 사찰`·`실내 놀거리`처럼 **문장 전체의 의도**를 읽어야 하는 질의는 8B 가 확실히 낫다.
그리고 **모든 모델이 `비 오는 날 갈만한 곳`을 못 맞혔다**(정답은 실내 시설) — 이런 질의는 사전에 수기로 넣어야 한다는 뜻이고,
질의 사전 설계(§2.5)가 모델 선택과 별개로 필요한 이유다.

비용 대비로 보면 **arctic-ko 가 8B 의 24분의 1 시간에 비슷하거나 나은 결과**를 낸다. 다만 이건 인상이고, 판정 뒤 nDCG 로 확정한다.

### 8.10 판정 — 사람 113 + LLM 759, 출처를 남긴다 (2026-09-05)

872건 전부 판정됐다. 사용자가 4질의 113건을 먼저 매겼고(`궁궐`·`한옥`·`해수욕장`·`경복궁`), 그 판정에서 **기준을 읽어**
나머지 26질의를 LLM 초안으로 채웠다. 읽어낸 기준 셋 — 궁궐은 본체 3·부속 전각 2·이름만 궁 1·음식점 0, 한옥은 **마을** 3·개별 건물 1·식당 0,
경복궁은 본체만 3·다른 궁 2·상호에 든 것 1.

**출처를 행마다 남긴다**(`by: human` / `by: llm`). 남기지 않으면 "이 nDCG 는 무엇 기준인가" 에 답할 수 없다. `--no-overwrite` 로
LLM 초안이 사람 판정을 덮지 못하게 했다.

> [!warning] LLM 이 만든 정답으로 임베딩 모델을 재는 것의 한계
> 저와 임베딩 모델은 다른 모델이라 특정 모델을 편애할 구조적 이유는 없지만, **같은 종류의 편향을 공유**할 수 있다(둘 다 웹 텍스트 학습).
> 그리고 `한복 대여` 에서 상점이 정답인지, `서울 근교 드라이브` 에 서울둘레길이 맞는지는 모델 지식이 아니라 **서비스 의도**의 문제다.
> 그래서 초안은 초안으로 두고, **경계 등급(1·2) × 모델 상위 5 노출** 79건을 골라 사람 검토 대상으로 세웠다(`doubts.md`).
> 그 79건이 규칙 10개에서 갈리므로 규칙만 답하면 일괄 적용된다. 나머지는 등급이 바뀌어도 상위 10 에 안 나와 nDCG 를 안 흔든다.
