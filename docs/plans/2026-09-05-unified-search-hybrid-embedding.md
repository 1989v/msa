...
| 모델 | **`snowflake-arctic-embed-l-v2.0-ko` 확정**(568M · 1024차원 · Apache-2.0) — 판정 세트 nDCG@10 ko 0.7404 / en 0.7773 으로 후보 중 최고 | §8.11 — 서버 비용이 0 이라 크기가 아니라 **판정 세트 nDCG 와 k-NN 메모리**로 골랐다 |
...
1. ~~모델~~ → **arctic-ko 확정**(§8.11). 남은 것은 **차원 512 vs 1024** 뿐 — 메모리는 둘 다 되므로(§8.4) nDCG 차이로만 정한다. P1 첫 채움 때 같이 잰다.
