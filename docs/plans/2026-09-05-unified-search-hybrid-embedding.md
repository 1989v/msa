...
| P0-2 | ✅ 파일 + 벡터 후보 풀링 — **판정(grade) 대기, 550건** | `judgments.yml` 30질의 × (BM25 상위 10 + 벡터 후보 250), `intents.yml` ko 117 · en 30 |
...
BM25 후보에 없던 문서를 `vector_candidates` 로 덧붙였다 — 판정 대상 300 → **550건**.

| 모델 | 코퍼스 | 처리량 | 결과 |
|---|---|---|---|
| `multilingual-e5-small` fp16 | ko 44,924 | 604 docs/s (74초) | ✅ 질의당 고유 10 |
| `multilingual-e5-small` fp16 | en 14,811 | 344 docs/s | ✅ |
| `harrier-oss-v1-270m` fp16 | ko 44,924 / en 14,811 | 60~80 docs/s (ko 564초) | ❌ **전 벡터 NaN** — 폐기 |
