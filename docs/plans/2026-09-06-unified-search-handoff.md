...
## 3-1. 모델 선정 재검토 — **끝났다. `Qwen/Qwen3-Embedding-4B` 로 바뀌었다** (2026-09-07)

**결론**: 전 코퍼스 nDCG@10 에서 `qwen3-4b` 0.7764 로 1위. 기존 선택 `arctic-ko` 는 0.7313.
ADR-0090 D5 를 개정했고 도구 사용 예시도 바꿨다. 커버리지는 7종 전부 100% 다.

| 모델 | 벡터 단독 | 하이브리드 | 906건 재순위 |
|---|---|---|---|
| BM25 | 0.6802 | — | — |
| **qwen3-4b** | **0.7764** | **0.7343** | **0.7765** |
| qwen3-8b | (전 코퍼스 미측정 — 로컬 메모리 부족) | — | 0.7686 |
| arctic-ko | 0.7313 | 0.7199 | 0.7342 |
| harrier-270m | 0.7127 | 0.7137 | 0.7126 |
| e5-small | 0.7018 | 0.7119 | 0.7018 |
| arctic-official | 0.6928 | 0.6938 | 0.6928 |
| bge-m3 | 0.6568 | 0.6894 | 0.6568 |

**8B 는 4B 보다 낮다** — 재순위 0.7686 vs 0.7765, 시간은 두 배. 4B 의 재순위(0.7765)가 전 코퍼스(0.7764)와
사실상 같아 재순위가 순위를 잘 대신함이 확인된다. 8B 가 불리한 이유는 절단 폭으로 보인다(4096→1024 는 3/4 를 버린다).
로컬 8B fp16 은 가중치만 **14.10GiB** 라 여유가 없을 때 두 번 죽었다 — 여유 25.6GiB 에서는 돌았다.

**남은 확인**: 새로 매긴 249건이 **LLM 초안**이다. 경계 사례를 사람이 확인하면 순위가 바뀔 수 있다 → §3-2.

<details><summary>재검토를 하게 된 경위 (2026-09-06)</summary>
