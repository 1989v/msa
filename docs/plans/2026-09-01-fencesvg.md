...
- 좌표는 **소수점 둘째 자리까지** 반올림해 출력한다(`Math.round(n*100)/100`). 폭·높이 등 viewBox 값만 정수다. 목적은 정수화가 아니라 부동소수 꼬리를 없애 스냅샷을 안정시키는 것이다.
...
describe('layoutGraph', () => {
  ...
  it('순환이 있어도 끝나고, 랭크가 0 부터 연속이다', () => {
    ...
    // 노드 수만 세면 랭크 0 이 통째로 빈 것을 못 잡는다 — 실제로 못 잡았다
    expect([...new Set(out.rankOf.values())].sort()).toEqual([0, 1]);
  });

  it('진입 간선 0 인 노드가 따로 있어도 분리된 순환이 랭크를 받는다', () => {
    // 시작점을 전역에서 한 번만 심으면 이 순환은 큐에 못 들어가 전부 랭크 0 이 된다
    const out = layoutGraph(
      [n('x'), n('a'), n('b')],
      [{ from: 'a', to: 'b' }, { from: 'b', to: 'a' }],
      'LR',
...
`scripts/no-deps.mjs` 를 함께 만든다. 인라인 `node -e` 로 두지 않는 이유 — `package.json` 이
`"type": "module"` 이라 `require` 가 노드 버전·플래그에 따라 갈린다.
...
```bash
git add scripts/size.mjs scripts/no-deps.mjs README.md .github/workflows/ci.yml package.json
git commit -m "chore: 번들 예산 게이트 · 의존성 0 검사 · README · CI"
```
...
**`file:` 의존성이 든 `package.json` 은 커밋하지 않는다.** msa CI 는 portal-fe 를 도커에서
빌드하는데 `../../fencesvg` 가 그 컨텍스트에 없어 이미지 빌드가 깨진다. npm 에 올린 뒤
`fencesvg@^0.1.0` 으로 바꿔 커밋한다 (Step 8).
...
- [ ] **Step 8: 커밋 — npm 배포 후에만**

`fencesvg` 가 npm 에 올라간 뒤 `file:` 을 버전으로 바꾸고 커밋한다.

```bash
cd ~/IdeaProjects/msa/portal-fe && npm i fencesvg@^0.1.0
cd ~/IdeaProjects/msa
...
배포 전이면 여기서 멈춘다 — 코드 변경은 로컬에 두고 커밋만 미룬다.
...
초안의 펜스가 전부 그려지는지 라이브러리로 직접 돌린다. 셸 인라인으로 두지 않는다 —
검사할 내용에 백틱이 들어 있어 이스케이프가 깨진다.

`~/IdeaProjects/fencesvg/scripts/check-fences.mjs` 를 만든다. `files` 에 없으니 배포물에는
안 들어간다.

```js
// scripts/check-fences.mjs — 마크다운 파일의 모든 다이어그램 펜스가 그려지는지 확인
import { readFileSync } from 'node:fs';
import { renderDiagram } from '../dist/index.js';

const path = process.argv[2];
if (!path) { console.error('사용: node scripts/check-fences.mjs <마크다운 경로>'); process.exit(2); }

const fence = new RegExp('^' + '`'.repeat(3) + '(?:mermaid|diagram)[^\\n]*\\n([\\s\\S]*?)^' + '`'.repeat(3), 'gm');
const bodies = [...readFileSync(path, 'utf8').matchAll(fence)].map((m) => m[1]);

let bad = 0;
bodies.forEach((body, i) => {
  const r = renderDiagram(body, { idPrefix: `d${i + 1}` });
  if (r.svg) console.log(`  ✓ ${i + 1}  ${r.caption ?? '(캡션 없음)'}`);
  else { bad += 1; console.log(`  ✗ ${i + 1}  ${r.warnings.join(' / ')}`); }
});
console.log(`  펜스 ${bodies.length}개 · 실패 ${bad}개`);
process.exit(bad > 0 ? 1 : 0);
```

```bash
cd ~/IdeaProjects/fencesvg && npm run build && node scripts/check-fences.mjs ~/IdeaProjects/msa/docs/drafts/2026-09-fencesvg.md
```

Expected: 다섯 개 전부 `✓`, `실패 0개`
