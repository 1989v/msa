...
**2026-09-22 — 평가는 이제 레포에 있다.** 판정 세트·스크립트가 노드 `/tmp` 에만 있던 것을 `scripts/search-eval/` 로 옮겼다
(`run-eval.sh` 가 노드에서 돈다). 매일 돌던 `search-eval-daily` 는 ADR-0050 의 상품 기준선 잡이고 `analytics.search_judgments`
0행이라 매일 건너뛰며 COMPLETED 만 찍고 있었다 — **`suspend: true`** 로 세웠다(되살리려면 실행기를 관광지 하이브리드로, 판정을 ClickHouse 로).

같은 날 OpenSearch 를 손봤다 — 한도 1536Mi 에서 JVM anon 이 1,480 MB 를 먹어 페이지 캐시가 4 MB 뿐이었고, 관광지 벡터 파일(272 MB)이
질의마다 디스크에서 올라와 kNN 112초·place 검색 500·통합 11/13 이었다. 09-13 과 **09-22 04:32 KST(야간 재색인 중)** 두 번 OOMKilled 였고,
두 번째는 벌크 15,800건이 죽은 채 별칭이 넘어가 **45,535건짜리 색인이 라이브**가 됐다(수동 재색인으로 복구).
한도 2560Mi(+NMT) · 보존 1벌 · 3.3.0 → **3.8.0** · 관광지 벡터 **1-bit 양자화 + 재채점 ×3**(다음 재색인부터). 한도 뒤 재측정:

| 구성 (2026-09-22, 한도 2560Mi · 양자화 전) | ko | en |
|---|---|---|
| A. BM25 단독 | 0.4305 | 0.5465 |
| B. 하이브리드 | 0.5954 | 0.6420 |
| C. 하이브리드 + 쿼리 언더스탠딩 | **0.6584** | **0.6602** |

09-13 과의 차는 색인 드리프트(개요 백필로 판정 없는 문서가 top-10 에 섞임). 순서·폭은 그대로다. **양자화 뒤 같은 세트로 한 번 더 잰다** — 그때 C 가 내려가면 `oversample` 을 올리거나 되돌린다.
남은 결함: 재색인 tasklet 이 벌크 오류가 수천 건이어도 별칭을 넘긴다 — 오류 비율·건수 하한 게이트가 없다.
...
### 3.5 용어 사전 가지 — 도메인마다 하나

계층의 노드는 **단계와 장치**다. 그것을 읽는 데 필요한 말(CRF · 비터비 · 문맥 ID · 켤레 사전 …)은 단계도 장치도 아니라
어디에도 걸 자리가 없었고, 장치 설명 안에 낱말로만 묻혀 있었다. 그래서 **도메인 루트마다 `<도메인>-glossary` 가지 하나**를 둔다.

| 규칙 | 내용 |
|---|---|
| 자리 | 루트의 **마지막 자식**(검색은 넷째). 진입점이 아니라 참조 가지다 — 인제스트·검색어·평가 셋이 진입점인 것은 그대로 |
| 묶음 | 사전 아래에 주제 묶음(형태소 분석 · 벡터 · 학습/평가)을 두고 그 아래에 용어. 묶음이 6개를 넘으면 묶음을 쪼갠다 |
| 두 부모 | 용어는 **사전 아래에도, 쓰이는 장치 아래에도** 건다(DAG). 사전에서 찾아도 장치를 펼쳐도 나온다. 깊이는 짧은 쪽(사전, 3)으로 잡히고 장치 아래 행은 놓인 위치의 깊이로 내려간다(`hierarchyModel.ts`) |
| 중복 금지 | 이미 장치로 있는 것(RRF · nDCG · HNSW · BM25 · 위치 편향)은 사전에 다시 넣지 않는다 — 사전은 **장치가 아닌 말**만 |
| 설명 | 한 줄 정의 + 이 계층에서 어디에 쓰이는지 한 구절. 히스토리 없음 |
| 동의어 | 원어 · 약어 · 파일명(`matrix.def`)을 `concept_synonym` 에 — 검색창에서 그 말로 찾히게 |

검색 도메인의 첫 판(V24): 형태소 분석 11(형태소 · 표층형 · 품사 태그 · 격자 · 비터비 · 단어 비용 · 연접 비용 · 문맥 ID · CRF · mecab-ko-dic · 세종 말뭉치) ·
벡터 3(코사인 유사도 · 마트료시카 임베딩 · HNSW 파라미터) · 학습/평가 6(랭킹 손실 세 갈래 · 쿼리 단위 교차검증 · 탐색/활용 · 베타-베르누이 켤레 · 성향 역수 가중 · 팀 드래프트 인터리빙).
격자 → 단어 비용 → 연접 비용 → 비터비는 `FLOWS_TO` 로 이어 「깔고 · 붙이고 · 고른다」 순서를 남겼다.
