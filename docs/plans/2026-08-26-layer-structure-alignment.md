...
- **면제는 파일 단위다.** ①~⑥의 모듈 면제는 비우면 사라지는 임시 상태라 괜찮지만, ⑦은 영구라
  모듈째 열면 구멍도 영구가 된다 — `code-dictionary` 는 호스트이면서 자기 도메인이기도 해서
  그 면제가 도메인 코드까지 덮는다. 파일 경로로 좁혀 **두 번째 교차 배선이 생기면 빌드가 막도록** 했고,
  그 덕에 재분리 체크리스트가 가리키는 "목록" 이 실제 파일 목록이 된다.
  (`commerce:app` 은 main 소스가 `CommerceApplication.kt` 한 파일뿐인 순수 합성 루트라 면제가 필요 없다)
