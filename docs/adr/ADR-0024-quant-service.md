...
『분할매매』의 분할 원칙(분할매수/분할매도, 독립회차 운용, 손실제한, 규칙 우선 등)을 국내 암호화폐 거래소(빗썸/업비트)에 적용하는 **규칙 기반(deterministic) 자동매매 서비스**를 MSA 플랫폼에 신규 추가한다.
...
- 입력 스펙: `docs/specs/2026-04-24-quant-crypto-trading/planning/spec.md`, `requirements.md`, `ideabank/docs/12-quant-crypto.md`

## Decision

### 1. 서비스 모듈 구조
- 중첩 Gradle 서브모듈로 신설: `:quant:domain` / `:quant:app` (ADR-0001 패턴 준수)
- 기본 패키지: `com.kgd.quant` (underscore 없음, 기존 컨벤션 준수)
...
- **ClickHouse**: 시세 틱·백테스트 시계열
  - analytics ClickHouse **인프라(노드/클러스터)는 재사용**하되, **별도 데이터베이스 `quant`을 신설**하여 스키마/테이블 소유는 독립. analytics DB/schema 직접 참조 금지.
  - 테이블 네이밍은 prefix 없이 db 네임스페이스로 격리: `quant.market_tick_{exchange}`, `quant.backtest_run`, `quant.execution_result` 등.
- **Redis**: Rate Limit 카운터, WS 세션 메타

### 13. 프론트엔드 배치
- **`quant/frontend/` 독립 모듈 신설** (React + Vite)
...
- **analytics ClickHouse DB 공유**: 테이블 네임스페이스 충돌·변경 커플링 위험 → `quant` DB 분리로 인프라만 공유
