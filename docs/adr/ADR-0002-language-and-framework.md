...
## Decision
- Java 25 LTS (JVM 런타임). Kotlin 2.2.21 컴파일러의 최대 JVM 타겟은 JVM 24이므로, 바이트코드 타겟은 JVM 24로 설정 (Java 25 런타임에서 실행)
...
- Kotest BehaviorSpec (BDD 기반 단위 테스트 프레임워크)
- JWT HS256 알고리즘 — Gateway가 JWT 검증 단독 수행 — 내부 서비스는 Gateway 헤더(X-User-Id, X-User-Roles)만 신뢰

*버전 근거: `search/build.gradle.kts` 에 이미 설정된 버전 기준 적용*
