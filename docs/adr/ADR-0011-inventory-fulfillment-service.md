...
## Refinement (2026-05-03)

본 ADR 의 §2 "Outbox 패턴으로 이벤트 발행의 원자성 보장" / §4 "보상 트랜잭션은 각 서비스가 자체 처리" 결정이 다음 두 ADR 로 보강됐다 (결정 자체는 유지, implementation refinement):

- **ADR-0029 (Idempotent Consumer Helper 표준화)** — Saga choreography 의 멱등 보장을 위한 common 헬퍼 표준 + `processed_event` 복합 PK 마이그레이션. inventory/fulfillment 측 4개 listener 가 helper 사용으로 전환됨 (`docs/adr/ADR-0029-idempotent-consumer-helper.md`).
- **ADR-0032 (Order Outbox + Cancellation Compensation)** — 본 ADR §2 의 적용 범위를 Order 까지 확장 + §4 의 누락된 cancellation 즉시 보상 (inventory `onOrderCancelled` listener) 보강 (`docs/adr/ADR-0032-order-outbox-cancellation.md`).
- 관련 implementation plan: `docs/plans/ADR-0029-implementation-plan.md`, `docs/plans/ADR-0032-implementation-plan.md`.
