...
**harrier-270m 이 예산 안에서 최대다.** 질의 150ms 는 P99 예산 300ms 의 50%(검색 자체는 p50 7ms),
영어 이득은 4B 의 87%(+0.0540 대 +0.0618)를 낸다. 한국어는 4B 대비 −0.020 을 내주지만 **BM25 대비로는 +0.033** 이다.
...
**D7. 건강 지표는 질의 인코딩 지연과 캐시 적중률 둘이다.** `search.qvec.encode.p99` 가 200ms 를 넘으면
사이드카 CPU 를 늘리거나 모델을 한 단 내린다(e5-small, 44ms). `search.qvec.cache.hit` 은 비용 지표이지
가용성 지표가 아니다 — 미적중이어도 사이드카가 답한다.
