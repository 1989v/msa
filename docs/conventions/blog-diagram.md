...
### 1.5.1 화살표와 라벨 표기 (fencesvg 0.11 기준)

간선 라벨은 **세 가지 표기가 모두 통한다.** 0.10 까지는 파이프 형식만 읽었고 나머지는
그림이 안 그려진 채 코드블록으로 남았다 — 그 판으로 쓴 글이 있으면 그대로 둬도 새 판이 읽는다.
...
끝 기호 `o` · `x` · 없음(`---`)과 양방향 `<` 도 같이 쓸 수 있다. `A --- B --> C` 는 체인으로 읽는다.

**따옴표는 쓰지 않아도 된다.** `A["텍스트"]` · `(("텍스트"))` · `[("텍스트")]` · `\|"텍스트"\|` ·
`subgraph S["텍스트"]` 에서 한 겹을 벗긴다. 짝이 안 맞으면 글자로 남으므로 열고 닫는 것을 맞춘다.

**여전히 못 그리는 것**: sequenceDiagram 의 `rect` 블록. 만나면 코드블록으로 남으니 §2 의
인라인 SVG 로 옮긴다.

### 1.5.2 그림이 넓어지지 않게

- **라벨을 짧게 쓴다.** 라벨이 있는 간선은 두 층 사이를 라벨 폭(LR)만큼 벌린다 —
  `hybrid: BM25 + kNN → RRF` 한 줄이 그림을 100px 넘게 넓힌다.
- **LR 로 5단이 넘으면 TB 로 돌린다.** 단이 늘수록 폭이 선형으로 늘어 상한(80rem)에 닿는다.
  실측: LR 7단이 고유 폭 1,485px 로 85% 축소를 받았다.
- **설명은 그림이 아니라 캡션에.** 노드 안 문장이 길면 상자가 커지고 층 간격이 따라 커진다.
