...
그림 폭은 본문 열에 갇히지 않는다. 데스크톱에서는 펜스로 그린 그림이 본문 열(44rem) 밖으로
나가 **최대 80rem** 까지 고유 크기 그대로 놓이고, 그보다 넓은 그림만 85% 까지 줄인 뒤 가로
스크롤한다(`Blog.css` 의 `.fs-figure` 규칙). 좁은 화면에서는 본문 폭이 곧 한계다. 실측으로
7단계 flowchart(고유 폭 1,265px)가 1440px 화면에서 스크롤 없이 들어갔다.
