...
> **출처**: ADR-0021 에서 이전 (ADR-0026 분류 정책에 따른 재배치). 코드 작성 규칙이라 conventions/ 가 본질적 위치.
> **History**: 원본 ADR 본문은 git history 의 `ADR-0021-logging-conventions.md` 참조.
...
- **companion object 만 존재하는 유틸리티 클래스**: companion object 내부에 선언
...
- mrt-package ADR-0018: 로깅 규칙 (원전 — MSA 환경에 맞게 보완됨)
