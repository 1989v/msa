...
- **Rule of Three**: 세 번째 반복에서 추상화한다. 두 번째까지는 중복을 그대로 둔다 — 두 사례만으로는
  무엇이 공통이고 무엇이 우연인지 알 수 없고, 그 상태에서 뽑은 추상화가 곧 잘못된 추상화다.
- **조기 최적화 금지**: 측정된 병목 없이 캐시·비동기·샤딩·서비스 분리를 넣지 않는다.
  제안할 때는 **수치를 함께 인용**한다 ("P99 가 X ms" 없이 "느릴 것 같아서"는 근거가 아니다).
- **동점이면 단순한 쪽(Occam)**: 두 설계가 같은 값을 낸다면 단순한 쪽을 고른다.
  **복잡한 쪽을 고를 때만 근거를 남긴다**(주석 또는 ADR) — 왜 단순한 쪽이 안 되는지가 그 근거다.
...
### 1.1 최소 수정(Minimal Diff) — 기본값

**태스크 목표에 직접 필요한 파일·라인만 바꾼다.** 빌드설정(gradle/tsconfig 등)·락파일·CI 설정·
포맷팅·인접 코드 "정리"는 **명시 요구가 없으면 건드리지 않는다.** 도구가 부수적으로 바꾼 diff
(`pnpm install` 이 만진 락파일 등)는 커밋 전 원복한다. **PR diff 의 모든 파일은 태스크 목표로
정당화 가능해야 한다.**

**보이스카우트 규칙은 이 레포에서 "발견 → 보고"까지다.** 원래 이 규칙은 *이미 손댄 코드*의 스멜을
함께 치우라는 뜻이지 다른 파일까지 가라는 뜻이 아니다. 그런데 "손댄 범위" 의 경계가 사람마다 달라
실제로는 인접 파일로 번진다. 그래서 여기서는 **그것조차 자동으로 하지 않는다** —

1. 스멜을 발견하면 **고치지 말고**, 무엇을·왜 고쳐야 하는지 한 줄로 보고한다.
2. **진행 여부를 묻고**, 승인되면 그때 고친다. 가능하면 **별도 커밋**으로 가른다.
3. 예외 — **안 고치면 이번 작업이 성립하지 않는 것**(컴파일 실패, 게이트 위반, 깨진 테스트)은
   묻지 말고 고치고 보고한다. 여기서 묻는 것은 진행을 막을 뿐이다.

> 승인 없이 손대면 그 커밋의 diff 가 **두 가지 이유를 섞는다.** 리뷰어는 어느 줄이 태스크이고
> 어느 줄이 청소인지 구분하지 못하고, 되돌려야 할 때 함께 되돌릴 수 없다.
> 2026-08-26 대량 이동 중 자동 보정 스크립트가 무관한 파일의 미사용 import 를 지웠고,
> **변경 이유가 그것뿐인 파일 하나를 되돌렸다** — 이 규칙이 없었으면 판정 근거가 없었다.
...
그리고 **주석에는 오래 가는 것만 쓴다.** 티켓/스펙 번호(`SP-10`, `TG-04`)나 "지금 작업 중" 같은
진행 맥락은 넣지 않는다 — 그 번호는 몇 달 뒤 아무것도 가리키지 않고, 주석은 남아 거짓을 말한다.
기능은 **durable 한 이름**으로 부른다. (설계 결정의 출처가 필요하면 ADR 번호는 예외 — ADR 은 지워지지 않는다)
