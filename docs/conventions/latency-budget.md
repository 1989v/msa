...
| 검색 — 질의 벡터 적중 | 150 ms | 실측 평균 74.6ms |
...
| 광고 결정 (`POST /api/v1/ads/decisions`) | 30 ms | 메모리 후보 인덱스 + Redis 읽기 1회·쓰기 1회, DB 호출 0 (ADR-0098, 미측정 추정 — R2 배포 뒤 실측) |

**검색을 두 줄로 나눈 이유**: 두 경로의 분포가 다르고, 섞은 P99 는 질의 벡터 적중률이
정한다. 적중률은 캐시와 `query_vector` 원장이 차면서 계속 오르므로, 한 숫자로 덮으면
같은 코드가 날마다 다른 판정을 받는다.

**검색의 실측 분포** (2026-09-08 oci-arm, 히스토그램 80건 · 미적중 비율 31%):
P50 89ms · P95 500ms. **P99 는 아직 정할 수 없다** — 80건에서 P99 는 최악 1건이고,
그 1건(2.86s)은 파드 기동 직후 JIT 예열이다. 실트래픽이 쌓인 뒤 갱신한다.
P95 가 예산 경계에 정확히 앉은 것은 이 표본의 미적중 비율이 높아서이고,
`query_vector` 원장이 차면 내려간다.
