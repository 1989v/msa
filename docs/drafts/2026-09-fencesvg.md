...
summary: 다이어그램의 어려운 부분은 그리기가 아니라 넣기다. sanitizer·lint·서버 렌더·CommonMark 가 SVG 를 지우는 네 지점과, 그걸 통과하도록 만든 12.3KB 라이브러리. 색은 사이트에서 읽는다.
---
...
대비는 **바탕에 얹은 뒤** 잰다. 사이트가 `color-mix(… 12%, transparent)` 로 만든 테두리 색을 알파 없이 재면 황토(밝기 147)와 어두운 바탕(19)이 대비 128 이라 통과한다. 실제로 보이는 것은 12% 만 얹힌 색이라 대비가 15 다.

이걸 놓쳐서 순차도 생명선이 사라졌다. 알파 0.12 인 색에 `stroke-opacity` 0.16 을 또 곱해 실효 0.019, 바탕 대비 2/255 였다.

면은 세 단으로 나눈다. 판단과 참가자 상자는 한 단 진하고, 흐름의 시작은 강조색을 옅게 섞는다. 셋 다 감지한 팔레트에서 뽑는다.
...
글자 크기는 본문에서 읽는다. 그런데 처음에는 상자 높이·행 높이·노드 간격이 전부 리터럴이었다. 12px 글자를 전제로 잡힌 값들이다.

본문이 16px 인 사이트에서 글자만 커지고 상자는 44px 에 머물렀다. 좌우 여백은 14px 인데 상하는 4px 인 눌린 모양이 됐다.

지금은 크기 상수 열일곱 개를 전부 글자 크기에서 뽑는다. 12px 에서는 옛 값과 같은 수가 나오도록 맞춰 두어서, 감지가 없는 경로의 출력은 한 픽셀도 안 바뀐다.

축소에는 하한이 있다. `max-width: 100%` 만 걸면 좁은 화면에서 끝없이 줄어들어 글자가 같이 작아진다. 실측에서 상태도가 0.70배가 되고 글자가 화면에서 11.1px 이 됐다.

`min-width` 는 CSS 우선순위에서 `max-width` 를 이긴다. 고유 폭의 85% 를 걸어 두면 그 아래로는 안 줄고 바깥 상자가 가로로 스크롤한다. 페이지 안에서 글자 크기가 일정해진다.
...
자체 렌더러는 gzip 12.3 KB 다. 대신 레이아웃을 직접 써야 하는데, 타입 수만큼 필요하지는 않았다. 계층 그래프 엔진 하나를 flowchart·state·ER·class 가 나눠 쓰고, sequence 만 별도 레인 배치를 쓴다.
...
이 글의 흐름도가 그 예다. 노드 14개에 되돌아가는 간선이 셋 있고, 그림은 그려지지만 잘 읽히지 않는다.

랭크를 최장 경로로 매기고 나면 되돌아가는 간선의 출발 노드가 도착 노드보다 뒤로 밀린다. 그 결과 `재고예약` 이 자기가 먹이는 `재고 충분` 보다 아래에 온다.

우회 경로도 문제다. 되돌아가는 간선은 상자 바깥으로 도는데, 그 차선이 다른 상자의 테두리를 스쳐 지나가면 그 상자에서 나가는 간선처럼 보인다.

라벨도 같은 이유로 어긋난다. 경로의 40% 지점에 놓는데 경로가 길면 그 지점이 엉뚱한 두 노드 사이에 떨어진다.

되돌아가는 간선이 하나까지는 읽을 만하다. 셋이면 순서를 눈으로 따라갈 수 없다. 랭크 배정과 순서 정렬을 다시 짜야 하는 문제라 그리기 단에서는 못 고친다.
