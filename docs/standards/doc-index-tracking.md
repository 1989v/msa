# Doc Index Tracking — 문서-소스 매핑 추적 표준

> **출처**: ADR-0023 에서 이전 (ADR-0026 분류 정책에 따른 재배치). 도구/자동화 정책이라 standards/ 가 본질적 위치.
> **History**: 원본 ADR 본문은 git history 의 `ADR-0023-doc-index-tracking.md` 참조.
...
5. **자체 구현 (본 표준)** — 최소 표면적, MSA 용어/구조 맞춤, 점진 도입 가능. **채택**.
...
- ADR-sensitive 경로 (ADR-0019~) 의 변경이 lock drift 를 유발 → false positive 가능. 정책의 `ignore_patterns` 로 조정.
