...
### 8.0 아트가 있는 게임은 **디자인 판이 선행이다** (2026-09-06 신설 — 필수)

유니티 웹게임은 화면이 곧 제품이다. **캐릭터·배경·레벨 아트를 코드로 먼저 그리지 않는다** —
`design` 스킬(클로드 디자인)에 스펙을 넘겨 **디자인 판을 먼저 받고**, 그 판을 스펙 삼아 구현한다.

| 넘기는 것 | 받는 것 |
|---|---|
| 영웅·장수 스펙(배분·공격·오라·실루엣 표식) | 캐릭터 시안 · 승급 단계별 색 |
| 적 조건(축·실루엣 규칙·보스) | 적 종류별 시안 |
| 맵 규격(길·지형·슬롯·경계) | 레벨 배치판 |
| 팔레트 조건 | 팔레트 · 조명 톤 기준판 |

- 코드가 그린 그림은 **배치·수치를 확인하는 관측용**이지 아트 시안이 아니다
- **연동이 안 되면 거기서 멈추고 사용자에게 이유를 먼저 확인한다.** 없는 대로 진행하지 않는다 —
  그렇게 진행한 결과가 「전란」 스캐폴드였고, 수치 게이트는 전부 초록불인데 벤치마크 일치도가
  **25~30%** 였다. 사용자가 같은 지적을 세 번 했다
- 판이 나오면 그 값(hex·비율·실루엣)이 **단일 원본**이다. 코드에서 색을 다시 정하지 않는다

**클로드 디자인이 못 하는 것도 미리 말한다** — `.fbx`·`.glb`·텍스처·리그를 못 내놓는다.
인게임 3D 에셋은 별도 경로(무료 CC0 킷 / 유료 에셋팩 / 외주)가 필요하고,
**어느 경로로 갈지가 품질 상한을 정하므로 착수 전에 사용자와 정한다.**

### 8.0.1 디자인을 구현 한계로 깎지 않는다 (2026-09-06 신설 — 필수)

**디자인 판은 목표를 그린다.** 「지금 코드로 만들 수 있는 형태」로 미리 깎지 않는다.
구현이 못 따라오면 그건 **간극으로 보고할 것**이지 그림을 낮출 이유가 아니다.

실제로 이렇게 어긋났다 — 전란에서 인게임 3D 를 「절차 생성 메시(상자 조립)」로 정해 두고,
디자인 판을 그릴 때 **그 구현이 만들 수 있는 형태로 스스로 제한**했다. 판에 「상자로 조립할 수
있는 형태만 그렸다」고 적어 두기까지 했고, 결과는 블록 인형이었다. 품질 상한이 **디자인
단계에서 이미 닫혀** 있었다.

- 디자인 → 구현은 **한 방향**이다. 구현 결정이 디자인 단계로 거슬러 올라오면 안 된다
- 구현 수단(절차 메시·스프라이트·에셋 킷)은 **디자인이 나온 뒤에** 고른다.
  먼저 골랐다면 그 선택이 **품질 상한을 어디로 정하는지** 그때 적고 사용자에게 알린다
- 도달 못 하는 축은 「미달 축 + 도달 방법 + 비용」으로 보고한다 — 조용히 눈높이를 내려 그리지 않는다
...
### 8.0.2 화면 축척은 **참조작에서 픽셀로 잰다** (2026-09-08 신설 — 필수)

「해상도가 이상하다」·「너무 확대돼 보인다」·「캐릭터가 크다」는 전부 **화면 축척** 하나를
가리킨다. 이 축은 엔진과 무관하다 — 클린룸 스킬 §1.45 와 같은 규칙이고, **유니티 라인도
똑같이 탄다.** (그 규칙이 캔버스 스킬에만 있어서 전란이 통째로 건너뛰었다.)

**장르로 추정하지 않는다. 참조작 스크린샷에서 잰다.**

1. 참조작 **인게임** 프레임을 `_ref/` 에 받는다 (로컬만, 커밋 금지). 광고·프로모 프레임은
   인물이 과장돼 있으니 **실제 플레이 화면**을 쓴다
2. 주인공을 crop 해 **픽셀 bbox 세로**를 잰다 → `1/N = 화면 짧은 변 ÷ 그 높이`
3. 같은 방법으로 **일반 유닛**도 잰다
4. 두 수치를 **PRD 에 「참조작 실측」으로 적고**, 우리 값을 그 **±15%** 안에서 정한다
5. **잰 파일명과 crop 좌표를 함께 적는다** — 안 적으면 다음 사람이 다시 못 잰다

> **추정치를 「실측」이라고 적지 않는다.** 전란 PRD 에 「참조작 실측 1/8.3」이라 적어 두고
> 실제로 잰 값은 **1/14.3** 이었다(1.5배). 그 한 줄 때문에 카메라가 처음부터 1.5배 가까웠고,
> 지형·아트를 아무리 손봐도 「확대된 느낌」이 안 없어졌다.

**축척 게이트는 두 가지를 다 재야 한다.**

| | 무엇 | 없으면 |
|---|---|---|
| ① | 우리 값이 화면에 그대로 나오는가 | 축척이 코드에만 있고 화면은 다르다 |
| ② | **그 값이 참조작 실측치의 ±15% 안인가** | **자기 사본을 재게 되어 두 배 틀려도 초록불** |

②가 없는 축척 게이트는 게이트가 아니다 — 전란 `ScaleGate` 가 「주인공 41.05px, 어긋남 0%」로
계속 통과하고 있었는데, 그 41.05 자체가 참조작의 1.5배였다.
...
| 글자 몇 개가 **빈다**(「이동 방  /」) | 글꼴 굽기가 소스 리터럴을 모으는데 **보간식 안의 문자열**(`{(a ? "WASD" : "방향키")}`)은 못 읽는다 | 문자열을 변수로 빼서 쓴다. `--skip-font` 는 이제 **굽기 대신 검사**를 돌린다(`FontBake.Check` — 굽기와 같은 charset 을 써서 아틀라스에 없는 글자가 있으면 빌드를 세운다). 이 줄은 원래 경고만이었고, 그래서 아틀라스가 사흘 동안 옛 글자 수에 멈춘 채 배포됐다 |
