...
| 인피니티 스탭 (2026-09-03, Only Up 개정판) | **7.07MB** | 3.77 / 4.17초 | 번들 없음 — 아트가 전부 코드. 지형을 단단한 덩이·비탈 프리즘으로 바꿔도 전송량 +0.01MB. 로컬 정적 서버(nginx 규약 재현) + 독립 프로필 헤드리스 크롬, 스크립트는 세션 스크래치패드(`serve.mjs`·`cdp.mjs`, 약 120줄) |
