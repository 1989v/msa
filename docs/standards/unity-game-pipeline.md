...
| 인피니티 스탭 (2026-09-05, 끼임 방지판) | **7.08MB** | 3.78 / 4.03초 | 번들 없음 — 아트가 전부 코드. 지형을 단단한 덩이·비탈 프리즘으로 바꾸고 지도를 가로지르는 나선 배치(발판 1,960)로 넓혀도 전송량 +0.01MB. **첫 프레임은 탑 생성 시간이 정한다**(1,450 → 1,960 발판 + 궤적 검사에 +1초) — 예산 5초 안이나 여유 0.2초. 로컬 정적 서버(nginx 규약 재현) + 독립 프로필 헤드리스 크롬, 스크립트는 세션 스크래치패드(`serve.mjs`·`cdp.mjs`, 약 120줄) |
