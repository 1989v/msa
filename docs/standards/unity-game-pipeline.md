...
# 유니티 게임 제작 파이프라인 (WebGL)

> [!important] **파일럿을 통과했다 — 유니티는 정식 선택지다.** 새 게임 착수는 §8 이 순서를
> 갖고 있고, 넘어야 할 선과 이미 잰 값은 §6·§6.1·§6.2 다.
>
> | | |
> |---|---|
> | Editor | `6000.5.9f1` — `/Applications/Unity/Hub/Editor/`, CLI `~/.unity/bin/unity` |
> | 서빙 중인 유니티 게임 | `archer-outbreak` · `nine-bells` · `last-one` · `infinity-step` |
> | 프레임워크 | `unity/packages/com.kgd.webgame` (API 원본은 그 `README.md`) |
> | 빌드 | `scripts/unity-build-web.sh <slug>` — **레포 루트에서** |
>
> **[확인]** 표시는 라이선스·브라우저 지원처럼 **공식 문서로 확인한 외부 사실**이다.
> 용량·성능은 추정이 아니라 §6.1·§6.2 의 실측값이고, **새 게임을 내면 §6.2 표에 그 게임의
> 값을 한 줄 더한다** — 안 더하면 예산이 어디까지 왔는지 다음 사람이 다시 재야 한다.
...
| 인피니티 스탭 (2026-09-05, 도시 테마판) | **7.08MB** | 4.14 / 4.20초 · **fps 는 부하 오염으로 보류**(측정 시 load 12.26, 소품을 줄였는데 값이 더 떨어졌다 — 소프트웨어 GL 은 CPU 경합에 딸려 간다) | 번들 없음 — 아트가 전부 코드. 지형을 단단한 덩이·비탈 프리즘으로 바꾸고 지도를 가로지르는 나선 배치(발판 1,960)로 넓혀도 전송량 +0.01MB. **첫 프레임은 탑 생성 시간이 정한다**(1,450 → 1,960 발판 + 궤적 검사에 +1초) — 예산 5초 안이나 여유 0.2초. 로컬 정적 서버(nginx 규약 재현) + 독립 프로필 헤드리스 크롬, 스크립트는 세션 스크래치패드(`serve.mjs`·`cdp.mjs`, 약 120줄) |
