...
### 계측을 잴 때는 UA 를 사람 것으로 바꾼다 (2026-09-20)

노출·클릭 원장(ADR-0095)의 수집 엔드포인트는 **크롤러 UA 를 조용히 버린다** — 202 를 돌려주고
`accepted=0` 이다(원장에 봇 순서가 남지 않게 한 것). 헤드리스 크롬의 기본 UA 에는
`HeadlessChrome` 이 들어 있어 이 목록에 걸린다.

그래서 브라우저에서 이벤트가 나가는 것까지 봐도 **원장은 비어 있다.** 계측을 재는 검사는
`Network.setUserAgentOverride` 로 사람 브라우저 UA 를 씌우고, 마지막에는 **보낸 건수와
저장된 행 수를 맞춰 본다** — 「보냈다」는 내가 한 일이고 「쌓였다」가 대상의 값이다.
...
### 링크가 안 열리면 — 남은 헤드리스가 URL 을 먹고 있다 (2026-09-03)

"터미널·IDE 에서 링크를 눌러도 크롬이 안 열린다"는 **먼저 떠 있는 헤드리스 크롬**의 증상이다. 같은 앱 번들이라
macOS 는 링크 열기 이벤트를 먼저 뜬 인스턴스에 보내고, 헤드리스는 창이 없으니 아무 일도 안 생긴다.
실측: 다른 세션이 스크립트를 우회해 띄우고 안 끈 인스턴스가 23시간 47분 살아 있었다.

```bash
scripts/cdp-chrome.sh sweep        # 스크래치패드 프로필로 뜬 헤드리스 중 30분 넘게 산 것을 종료 (사용자 크롬·MCP 제외)
scripts/cdp-chrome.sh sweep 5      # 임계를 5분으로
```

`stop`/`clean` 은 자기 세션만 끄지만 `sweep` 은 세션을 가리지 않는다 — 대신 **헤드리스**이고 **스크래치패드 프로필**이고
**오래 산 것**만 고른다. 사용자 크롬은 `--headless` 가 없고, MCP 프로필은 경로로 거른다.
