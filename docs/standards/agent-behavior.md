...
- **progress.md 에 반드시 넣는 넷** (2026-09-06 보강 — 없어서 실제로 곤란했던 것들):
  1. **재계산 비용이 큰 중간 산출물의 경로와 비용.** 스크래치패드의 임베딩 풀링 결과·모델 캐시·벤치 산출물처럼 다시 만드는 데 수십 분
     걸리는 것은 경로를 적지 않으면 컴팩션 후 그냥 다시 돈다. `<경로> — 재생성 N분` 형식.
     **세션이 바뀌면 스크래치패드 경로도 바뀐다** — 이어질 작업이면 레포나 볼트로 옮기거나, 최소한 재생성 명령을 남긴다.
  2. **작업 위치.** 공유 워킹트리가 아닌 곳(별도 `git worktree`, 다른 클론)에서 작업 중이면 그 경로와 이유. 모르고 공유 트리에서
     이어받으면 다른 세션의 리베이스에 스테이지 파일이 사라진다(2026-09-05 실제 발생).
  3. **막힌 것과 그 주체.** "사용자가 `gh auth switch` 를 해야 푸시된다" 처럼 **내가 풀 수 없는 것**을 적는다.
     안 적으면 다음 세션이 같은 시도를 반복한다.
  4. **되풀이하면 안 되는 것.** 이 작업에서 실제로 밟은 함정 — 조용한 실패의 모양과 그것을 잡은 방법.
- 다단계 작업(플랜이 있는 것)은 progress.md 대신 **이어받기 문서**(`docs/plans/<날짜>-<주제>-handoff.md`)를 써도 된다.
  판단 기준은 하나 — **다른 세션이 그 문서만 읽고 이어받을 수 있는가.** 산출물 위치·다음 단계·막힌 것이 없으면 아직 아니다.
