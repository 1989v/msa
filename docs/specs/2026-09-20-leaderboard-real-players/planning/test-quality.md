...
- 단위(`GameScoreServiceTest`): `isOperator`/`isAutomation` 각각과 둘 다 false. 판정 근거는 **`scoreRepository.submit` 호출 여부**(`verify(exactly = 0)`)와 반환값 — 응답 필드만 보면 저장이 새도 초록불이다.
...
- 데이터: 패턴으로 행을 **읽어** 목록을 남기고 id 로 지운다. 순서는 빨간불 e2e(배포 전, 옛 서버) → 배포 → 초록불 e2e → 운영자 프로브 → 정리(마지막). 미리보기 행 수 = 목록 길이일 때만 실행.
