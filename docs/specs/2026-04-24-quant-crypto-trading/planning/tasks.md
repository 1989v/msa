...
- [ ] TG-08.0 **Complete**: Flyway 마이그레이션 성공 + JPA 엔티티 CRUD 통합 테스트(Testcontainers MySQL) green.
  ...
  - [ ] TG-08.2 JPA Entity 매핑 — `TrancheStrategyEntity`, `StrategyRunEntity`, `TrancheSlotEntity`, `OrderEntity`, `ExchangeCredentialEntity`, `NotificationTargetEntity`, `OutboxEntity`, `ProcessedEventEntity`, `AuditLogEntity`. 도메인 모델은 Entity와 분리(infrastructure/persistence/mapper)
  ...
  - [ ] TG-08.4 Mapper — `TrancheStrategy ↔ TrancheStrategyEntity`, `TrancheSlot ↔ TrancheSlotEntity` 등. 도메인 ↔ 엔티티 단방향 변환만 허용
  - [ ] TG-08.5 Adapter 구현(port → JPA): `JpaStrategyRepository : StrategyRepositoryPort`, `JpaTrancheSlotRepository : TrancheSlotRepositoryPort`, 등
  ...
  - [ ] TG-08.9 **Verify**: `./gradlew :quant:app:test --tests '*JpaRepository*' --tests '*PersistenceIntegrationSpec*'` 성공
