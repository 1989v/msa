...
- [ ] TG-03.0 **Complete**: `./gradlew :quant:domain:test` 실행 시 INV-01 ~ INV-07 모두 green.
  - [ ] TG-03.1 `fixtures/TrancheFixtures.kt` — `arbTrancheStrategyConfig()`, `arbTrancheSlot()`, `arbPriceTick()` Kotest Arb 제공, 공유 가변 상태 금지
  - [ ] TG-03.2 `TrancheStrategyConfigPropertySpec` — INV-07 검증: 생성 가능한 모든 조합이 `roundCount ∈ [1,50]`, `entryGapPercent < 0`, `takeProfitPercentPerRound` 각 원소 > 0 이고 길이 = roundCount. 경계값 및 위반 입력에 대해 예외 발생
  - [ ] TG-03.3 `NoStopLossInvariantSpec` — INV-01: 임의의 가격 경로(급락 포함)에서 `TrancheStrategy` 시뮬 실행 후 수집된 `DomainEvent` 목록에 `StopLoss*`류 이벤트가 0건임을 확인
  - [ ] TG-03.4 `TrancheSlotIndependentSellSpec` — INV-02: 슬롯 i의 `fillSell`은 슬롯 i의 `entryPrice`에만 의존. 평균단가를 주입해도 매도 precondition이 참조하지 않음을 확인 (mock Context)
  ...
  - [ ] TG-03.6 `TrancheSlotStateTransitionSpec` — 허용된 전이 path만 성공, 나머지는 `IllegalSlotTransitionException`. BehaviorSpec Given/When/Then 서술
  ...
  - [ ] TG-03.9 **Verify**: `./gradlew :quant:domain:test --tests '*PropertySpec' --tests '*InvariantSpec'` 전부 성공
