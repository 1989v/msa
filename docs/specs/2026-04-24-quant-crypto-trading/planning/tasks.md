...
- [ ] TG-06.0 **Complete**: `quant/app/src/main/resources/clickhouse/quant/` 아래 스키마 DDL이 존재하고 Testcontainers에서 create가 성공한다.
  - [ ] TG-06.1 DDL 파일 `V001__create_database.sql` — `CREATE DATABASE IF NOT EXISTS quant`
  ...
  - [ ] TG-06.5 `ClickHouseConfig` — 애플리케이션에서 사용할 JDBC/HTTP 클라이언트 설정, database = `quant` 고정
  ...
  - [ ] TG-06.8 **Verify**: `./gradlew :quant:app:test --tests '*ClickHouseSchemaSmokeSpec*'` 성공
