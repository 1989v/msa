...
# Task Breakdown — quant-crypto-trading (Phase 1: 백테스트)

## Overview

본 문서는 `quant` 서비스의 **Phase 1 (백테스트 엔진) MVP** 구현 태스크 리스트이다. Phase 2 (페이퍼 트레이딩)와 Phase 3 (실매매)는 범위 밖이며, 별도 스펙/태스크 사이클로 진행한다. (tasks.md 말미 "Out of scope for Phase 1" 참조)
...
- **Complexity**: S
- **산출물**: `docs/specs/2026-04-24-quant-crypto-trading/context/exchange-terms.md` (약관 조항 링크 + 발췌 요약)
- **의존**: 없음
- **DoD**:
  - [ ] 빗썸·업비트 OpenAPI 약관에서 자동매매 허용·금지 조항 캡처
  - [ ] 개인 운용 허용 범위 + 상업적 사용/재판매 제한 명시
  - [ ] `open-questions.yml` OQ-011 `status: closed` 로 갱신
- **Verification**: `grep -n "OQ-011" docs/specs/2026-04-24-quant-crypto-trading/context/open-questions.yml` → status closed 확인

### P.1 OQ-008 데이터 적재 파이프라인 설계 문서
- **Complexity**: S
- **산출물**: `docs/specs/2026-04-24-quant-crypto-trading/context/data-ingestion.md`
- **의존**: 없음
- **DoD**:
  - [ ] 빗썸 REST 히스토리 API 엔드포인트·파라미터·응답 스키마 정리
  - [ ] `quant.market_tick_bithumb` ClickHouse 스키마 제안 (PK/ORDER BY/TTL/partition 포함)
...
- [x] TG-01.0 **Complete**: quant 서브모듈이 `./gradlew projects`에 등록되고 빈 `:quant:domain:test`, `:quant:app:bootJar` 이 성공한다. (2026-04-24 완료)
  - [x] TG-01.1 `settings.gradle.kts`에 `"quant:domain"`, `"quant:app"` include 추가
  - [x] TG-01.2 파일시스템 스캐폴드: `quant/domain/`, `quant/app/` 디렉토리 + 빈 `build.gradle.kts`
  - [x] TG-01.3 `quant/domain/build.gradle.kts`: Kotlin JVM only, 의존성 `common` 만 (Spring/JPA 없음) — `product/domain` 패턴 복제
  - [x] TG-01.4 `quant/app/build.gradle.kts`: Spring Boot 4.0.4 + Kotlin 2.2.21, `:quant:domain` + `common` + JPA/Web/Kafka/Redis starter — `analytics/app` 패턴 참고
  ...
  - [x] TG-01.6 패키지 루트 스캐폴드: `quant/app/src/main/kotlin/com/kgd/quant/` + `QuantApplication.kt` (`@SpringBootApplication`), `application.yml`/`application-kubernetes.yml` 빈 템플릿
  - [x] TG-01.7 domain 테스트 1개 더미 BehaviorSpec (`SmokeSpec`) 추가해서 빌드 파이프라인 확인
  - [x] TG-01.8 Jib 설정 (`jibBuildTar`) 추가 — 이미지 name `quant`, 기존 서비스 패턴 복제 (루트 `build.gradle.kts`의 `commerce.jib-convention`이 `spring-boot` 플러그인에 자동 적용되므로 별도 설정 불필요)
  - [x] TG-01.9 **Verify**: `./gradlew :quant:domain:test :quant:app:bootJar` 모두 성공 (2026-04-24 완료, SmokeSpec 1 passed, bootJar 생성 성공, domain 소스 Spring import 0건)

**Acceptance Criteria**:
- `./gradlew projects` 결과에 `:quant:domain`, `:quant:app` 포함
- domain 모듈에 Spring/JPA 의존이 전이되지 않음 (`./gradlew :quant:domain:dependencies --configuration runtimeClasspath` 로 확인)
- `libs.turbine`, `libs.kotest.property`, `libs.testcontainers.*` 참조가 domain/app 빌드에서 해소됨
- `CLAUDE.md` Navigation 서비스 표에 quant CLAUDE.md 자리 표기 (실제 CLAUDE.md는 TG-14에서 생성)

---

### Task Group TG-02: 도메인 모델 + 상태머신 (TrancheStrategy / StrategyRun / TrancheSlot / Order)
...
순수 도메인(`:quant:domain`)에 Aggregate/Entity/VO + 상태머신을 올린다. Spring/JPA/프레임워크 의존 금지. 부분체결(OQ-020)은 **별도 substate 없이 `TrancheSlot`에 `filledQty / targetQty` 필드를 두어 partial fill을 표현**하는 방식으로 인라인 확정한다 (상태는 기존 5단계 유지: `EMPTY → PENDING_BUY → FILLED → PENDING_SELL → CLOSED → EMPTY`, `FILLED`은 `filledQty >= targetQty` 조건).

- [x] TG-02.0 **Complete**: domain 모듈에 Aggregate/Entity/VO + 상태 전이 메서드가 존재하고, domain 단위 테스트(다음 TG-03에서 작성)에서 호출 가능하다.
  - [x] TG-02.1 패키지 생성: `quant/domain/src/main/kotlin/com/kgd/quant/domain/{strategy,slot,order,credential,notification}/` + `common/{Price.kt, Quantity.kt, TenantId.kt, ExecutionMode.kt}`
  - [x] TG-02.2 `TrancheStrategyConfig` VO — `roundCount: Int`, `entryGapPercent: BigDecimal`, `takeProfitPercentPerRound: List<BigDecimal>`, `initialOrderAmount: BigDecimal`, `targetSymbol: String`, 생성자에서 INV-07 검증 (`roundCount ∈ [1,50]`, `entryGapPercent < 0`, 배열 길이 == roundCount, 모든 요소 > 0), 위반 시 `TrancheStrategyConfigInvalidException : BusinessException`
  - [x] TG-02.3 `TrancheStrategy` Aggregate Root — `strategyId`, `tenantId`, `TrancheStrategyConfig`, `ExecutionMode`, `StrategyStatus`, 메서드: `activate()`, `pause()`, `resume()`, `liquidate(reason)`, `nextRoundEntryCondition(lastFilledRound): PriceCondition`
  ...
  - [x] TG-02.7 `TrancheSlot` Entity — `slotId`, `runId`, `roundIndex`, `state: TrancheSlotState`, `entryPrice: Price?`, `targetQty: Quantity`, `filledQty: Quantity`, `takeProfitPercent: BigDecimal`. 메서드: `requestBuy(price)`, `fillBuy(executedPrice, executedQty)` (partial fill 누적, `filledQty >= targetQty`이면 `FILLED` 전이), `requestSell()`, `fillSell(executedPrice)` precondition 검증(INV-02, 위반 시 `StopLossAttemptException`)
  - [x] TG-02.8 `TrancheSlotState` enum + 전이 가드 (`EMPTY → PENDING_BUY → FILLED → PENDING_SELL → CLOSED → EMPTY`)
  ...
  - [x] TG-02.12 도메인 이벤트 sealed class 계층 `DomainEvent` + 구현(`StrategyActivated`, `StrategyPaused`, `StrategyResumed`, `StrategyLiquidated`, `TrancheSlotOpened`, `TrancheSlotClosed`, `OrderPlaced`, `OrderFilled`, `OrderPartiallyFilled`, `OrderFailed`, `OrderCancelled`, `RiskLimitBreached`, `EmergencyLiquidationTriggered`, `ExchangeConnectionDegraded`, `ExchangeConnectionRestored`)
  - [x] TG-02.13 `BusinessException` 파생 도메인 예외 정리: `TrancheStrategyConfigInvalidException`, `IllegalStrategyTransitionException`, `IllegalSlotTransitionException`, `StopLossAttemptException`, `LeverageAttemptException`
  - [x] TG-02.14 **Verify**: `./gradlew :quant:domain:compileKotlin` 성공, domain 모듈이 Spring/JPA 의존성을 포함하지 않음을 `./gradlew :quant:domain:dependencies --configuration compileClasspath` 로 재확인

**Acceptance Criteria**:
- 모든 Aggregate/Entity가 프레임워크 애노테이션 없이 순수 Kotlin으로 컴파일됨
- `TrancheSlot`의 상태 전이는 `private set` 또는 메서드 호출로만 가능 (ADR-0022 캡슐화)
...
`test-quality.md §5` 정합성 불변식을 kotest-property로 강제한다. `:quant:domain` 테스트 소스셋에만 위치하며 Spring context를 로드하지 않는다. 테스트는 각 INV당 1개 핵심 property + behavioural Given/When/Then 1~2개로 구성, 과도한 exhaustive 테스트는 지양한다 (2~8 focused tests 원칙).

- [x] TG-03.0 **Complete**: `./gradlew :quant:domain:test` 실행 시 INV-01 ~ INV-07 모두 green.
  - [x] TG-03.1 `fixtures/TrancheFixtures.kt` — `arbTrancheStrategyConfig()`, `arbTrancheSlot()`, `arbPriceTick()` Kotest Arb 제공, 공유 가변 상태 금지
  - [x] TG-03.2 `TrancheStrategyConfigPropertySpec` — INV-07 검증: 생성 가능한 모든 조합이 `roundCount ∈ [1,50]`, `entryGapPercent < 0`, `takeProfitPercentPerRound` 각 원소 > 0 이고 길이 = roundCount. 경계값 및 위반 입력에 대해 예외 발생
  - [x] TG-03.3 `NoStopLossInvariantSpec` — INV-01: 임의의 가격 경로(급락 포함)에서 `TrancheStrategy` 시뮬 실행 후 수집된 `DomainEvent` 목록에 `StopLoss*`류 이벤트가 0건임을 확인
  - [x] TG-03.4 `TrancheSlotIndependentSellSpec` — INV-02: 슬롯 i의 `fillSell`은 슬롯 i의 `entryPrice`에만 의존. 평균단가를 주입해도 매도 precondition이 참조하지 않음을 확인 (mock Context)
  ...
  - [x] TG-03.6 `TrancheSlotStateTransitionSpec` — 허용된 전이 path만 성공, 나머지는 `IllegalSlotTransitionException`. BehaviorSpec Given/When/Then 서술
  ...
  - [x] TG-03.9 **Verify**: `./gradlew :quant:domain:test --tests '*PropertySpec' --tests '*InvariantSpec'` 전부 성공
...
- [x] TG-04.0 **Complete**: port 인터페이스만으로 컴파일되고, 각 port에 Kdoc 시그니처/계약/에러 조건이 기술됨.
  - [x] TG-04.1 `com.kgd.quant.domain.common.Clock` — `fun now(): Instant`, 테스트용 `FakeClock` 구현 domain test 소스셋에 동반
  - [x] TG-04.2 `com.kgd.quant.application.exchange.ExchangeAdapter` (application layer) — `suspend fun placeOrder(cmd: OrderCommand): OrderAck`, `suspend fun cancelOrder(tenantId, exchangeOrderId)`, `suspend fun fetchBalance(tenantId, exchange): Balance`, `suspend fun fetchExecution(orderId: UUID): ExecutionReport?`
  ...
  - [x] TG-04.7 Repository ports: `StrategyRepositoryPort`, `StrategyRunRepositoryPort`, `TrancheSlotRepositoryPort`, `OrderRepositoryPort`, `ExchangeCredentialRepositoryPort`, `OutboxRepositoryPort`, `BacktestRunRepositoryPort` — 모든 조회/저장 시그니처에 `tenantId: TenantId` 필수 파라미터
  ...
  - [x] TG-04.9 **Verify**: `./gradlew :quant:domain:compileKotlin :quant:app:compileKotlin` 성공 + Kdoc missing 경고 0
...
- [x] TG-05.0 **Complete**: 고정 시드·고정 fixture CSV 입력에 대해 결정론적으로 동일한 `StrategyRun` 결과(체결 리스트·실현 손익·종료 시각)를 산출한다.
  - [x] TG-05.1 `StrategyExecutor` — `ExecutionMode`, `ExchangeAdapter`, `HistoricalMarketDataSource`(또는 실시간 Subscriber), `Clock`, `EventPublisher`를 주입받아 실행. Phase 1은 백테스트 전용 경로
  - [x] TG-05.2 `BacktestExchangeAdapter : ExchangeAdapter` — placeOrder 호출 시 직전 tick 가격으로 즉시 체결(또는 slippage 0 기본), UUID v7 `orderId` idempotent 저장, 가상 잔고 관리
  - [x] TG-05.3 `CsvHistoricalMarketDataSource : HistoricalMarketDataSource` — `src/test/resources/golden/bithumb/*.csv` 를 `Flow<Bar>`로 변환, timestamp 오름차순 보장
  - [x] TG-05.4 `StrategyEngineLoop` — 각 Bar마다 (1) 매수 트리거 평가(FR-ENG-03) (2) 매도 트리거 평가(FR-ENG-04) (3) 전 회차 소진 감지(FR-ENG-06, `AWAITING_EXHAUSTED` 전이) (4) 도메인 이벤트 EventPublisher로 발행
  - [x] TG-05.5 `FakeClock` 주입 경로 — Bar timestamp가 곧 현재시각이 되도록 엔진 내부에서 Clock 래핑. `Random` 시드도 명시 주입(`RandomSource(seed)`)
  - [x] TG-05.6 **결정론 테스트**: 동일 입력(동일 CSV + 동일 config + 동일 seed)에 대해 2회 실행 후 이벤트 시퀀스·체결가·PnL이 완전히 일치(byte-level). 2개 시나리오(tight/volatile) 최소 커버
  - [x] TG-05.7 **AWAITING_EXHAUSTED 시나리오 테스트**: 급락으로 모든 회차 소진 후 추가 하락 시 신규 매수 이벤트가 발행되지 않음
  - [x] TG-05.8 **슬롯 재사용 테스트 (FR-ENG-05)**: 슬롯이 매도 완료 후 `EMPTY`로 복귀하고 추가 하락 시 재매수 트리거가 다시 성립
  - [x] TG-05.9 **Verify**: `./gradlew :quant:app:test --tests '*BacktestEngineSpec*'` 성공, 실행 로그에 시드·입력 해시 기록

**Acceptance Criteria**:
- 두 번 실행 결과 byte-level diff 0
- `:quant:domain` 모듈에는 백테스트 구현 코드가 들어가지 않음(도메인은 순수)
- `BacktestExchangeAdapter`는 margin/future API를 호출하지 않음 (원칙 2)
- `AWAITING_EXHAUSTED` → 슬롯 매도 → `ACTIVE` 복귀 경로가 테스트로 보호됨

---

### Task Group TG-06: ClickHouse `quant` DB 스키마
...
ADR-0024 §12에 따라 analytics 인프라 재사용 + 별도 DB `quant`에 테이블 신설. Phase 1은 `market_tick_bithumb`(분봉) + `backtest_run` 중심. `execution_result`는 Phase 2/3에서 확장.

- [x] TG-06.0 **Complete**: `quant/app/src/main/resources/clickhouse/quant/` 아래 스키마 DDL이 존재하고 Testcontainers에서 create가 성공한다.
  - [x] TG-06.1 DDL 파일 `V001__create_database.sql` — `CREATE DATABASE IF NOT EXISTS quant`
  - [x] TG-06.2 DDL 파일 `V002__market_tick_bithumb.sql` — 컬럼: `symbol String, ts DateTime64(3, 'UTC'), open Decimal(38,8), high Decimal(38,8), low Decimal(38,8), close Decimal(38,8), volume Decimal(38,8), interval LowCardinality(String), ingestedAt DateTime64(3,'UTC')`, `ENGINE = ReplacingMergeTree(ingestedAt)`, `PARTITION BY toYYYYMM(ts)`, `ORDER BY (symbol, interval, ts)`, TTL 정책(5년) — Phase 1에서는 TTL은 코멘트로만 두고 실제 적용은 OQ-008 해소 이후
  - [x] TG-06.3 DDL 파일 `V003__backtest_run.sql` — `runId UUID, tenantId String, strategyId UUID, symbol String, configJson String, seed Int64, fromTs DateTime64(3,'UTC'), toTs DateTime64(3,'UTC'), realizedPnl Decimal(38,8), mdd Decimal(10,6), sharpe Decimal(10,6), fillCount UInt64, startedAt DateTime64(3,'UTC'), endedAt DateTime64(3,'UTC')`, `ENGINE = MergeTree`, `ORDER BY (tenantId, strategyId, startedAt)`
  - [x] TG-06.4 DDL 파일 `V004__execution_result_placeholder.sql` — Phase 1 최소 스키마(체결 시계열 스텁), 실 테이블 확장은 Phase 2. 본 TG에서는 create만 확보
  - [x] TG-06.5 `ClickHouseConfig` — 애플리케이션에서 사용할 JDBC/HTTP 클라이언트 설정, database = `quant` 고정
  - [x] TG-06.6 Testcontainers `clickhouse/clickhouse-server` 이미지로 스키마 create 스모크 테스트 (`ClickHouseSchemaSmokeSpec`)
  - [x] TG-06.7 fixture CSV → ClickHouse insert 유틸(나중 TG-07 배치에서 재사용)
  - [x] TG-06.8 **Verify**: `./gradlew :quant:app:test --tests '*ClickHouseSchemaSmokeSpec*'` 성공

**Acceptance Criteria**:
- DB명이 `quant`이며 analytics DB를 직접 참조하지 않음 (grep으로 확인)
...
- [x] TG-08.0 **Complete**: Flyway 마이그레이션 성공 + JPA 엔티티 CRUD 통합 테스트(Testcontainers MySQL) green.
  ...
  - [x] TG-08.2 JPA Entity 매핑 — `TrancheStrategyEntity`, `StrategyRunEntity`, `TrancheSlotEntity`, `OrderEntity`, `ExchangeCredentialEntity`, `NotificationTargetEntity`, `OutboxEntity`, `ProcessedEventEntity`, `AuditLogEntity`. 도메인 모델은 Entity와 분리(infrastructure/persistence/mapper)
  ...
  - [x] TG-08.4 Mapper — `TrancheStrategy ↔ TrancheStrategyEntity`, `TrancheSlot ↔ TrancheSlotEntity` 등. 도메인 ↔ 엔티티 단방향 변환만 허용
  - [x] TG-08.5 Adapter 구현(port → JPA): `JpaStrategyRepository : StrategyRepositoryPort`, `JpaTrancheSlotRepository : TrancheSlotRepositoryPort`, 등
  ...
  - [x] TG-08.9 **Verify**: `./gradlew :quant:app:test --tests '*JpaRepository*' --tests '*PersistenceIntegrationSpec*'` 성공
...
- [x] TG-09.0 **Complete**: UseCase 단위 테스트(MockK로 port 대체)가 모두 green이며, 통합 테스트에서 실 백테스트 경로가 DB/ClickHouse에 결과를 남긴다.
  - [x] TG-09.1 `CreateStrategyUseCase` — input DTO → `TrancheStrategyConfig` 검증 → Repository 저장 → `StrategyCreated`(도메인 이벤트 필요 시) 또는 바로 API 응답
  - [x] TG-09.2 `RunBacktestUseCase` — `tenantId`, `strategyId`, `from`, `to`, `seed?` 받아 `StrategyRun` 생성 → `StrategyExecutor` 실행(TG-05) → 결과 집계(실현 PnL / MDD / sharpe / 체결 수) → `strategy_run` + ClickHouse `backtest_run` 저장 → Outbox에 `StrategyActivated` + `StrategyLiquidated(Completed)` append. 외부 IO(ClickHouse read, Outbox append)는 트랜잭션 경계 분리
  - [x] TG-09.3 `GetStrategyQuery`, `ListStrategiesQuery`, `GetStrategyDetailQuery` — tenantId 스코프, QueryDSL projection
  - [x] TG-09.4 `GetBacktestRunsQuery`, `GetBacktestRunDetailQuery` — dashboard용, 정렬/필터
  - [x] TG-09.5 Leaderboard read-model skeleton — `LeaderboardQuery`(수익률/MDD/fillCount 정렬, tenantId 내부 랭킹), 산식은 OQ-010 해소 전까지 "현재 기본값"으로 명시 (실현 PnL 절대값 기준)
  - [x] TG-09.6 `ExecuteLiquidationUseCase`(Phase 3 진입 전에는 no-op stub + `NotImplementedInPhase1Exception`)
  - [x] TG-09.7 단위 테스트: 각 UseCase 성공 경로 1 + 실패 경로 1 (예: 존재하지 않는 strategyId, tenantId mismatch) — 포커스 4~6개
  - [x] TG-09.8 **Verify**: `./gradlew :quant:app:test --tests '*UseCaseSpec*'` 성공
...
- [x] TG-10. **Complete**: Phase 1 엔드포인트가 `ApiResponse<T>` 포맷으로 응답하고, MockMvc 슬라이스 테스트가 green.
  - [x] TG-10. `StrategyController` — `POST /api/v1/strategies`, `GET /api/v1/strategies`, `GET /api/v1/strategies/{id}`, `PATCH /api/v1/strategies/{id}` (paramter change, pause/resume)
  - [x] TG-10. `BacktestController` — `POST /api/v1/backtests`, `GET /api/v1/strategies/{id}/runs`
  - [x] TG-10. `DashboardController` — `GET /api/v1/dashboard/overview`, `GET /api/v1/dashboard/executions` (Phase 1은 백테스트 결과만 노출)
  - [x] TG-10. `LeaderboardController` — `GET /api/v1/leaderboard`, `POST /api/v1/leaderboard/compare`
  - [x] TG-10. `GlobalExceptionHandler` 확장 — `TrancheStrategyConfigInvalidException`, `IllegalStrategyTransitionException`, `StopLossAttemptException`, `NotImplementedInPhase1Exception` 매핑
  - [x] TG-10. `TenantIdHeaderArgumentResolver` — `X-User-Id` → `TenantId` 주입 (Gateway에서 JWT 검증 이후 헤더만 신뢰)
  - [ ] TG-10.7 MockMvc 슬라이스 테스트 (`@WebMvcTest`) — 컨트롤러별 happy path 1 + 403(다른 tenant) 1 + 400(config invalid) 1 = 3 × 4컨트롤러 ≤ 12 케이스, 포커스 6~8 커버
  - [ ] TG-10.8 **Verify**: `./gradlew :quant:app:test --tests '*ControllerSpec*'` 성공
...
### Task Group TG-11: FE 스캐폴드 `quant/frontend/` (React + Vite)
...
- [x] TG-11.0 **Complete**: `cd quant/frontend && npm install && npm run build` 성공 + `npm run test` (Vitest) green + 로컬 dev 서버에서 백엔드 API 연동 확인.
  - [x] TG-11.1 Vite + React + TypeScript + React Router + TanStack Query 스캐폴드, 프로젝트 루트 `quant/frontend/`
  - [x] TG-11.2 `src/api/client.ts` — `X-User-Id` 헤더 주입 (dev 환경에서는 고정값, 운영은 Gateway가 주입), `ApiResponse<T>` 언래핑 유틸
  - [x] TG-11.3 `src/api/endpoints/` — `strategies.ts`, `backtests.ts`, `dashboard.ts`, `leaderboard.ts` TypeScript 타입 + fetch 함수
  - [x] TG-11.4 라우트: `/strategies`(리스트), `/strategies/:id`(상세), `/backtests/new`(제출 폼), `/runs/:runId`(결과), `/leaderboard`(랭킹)
  - [x] TG-11.5 최소 대시보드 컴포넌트: 회차 카드 그리드(Phase 1은 빈 상태), 체결 이력 테이블(백테스트 fill list), 수익률 그래프(Recharts 경량)
  - [x] TG-11.6 `docs/conventions/frontend-design.md` 가드레일 준수: 타이포/색상/레이아웃/모션/접근성. Tailwind 설정 포함
  - [x] TG-11.7 Vitest + React Testing Library 테스트 4~6개: API 클라이언트 mock, 리스트 렌더, 리더보드 정렬 토글, 에러 핸들링
  - [x] TG-11.8 Playwright E2E는 본 TG 범위 밖 (Phase 2)
  - [x] TG-11.9 **Verify**: `cd quant/frontend && npm run build && npm run test` 성공
...
- [ ] TG-12.0 **Complete**: `./gradlew :quant:app:goldenTest` 태스크가 3종 시드 모두 green.
  ...
  - [ ] TG-12.4 Gradle 태스크 `goldenTest` — `:quant:app` 하위에 `test` 의 변종 태스크로 등록, 태그(`@Tag("golden")`) 필터링
  - [ ] TG-12.5 expected.json 업데이트 절차 문서화 — `quant/docs/golden-set-update.md` (TG-14에서 docs 생성 시 배치)
...
ADR-0019 Phase 2 배포 모드에 맞춰 `k8s/overlays/k3s-lite/quant/` overlay 작성. prod-k8s overlay는 Phase 3 직전 별도 TG.
...
- [x] TG-13.0 **Complete**: `kubectl apply -k k8s/overlays/k3s-lite` 실행 시 quant Deployment/Service가 Ready 상태.
  ...
  - [x] TG-13.7 k3d 로컬 클러스터에서 apply → `kubectl -n quant get pods` → Ready. `/actuator/health` GET 200 확인
  - [x] TG-13.8 **Verify**: `kubectl apply -k k8s/overlays/k3s-lite` + `kubectl rollout status deployment/quant-app -n quant` 성공
...
ADR-0021 로깅 규칙 + Phase 1 최소 메트릭 + `quant/CLAUDE.md` + `quant/docs/` 신설. 루트 `CLAUDE.md` Navigation 표 업데이트.

- [x] TG-14.0 **Complete**: 서비스 기동 시 `/actuator/prometheus` 에서 Phase 1 메트릭이 노출되며 `quant/CLAUDE.md`가 생성됨.
  ...
  - [x] TG-14.2 Micrometer 메트릭 Phase 1 subset:
    - `quant_strategy_evaluation_latency_seconds{mode="backtest"}` (p50/p95/p99)
    - `quant_backtest_run_total{status}`
    - `quant_backtest_run_duration_seconds`
    - `quant_ingest_bithumb_rows_total{symbol}`
    - `quant_outbox_pending_rows` (gauge)
  - [x] TG-14.3 Outbox 테이블 모니터링 기본 — `OutboxEntity` 미발행 행 수 gauge 노출, 단순 알람 기준은 문서에만 명시
  - [x] TG-14.4 `quant/CLAUDE.md` 작성 — 서비스 개요, Phase 로드맵, 모듈 구조, 주요 명령어, Navigation(spec/ADR/docs 링크)
  - [x] TG-14.5 `quant/docs/` 초기 구조 — `README.md`(요약), `golden-set-update.md`(TG-12 참조), `ingest-bithumb.md`(TG-07 참조)
  - [x] TG-14.6 루트 `CLAUDE.md` Navigation 표에 quant 항목 `(신규, Phase 1 진행 중)` 으로 업데이트 (기존 "미생성" → 경로 지정)
  - [x] TG-14.7 **Verify**: `./gradlew :quant:app:bootRun --args='--spring.profiles.active=local'` 기동 후 `curl localhost:<port>/actuator/prometheus | grep quant` 최소 3개 메트릭 노출

**Acceptance Criteria**:
- `logger.{info,debug,warn}` 호출이 모두 람다 형식 (grep 기반 검사 통과)
- Phase 1 메트릭 5종 이상 노출
- `quant/CLAUDE.md` 존재 + 루트 Navigation 갱신
...
- [ ] TG-15.0 **Complete**: E2E 통합 테스트 green + Readiness Checklist 모두 통과.
  - [x] TG-15. `QuantPhase1E2ESpec` (Testcontainers 기반 MySQL + ClickHouse + Kafka) — 시나리오:
    ...
    6. ClickHouse `quant.backtest_run` 테이블에 실행 메타 저장 확인
  - [ ] TG-15.2 결정론 재실행 체크 — 동일 전략 + 동일 기간 2회 백테스트 시 결과 완전 일치(시드 고정)
  - [ ] TG-15.3 FE dev 서버 + 백엔드 dev 서버 수동 검증 스크립트 — 전략 생성 → 백테스트 → `/runs/:runId` 결과 페이지 렌더 체크리스트(`quant/docs/phase1-readiness.md`)
  - [x] TG-15. Readiness Checklist 문서 `quant/docs/phase1-readiness.md`:
    - [ ] 모든 TG 완료
    - [ ] `./gradlew :quant:domain:test :quant:app:test :quant:app:goldenTest` 성공
  ...
  - [x] TG-15. 릴리즈 노트 초안 `docs/specs/2026-04-24-quant-crypto-trading/phase1-release-notes.md` — 구현된 FR/NFR 목록 + 범위 밖 항목 + Phase 2 Preflight 후보(OQ-007/017/018)
  - [ ] TG-15.6 **Verify**: `./gradlew :quant:app:test --tests '*Phase1E2ESpec*'` + Readiness Checklist 전 항목 수동 체크 + 최종 PR 리뷰
...
- `k8s/overlays/prod-k8s/quant/` overlay (HPA/PDB/TLS/NetworkPolicy)
