...
- [ ] TG-02.0 **Complete**: domain 모듈에 Aggregate/Entity/VO + 상태 전이 메서드가 존재하고, domain 단위 테스트(다음 TG-03에서 작성)에서 호출 가능하다.
  - [ ] TG-02.1 패키지 생성: `quant/domain/src/main/kotlin/com/kgd/quant/domain/{strategy,slot,order,credential,notification}/` + `common/{Price.kt, Quantity.kt, TenantId.kt, ExecutionMode.kt}`
  - [ ] TG-02.2 `TrancheStrategyConfig` VO — `roundCount: Int`, `entryGapPercent: BigDecimal`, `takeProfitPercentPerRound: List<BigDecimal>`, `initialOrderAmount: BigDecimal`, `targetSymbol: String`, 생성자에서 INV-07 검증 (`roundCount ∈ [1,50]`, `entryGapPercent < 0`, 배열 길이 == roundCount, 모든 요소 > 0), 위반 시 `TrancheStrategyConfigInvalidException : BusinessException`
  - [ ] TG-02.3 `TrancheStrategy` Aggregate Root — `strategyId`, `tenantId`, `TrancheStrategyConfig`, `ExecutionMode`, `StrategyStatus`, 메서드: `activate()`, `pause()`, `resume()`, `liquidate(reason)`, `nextRoundEntryCondition(lastFilledRound): PriceCondition`
  ...
  - [ ] TG-02.12 도메인 이벤트 sealed class 계층 `DomainEvent` + 구현(`StrategyActivated`, `StrategyPaused`, `StrategyResumed`, `StrategyLiquidated`, `TrancheSlotOpened`, `TrancheSlotClosed`, `OrderPlaced`, `OrderFilled`, `OrderPartiallyFilled`, `OrderFailed`, `OrderCancelled`, `RiskLimitBreached`, `EmergencyLiquidationTriggered`, `ExchangeConnectionDegraded`, `ExchangeConnectionRestored`)
  - [ ] TG-02.13 `BusinessException` 파생 도메인 예외 정리: `TrancheStrategyConfigInvalidException`, `IllegalStrategyTransitionException`, `IllegalSlotTransitionException`, `StopLossAttemptException`, `LeverageAttemptException`
  - [ ] TG-02.14 **Verify**: `./gradlew :quant:domain:compileKotlin` 성공, domain 모듈이 Spring/JPA 의존성을 포함하지 않음을 `./gradlew :quant:domain:dependencies --configuration compileClasspath` 로 재확인
