...
- [ ] TG-09.0 **Complete**: UseCase 단위 테스트(MockK로 port 대체)가 모두 green이며, 통합 테스트에서 실 백테스트 경로가 DB/ClickHouse에 결과를 남긴다.
  - [ ] TG-09.1 `CreateStrategyUseCase` — input DTO → `TrancheStrategyConfig` 검증 → Repository 저장 → `StrategyCreated`(도메인 이벤트 필요 시) 또는 바로 API 응답
  ...
  - [ ] TG-09.8 **Verify**: `./gradlew :quant:app:test --tests '*UseCaseSpec*'` 성공
