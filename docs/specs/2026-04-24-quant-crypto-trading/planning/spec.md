...
depends-on:
  ...
  - ideabank/docs/12-quant-crypto.md
standards:
...
# Specification — quant-crypto-trading

> Last revised: 2026-04-24 (post review-1)

## 1. Overview & Goals

『분할매매』의 정통 분할 원칙(특히 원칙 5·6·7: 직전 회차 대비 -3% 추가매수, 회차별 동일 매수 금액, 손절 없음)을 빗썸/업비트 현물 암호화폐 거래에 적용하는 **자동매매 웹 서비스**를 구축한다. 변동성을 리스크가 아닌 **회차별 독립 익절 기회**로 전환해 꾸준한 현금흐름을 만드는 것이 제품의 존재 이유다.
...
| Gradle 경로 | 파일시스템 | 역할 |
|---|---|---|
| `:quant:domain` | `quant/domain/` | 순수 도메인: `TrancheStrategy`, `TrancheSlot`, 트리거 정책, 도메인 이벤트 |
| `:quant:app` | `quant/app/` | Spring Boot: UseCase, Port, Infrastructure Adapter, REST |

`:quant:domain` 의존: `common` (BusinessException/ErrorCode) 한정. Spring 의존성 없음.
`:quant:app` 의존: `:quant:domain` + `common` + Spring Boot 4.0.4 / Kotlin 2.2.21 / Java 25 런타임 (ADR-0002).
...
### 2.4 패키지 (`com.kgd.quant.*`)

```
com.kgd.quant/
├── domain/
│   ├── strategy/   (model, policy, event, exception)
│   ├── slot/       (TrancheSlot 상태머신)
...
> 네이밍 주의: 기존 플랫폼 컨벤션(서비스 식별자에 underscore 없음)을 따라 `com.kgd.quant` (snake_case 금지) 사용.
...
| 이름 | 타입 | 주요 속성 | 불변식 |
|---|---|---|---|
| `TrancheStrategy` | Aggregate Root | `strategyId`, `tenantId`, `targetSymbol`, `TrancheStrategyConfig`, `ExecutionMode`, `StrategyStatus` | `roundCount ∈ [1,50]`, `entryGapPercent < 0`, 모든 `takeProfitPercentPerRound[i] > 0` (INV-07) |
| `TrancheStrategyConfig` | Value Object | `roundCount`, `entryGapPercent`, `takeProfitPercentPerRound: List<BigDecimal>`, `initialOrderAmount`, 위험한도 | 회차별 익절 배열 길이 = roundCount, 모든 회차 매수 명목 동일 (INV-03) |
...
| `TrancheSlot` | Entity | `slotId`, `runId`, `roundIndex`, `state`, `entryPrice`, `quantity`, `takeProfitPercent` | 상태 전이: `EMPTY → PENDING_BUY → FILLED → PENDING_SELL → CLOSED → EMPTY` (FR-ENG-05, ADR-0022 캡슐화) |
...
- `TrancheSlot.state`: `EMPTY → PENDING_BUY → FILLED → PENDING_SELL → CLOSED → EMPTY` (`EMPTY` 복귀 시 재사용)
...
- `TrancheSlotOpened`, `TrancheSlotClosed`
...
- **도메인 메서드 시그니처**:
  - `TrancheSlot.evaluateSellTrigger(tick: Price): SellDecision`
  - `TrancheStrategy.nextRoundEntryCondition(lastFilledRound: TrancheSlot): PriceCondition`
  - `TrancheSlot.sell(executedPrice: Price)` precondition: `executedPrice >= entryPrice × (1 + takeProfitPercent)`. 위반 시 `StopLossAttemptException`.
...
| **Kafka** | 도메인 이벤트 발행 (`quant.*.v1` 토픽). DLQ는 `.DLT` 접미사 (kafka-convention.md) |
...
analytics ClickHouse **인프라(노드/클러스터)는 재사용**하되, 별도 데이터베이스 **`quant`**을 신설하여 스키마/테이블 소유는 독립. analytics DB/schema 직접 참조 금지. 테이블 네이밍은 prefix 없이 db 네임스페이스로 격리:
- `quant.market_tick_{exchange}` (분봉/틱 OHLCV)
- `quant.backtest_run` (백테스트 실행 메타 + 결과 집계)
- `quant.execution_result` (체결 시계열)
...
| Phase | 목표 | 포함 | 제외 |
|---|---|---|---|
| **Phase 1 — 백테스트** | 결정론 시뮬레이터로 파라미터 튜닝 | 빗썸 REST 히스토리 배치 수집(BTC/KRW·ETH/KRW 분봉 2023-01~현재) → ClickHouse `quant` DB 적재, `BacktestExchangeAdapter`, 골든셋 테스트, 기본 대시보드(실행 결과 표), 리더보드 read-model 스켈레톤 | WebSocket 구독, 실 주문, 텔레그램 알림 |
...
네이밍: `quant.{entity}.{event}.v1` (docs/architecture/kafka-convention.md 준수). 버전 suffix `v1` 고정, 스키마 breaking 변경 시 `v2` 새 토픽.

| Topic | Payload 요약 | 수신(예상) |
|---|---|---|
| `quant.strategy.activated.v1` | `{tenantId, strategyId, config, startedAt}` | analytics |
| `quant.strategy.liquidated.v1` | `{tenantId, strategyId, reason, triggeredBy}` | analytics, notification |
| `quant.slot.opened.v1` | `{tenantId, runId, slotId, roundIndex, price, qty}` | notification, analytics |
| `quant.slot.closed.v1` | `{tenantId, slotId, entryPrice, exitPrice, pnl}` | notification, analytics, leaderboard |
| `quant.order.placed.v1` | `{tenantId, orderId, exchange, side, qty, price}` | — |
| `quant.order.filled.v1` | `{tenantId, orderId, exchangeOrderId, filledQty, avgPrice, fee}` | leaderboard |
| `quant.order.failed.v1` | `{tenantId, orderId, reason, retryable}` | notification, DLQ 대상 |
| `quant.risk.limit_breached.v1` | `{tenantId, strategyId, limitType, value}` | notification |

DLQ는 `.DLT` 접미사(공용 규약). Consumer group: `quant-{purpose}` (예: `quant-leaderboard`, `quant-notification`).
...
- **메트릭** (Micrometer):
  - `quant.market.tick.received_total{exchange,symbol}`
  - `quant.strategy.evaluation.latency_seconds{mode}` (p50/p95/p99, NFR-PERF-01)
  - `quant.order.submit.success_total` / `..failure_total{reason}`
  - `quant.exchange.ratelimit.usage_ratio{exchange,tenant}` (80% 경보)
  - `quant.ws.connection.state{exchange}` (connected/degraded/fallback)
  - `quant.notification.send.latency_seconds{channel}`
- **트레이싱**: 각 전략 실행에 `executionId` 부여(NFR-OBS-03), OpenTelemetry span 전파(틱 수신 → 평가 → 주문).
- **대시보드 SLO**: 별도 Grafana 대시보드(운영), 제품 대시보드(사용자)와 분리.
- **대시보드 엔드포인트 SLO (NFR-PERF-04)**: `/dashboard/*`, `/leaderboard` p95 ≤ 500ms. `quant.dashboard.response.latency_seconds{endpoint}` 메트릭으로 감시.
...
| 모드 | Overlay | 비고 |
|---|---|---|
| 로컬/엣지 | `k8s/overlays/k3s-lite/quant/` | Redis standalone 전환(기존 5개 서비스 패턴 재사용), ClickHouse/Kafka 단일 인스턴스 공유 |
| 운영 | `k8s/overlays/prod-k8s/quant/` | HPA, PDB, TLS, NetworkPolicy |

- **이미지**: Jib 기반 tar (`./gradlew :quant:app:jibBuildTar`), `scripts/image-import.sh` 경로에 통합
- **HPA 기준**: CPU 70% 또는 `quant.strategy.evaluation.latency_seconds` p95 > 500ms 가 연속 3분 유지되면 스케일아웃. 최소 2 replica(가용성), 최대 10
...
**프론트엔드 배치**: `quant/frontend/` **독립 모듈 신설** (React + Vite). `charting` 서비스 재사용 대신 독립 유지 — 사유: 대시보드/리더보드가 **내부 관리용**이라 `charting`의 **공개 차트 렌더링**과 도메인 분리 필요.

**[결정 2026-04-24] FE 폼팩터: 모바일 웹앱 우선 (mobile-first PWA)** — 본인 1인 운용 시나리오상 외부에서 휴대폰으로 백테스트 결과·대시보드를 빠르게 확인하는 패턴이 주. 터치 친화 UI(하단 탭 내비, 큰 hit-target, 세로 레이아웃 우선), 데스크탑은 부수적으로 반응형 지원. PWA 매니페스트 + 오프라인 셸 + 홈 화면 추가 가능.
...
| 차트 협업 | `charting` (Python/FastAPI) | 공개 차트 엔드포인트. 본 서비스 내부 관리 FE는 별도 `quant/frontend/`로 분리 |
...
**프론트엔드**: `quant/frontend/` React + Vite 독립 모듈 신설 (위 §12 확정).
...
| OQ-002 | 암호화폐 변동성 기본값 (-3/+10 vs -5~-8/+5~+15) | TrancheStrategyConfig 기본값 |
...
### 15.1 분할 원칙 MVP 범위 선언

**분할매매 전략 분할 원칙 중 포트폴리오 차원 3개 (원칙 1·3·4)는 MVP 범위 밖으로 선언한다.**
...
MVP에서 강제되는 분할 원칙: **2(레버리지 금지), 5(-n% 추가매수), 6(회차 균등 금액), 7(손절 없음) + 회차 독립 익절**.
