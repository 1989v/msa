...
『분할매매』 분할 원칙을 빗썸/업비트 암호화폐 거래소에 적용하는 규칙 기반 자동매매 서비스의 **Phase 1 (백테스트 엔진)** 릴리즈. 실거래/페이퍼 트레이딩은 Phase 2/3 범위.
...
### 도메인 / 전략 엔진 (FR-ENG)
- ✅ **FR-ENG-01** — `TrancheStrategyConfig` (roundCount/entryGapPercent/takeProfitPercentPerRound/initialOrderAmount) 검증, INV-07 강제
...
### 성능 (NFR-PERF)
- ⏳ **NFR-PERF-01** — SLO 메트릭 노출 (`quant_strategy_evaluation_latency_seconds` 등록), 실측은 Phase 2 부하 시
...
- 텔레그램 알림 발송 (Phase 2)
- 업비트 어댑터 (Phase 3)
- 분할 원칙 1·3·4 (포트폴리오 차원 — `AccountPortfolio` 도메인 미도입, spec §15.1 명시)
...
- 중첩 서브모듈: `:quant:domain` (순수 Kotlin) / `:quant:app` (Spring)
...
- ClickHouse 24.3 (시세 시계열, `quant` DB 별도 신설)
...
### K8s
- Base manifest: `k8s/base/quant/`
- k3s-lite overlay 자동 포함
- Jib convention 자동 적용 (이미지 `commerce/quant`)
...
- quant/CLAUDE.md + docs/README.md
...
1. `TrancheSlot.targetQty` 가 placeholder `Quantity(BigDecimal.ONE)` — Phase 2 에서 동적 계산 (`initialOrderAmount / roundCount / firstBarPrice`) 도입
