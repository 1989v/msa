...
- **결정**: 기본값 채택
  ...
  - 코드 한도: **최대 50 거래쌍 × 50 회차**까지 설정 가능하도록 `TrancheStrategyConfig` 검증 한도 설계
...
| Kafka 컨벤션 | `docs/architecture/kafka-convention.md` | 토픽 네이밍(`quant.<event>.v1`), 스키마 버전링 |
...
| Entity 수정 | `docs/adr/ADR-0022-entity-mutation-conventions.md` | `TrancheSlot` 상태 전이 캡슐화 |
| K8s 배포 | `k8s/overlays/k3s-lite`, `k8s/overlays/prod-k8s` | `quant` Deployment/Service overlay 추가 |
| 시계열 저장소 | `analytics` 서비스의 ClickHouse 사용 패턴 | 백테스트 OHLCV, 틱 재생 (인프라만 공유, DB는 `quant` 분리) |
...
### 6.1 전략 엔진 (FR-ENG)
- **FR-ENG-01**: `TrancheStrategyConfig`는 `roundCount`(1~50), `entryGapPercent`(음수, 예 `-3.0`), `takeProfitPercentPerRound`(배열, 각 요소 양수), `initialOrderAmount`(모든 회차 동일), `targetSymbol`을 보유한다.
...
### 6.4 Phase별 모드 (FR-PH)
- **FR-PH-01 (Phase 1 백테스트)**: ClickHouse `quant` DB의 과거 분봉을 소스로 한 **결정론적 시뮬레이터**. 동일 입력 → 동일 출력 보장. 30일치 5분 내 재생.
...
- **NFR-PERF-04**: 대시보드/리더보드 엔드포인트(`/dashboard/*`, `/leaderboard`) 응답 **p95 ≤ 500ms**. `quant.dashboard.response.latency_seconds{endpoint}` 메트릭 감시.
...
- **A-01**: ClickHouse·Kafka·MySQL·K8s 등 기존 MSA 공통 인프라를 재사용한다. 단, ClickHouse는 **DB `quant` 신설**로 스키마 격리.
...
- **A-06**: 대시보드 FE는 **신규 독립 FE 모듈 `quant/frontend/` (React + Vite)로 제공**. `charting` 서비스 재사용 안 함 (내부 관리용 ↔ 공개 차트 도메인 분리).
...
- ADR-0024 Quant 서비스 도입 (본 스펙과 동반)
...
| ClickHouse 통합 (`quant` DB) | `testcontainers-clickhouse` | 1 | 백테스트 데이터 파이프라인 검증 |
