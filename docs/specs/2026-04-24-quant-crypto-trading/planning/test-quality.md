...
# Test & Quality Strategy — quant-crypto-trading
...
### 2.1 `:quant:domain` (순수 도메인)
- **Framework 의존 금지** (Spring, JPA, WebClient 등). Kotest + MockK만 사용.
- **대상**:
  - `TrancheStrategyConfig` 유효성 검증 (회차 1~50, 익절 배열 길이, `entryGapPercent < 0`)
  - `TrancheSlot` 상태 전이: `EMPTY → PENDING_BUY → FILLED → PENDING_SELL → CLOSED → EMPTY`
...
### 2.2 `:quant:app` (서비스·어댑터·Repository)
...
- **대상**:
  ...
  - `TrancheSlotJpaRepository` — H2/MariaDB Testcontainers
...
- 클래스명: `XxxSpec` (예: `QuantBuyTriggerSpec`).
- 상속: `BehaviorSpec({ ... })`.
...
- 픽스처는 `fixtures/` 하위 파일로 추출 (`TrancheFixtures.kt`).
...
### 4.3 실행·CI
- `./gradlew :quant:app:goldenTest` 커스텀 태스크 (태그 `golden`).
...
- PR 필수 체크: `:quant:domain:test`, `:quant:app:test`, 골든셋, 린트.
...
|---|---|---|
| `:quant:domain` | **≥ 90%** | ≥ 85% |
| `:quant:app` (executor, strategy service) | ≥ 80% | ≥ 70% |
