...
빗썸 BTC/KRW, ETH/KRW 분봉을 2023-01-01 ~ 현재까지 ClickHouse `quant.market_tick_bithumb`에 적재. 백테스트 엔진의 `HistoricalMarketDataSource` 입력으로 사용.
...
위치: `quant/app/src/main/resources/clickhouse/quant/V002__market_tick_bithumb.sql`

```sql
CREATE TABLE IF NOT EXISTS quant.market_tick_bithumb (
    symbol       String,
...
SELECT symbol, `interval`, 
       ...
       min(ts) AS minTs,
       max(ts) AS maxTs
FROM quant.market_tick_bithumb
