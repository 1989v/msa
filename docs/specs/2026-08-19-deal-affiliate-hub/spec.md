...
2. 쿠팡 파트너스 사이트 등록 심사 + 지정 고지 문구 확인
3. 여행 네트워크(트립닷컴·클룩·아고다·마이리얼트립) 파트너 가입

1 이 안 되면 배포해도 400/526 이 난다. 2·3 은 오퍼 데이터 입력의 선행 조건이지 배포 차단은 아니다.

**Origin 인증서는 건드리지 않는다.** `cf-origin-ca-tls` 는 `*.1989v.com` 와일드카드(2041-05 만료)라
새 서브도메인이 이미 커버된다 (ADR-0061 §5, game/resume/place 도 인증서 작업 없이 올라갔다).
DNS 를 붙였는데 526 이면 인증서가 아니라 **ingress 가 아직 클러스터에 반영되지 않은 것**이다 —
server block 이 없어 SNI 매칭에 실패하면 기본 인증서가 나가고 CF Full(strict) 가 거부한다.
