...
| 2 | `cf-origin-ca-tls` SAN 에 `deal.1989v.com` 추가 재발급 | 배포해도 526 |
| 3 | 쿠팡 파트너스 사이트 등록 심사 + 지정 고지 문구 | 고지 배너 문구 확정 불가 |
| 4 | 여행 파트너 가입 (트립닷컴·클룩·아고다·마이리얼트립) | 오퍼 데이터 없음 (배포는 가능) |

1·2 는 **배포 차단**, 3·4 는 데이터 입력의 선행이다.
