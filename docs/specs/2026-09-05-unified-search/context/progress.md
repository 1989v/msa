...
- **모델 선정 재검토** (2026-09-06, 사용자 지적) — arctic-ko 는 개인 계정 파인튜닝이고, Qwen 배제 근거가
  전 코퍼스 측정 없이 인코딩 속도뿐이었다. 후보 5종(arctic-ko · arctic-official · bge-m3 · qwen3-4b · qwen3-8b)을
  전 코퍼스에서 다시 잰다. 차원 프로브(1024 vs 4096)도 함께. 상세는 이어받기 문서 §3-1
- **첫 채움 전이라 되돌리는 값이 0** — 첫 채움 뒤에 바꾸면 전량 재임베딩이다
