...
**P0(측정·결정) 완료 → P1(구현) 착수 전.**

## 완료

- ADR-0090 작성(Proposed). 결정: 임베딩은 서버 밖, 질의는 사전, 파드 +0
...
1. ADR-0090 을 Accepted 로 승격(모델이 정해졌으므로)
2. P1-1 place 벡터 표 + 내부 API — Flyway 는 **V12**(운영 적용 V11 까지 확인, 2026-09-06)
3. P1-2~9 — 도구 나머지 · 재색인 · 사전 · hybrid 분기 · 테스트 · 첫 채움
4. P1 중에 **차원 512 vs 1024** nDCG 차이 측정(§8.11 이 남긴 유일한 미결)

## 블로커

- **푸시**: `gh` 활성 계정이 회사 것이라 pre-push 훅이 막는다. 커밋 12개 대기.
