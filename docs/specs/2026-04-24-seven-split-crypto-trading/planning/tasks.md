...
- [x] TG-05.0 **Complete**: 고정 시드·고정 fixture CSV 입력에 대해 결정론적으로 동일한 `StrategyRun` 결과(체결 리스트·실현 손익·종료 시각)를 산출한다.
  ...
  - [x] TG-05.9 **Verify**: `./gradlew :seven-split:app:test --tests '*BacktestEngineSpec*'` 성공, 실행 로그에 시드·입력 해시 기록
...
- [x] TG-06.0 **Complete**: `seven-split/app/src/main/resources/clickhouse/seven_split/` 아래 스키마 DDL이 존재하고 Testcontainers에서 create가 성공한다.
  - [x] TG-06.1 DDL 파일 `V001__create_database.sql` — `CREATE DATABASE IF NOT EXISTS seven_split`
  ...
  - [x] TG-06.5 `ClickHouseConfig` — 애플리케이션에서 사용할 JDBC/HTTP 클라이언트 설정, database = `seven_split` 고정
  ...
  - [x] TG-06.8 **Verify**: `./gradlew :seven-split:app:test --tests '*ClickHouseSchemaSmokeSpec*'` 성공
...
- [x] TG-07.0 **Complete**: `./gradlew :seven-split:app:bootRun --args='--spring.profiles.active=ingest-bithumb'` 또는 `BithumbHistoryIngestCommand` 수동 실행 시 대상 2종 × 2023-01~현재 분봉이 `seven_split.market_tick_bithumb`에 적재된다.
  ...
  - [x] TG-07.8 **Verify**: 단위 테스트 + 통합 스모크 통과 + 수동 실행 후 `SELECT min(ts), max(ts), count() FROM seven_split.market_tick_bithumb WHERE symbol='BTC_KRW'` 로 2023-01 ~ 현재 범위 확인
...
- [x] TG-09.0 **Complete**: UseCase 단위 테스트(MockK로 port 대체)가 모두 green이며, 통합 테스트에서 실 백테스트 경로가 DB/ClickHouse에 결과를 남긴다.
  - [x] TG-09.1 `CreateStrategyUseCase` — input DTO → `SplitStrategyConfig` 검증 → Repository 저장 → `StrategyCreated`(도메인 이벤트 필요 시) 또는 바로 API 응답
  ...
  - [x] TG-09.8 **Verify**: `./gradlew :seven-split:app:test --tests '*UseCaseSpec*'` 성공
...
- [x] TG-10. **Complete**: Phase 1 엔드포인트가 `ApiResponse<T>` 포맷으로 응답하고, MockMvc 슬라이스 테스트가 green.
  ...
  - [x] TG-10. `GlobalExceptionHandler` 확장 — `SplitStrategyConfigInvalidException`, `IllegalStrategyTransitionException`, `StopLossAttemptException`, `NotImplementedInPhase1Exception` 매핑
...
- [x] TG-11.0 **Complete**: `cd seven-split/frontend && npm install && npm run build` 성공 + `npm run test` (Vitest) green + 로컬 dev 서버에서 백엔드 API 연동 확인.
  - [x] TG-11.1 Vite + React + TypeScript + React Router + TanStack Query 스캐폴드, 프로젝트 루트 `seven-split/frontend/`
  ...
  - [x] TG-11.9 **Verify**: `cd seven-split/frontend && npm run build && npm run test` 성공
...
- [x] TG-13.0 **Complete**: `kubectl apply -k k8s/overlays/k3s-lite` 실행 시 seven-split Deployment/Service가 Ready 상태.
  - [x] TG-13.1 `k8s/overlays/k3s-lite/seven-split/kustomization.yaml` + `deployment.yaml` + `service.yaml` + `configmap.yaml` + (필요시) `secret.yaml` 템플릿
  ...
  - [x] TG-13.3 ConfigMap — MySQL/Kafka/ClickHouse 엔드포인트, `SPRING_PROFILES_ACTIVE=kubernetes`, `SEVEN_SPLIT_CLICKHOUSE_DATABASE=seven_split`
  - [x] TG-13.4 Secret stub — 거래소 API KEK/Telegram 토큰은 Phase 1 미사용이므로 빈 값, 구조만 준비
  - [x] TG-13.5 Jib 이미지 빌드 확인 — `./gradlew :seven-split:app:jibBuildTar` → tar 산출 → `scripts/image-import.sh --all` 에 포함
  - [x] TG-13.6 `image-import.sh` 가 seven-split 서비스를 인식하는지 확인(기존 스크립트 패턴 확장)
  - [x] TG-13.7 k3d 로컬 클러스터에서 apply → `kubectl -n seven-split get pods` → Ready. `/actuator/health` GET 200 확인
  - [x] TG-13.8 **Verify**: `kubectl apply -k k8s/overlays/k3s-lite` + `kubectl rollout status deployment/seven-split-app -n seven-split` 성공
