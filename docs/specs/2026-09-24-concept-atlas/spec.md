...
## revision 3 — 망라 (2026-09-24)

revision 2 는 운영 행을 놓는 데서 멈춰 원천 대비 절반 이상이 빠져 있었다(검색 개념 지도 14% · study 카탈로그 40% · 광고 0%).
도메인마다 망라 목록(체크리스트)을 먼저 쓰고 목록의 모든 항목을 놓았다.

| 항목 | 값 |
|---|---|
| 도메인 | 11 → 20 (새: JVM·런타임 · Spring · 메시징 · 클라우드 · 관측성 · 추천 · 상품·전시·재고 · 주문·결제·정산 · 광고) |
| 개념 · 관계 | 531 · 1022 → 4002 · 7989 |
| 체크리스트 | 20개, 원천 = study 카탈로그 19 · 개념 지도(검색 · 주문·결제·정산 · 광고 107) · 레포 스펙 · 분야 표준 목차 |
| 게이트 | `OntologyCoverageSpec` — 회귀 주입(가짜 placed · 체크리스트 삭제)에 빨간불 확인 |
| 병합 | 도메인 간 같은 개념 17개를 하나로(CTR · 가용성 · 팬아웃 · 순차 일관성 · 편집 거리 · 페이지 캐시 · 오프라인 지표 등) |

## 병풍 재설계 (2026-09-25)

사용자 지적 넷 — 모바일에서 누를 때마다 「펼칠지」를 되묻는다 · 3초 걸린다 · 관계의 흐름을 미리 못 본다 · 그래프가 구식이고 k-heritage 가 아니다.
타 서비스(Quanta 수학 지도 · roadmap.sh · NotebookLM 마인드맵 · Connected Papers · Neo4j Bloom · Obsidian 로컬 그래프 · mathgraph)를 본 뒤 시안 B-3 을 승인받아 바꿨다.

| 항목 | 내용 |
|---|---|
| 도메인 화면 | 층을 하나씩 펼치던 그래프 → **병풍** 아웃라인. 첫 층 묶음이 패널로 옆에 늘어서 도메인 전체가 보인다. 한 번 누르면 펼치고 고른다. 형제 흐름은 1 → 2 → 3 |
| 개념 판 | **관계 판**(들어오는 쪽 \| 나가는 쪽, 관계 이름별 칩, 깊이 1~2)이 기본, **그래프 판**(로컬 그래프, 깊이 2 기본)이 둘째. × · Esc 로 닫는다. 모바일은 아래 시트, 관계 판은 위아래 |
| 검색 | 도메인 화면에 개념 바로 가기(`/`) — 빌드에 실린 이름에서 찾는다 |
| 데이터 | 아틀라스 · 계층 · 관계 API 대신 **빌드에 실린 정적 그래프**. `AtlasGraphExportSpec` 이 부팅 로더와 같은 리더로 YAML 을 읽어 `portal-fe/src/pages/atlas/generated/` 에 내보내고, 다르면 테스트가 막는다. 구조 한 청크(gzip 약 96KB) + 도메인별 설명 청크. 해시 이름 `.js` 라 Cloudflare 가장자리에서 받는다 |
| 개념 화면 | `/tech/c/<id>` 는 그대로 API(코드 · 글 · 질문) |
| 주소 | `/tech/d/<domain>?sel=<id>` — 옛 `at=` 은 `sel` 로 읽는다 |

느린 원인은 둘이었다. 앱 쪽은 HTML → JS → 아틀라스 API → 계층 API 가 줄지어 오리진을 다녀오는 구조였고(이번에 API 둘을 없앴다),
망 쪽은 Cloudflare ↔ 오리진 구간에서 요청의 약 25% 가 10~60초 걸리거나 끊기는 현상이다(오리진 안은 5~10ms, 가장자리만 거친 응답은 정상). 망 쪽은 이 스펙 밖이다.
...
6. `AtlasGraphExportSpec` — 내보낸 정적 그래프가 YAML 과 같다(생성 파일 한 글자 변경에 빨간불 확인)
