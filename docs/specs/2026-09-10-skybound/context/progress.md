...
**T04A-1 물체 선택·운반·배치 순수 규칙 저장. 다음 T04A-2a 데모 선택/운반 연결.**
순수 조작7/7 직접 통과. `implementation/t04a/`, `verifications/t04a-1.md`.
등록 돌/프리즘만 1개 운반, 90도 회전, 현재 사거리·시선·겹침·쓸고가는 경로 검사.
취소/reset은 복구 재배치이므로 actor 겹침 안전 해결은 화면 연결 단계의 필수 과제다.
기존 데모에는 아직 물체 조작을 연결하지 않았다. 두 착지점 경로는 기존대로 유지한다.
