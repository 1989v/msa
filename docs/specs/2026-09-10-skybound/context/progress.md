...
**베타 운영 배포 완료. 사용자 요청에 따라 추가 게임 점검 없이 먼저 공개했다.**
접속: https://game.1989v.com/games/skybound/index.html
arena와 같은 games 저장소/portal-fe 서비스 경로. games `57aa280e`, 운영 msa 포인터 `09d33d96`, images run34749426589 success.
HTML/메타데이터200 확인(실행환경 curl CA 오류로 본문은 -k 확인; TLS/실기기 검증 아님). `release/deployment.md`.
게임 카탈로그 DB 등록은 아직 하지 않았다. 직접 실행 URL로 공개되어 있다.
다음: 배포된 베타를 유지하며 개발 재개. 작은 가로 화면/운반 경계/제목 줄바꿈 등은 후속 개선, 새 기능은 T05A 전투 규칙부터 소단위 진행 가능.
로컬 msa와 운영 main이 분기되어 있어 별도 /private/tmp/skybound-*-release 작업 공간으로 배포했다. 로컬 전체 브랜치를 push하거나 원격 포인터로 무조건 덮어쓰지 않는다.
사용자 검증 방침: 필수 부분만 확인하고 추가 플레이 점검은 모아서 한다. 배포 후 확인은 파일 반환만 수행했다.

**T04B-1 프리즘 받침 퍼즐 저장. 다음 T04B-2 첫 퍼즐 통합 플레이 점검 후 T05A 전투 규칙.**
시작점 오른쪽 (1.6,33)에 지면 표시 받침. 프리즘 전체 바닥이 받침 안이고 yaw90/270 및 바닥 높이 허용오차 .05m일 때 완료한다.
운반/무효 예고/일시정지는 새 완료를 막는다. 완료는 유지되고 프리즘 재집기는 잠기며 출발점 reset으로 초기화한다.
이번 관련 단위4/4 및 번들 빌드 통과. 실제 지형/벽/초기 actor 조건으로 두 view 초기화와 core 배치·완료/잠금/reset 확인. `verifications/t04b-1.md`.
이전 T04B-1 당시 브라우저 미실행. 현재 패키지로 퍼즐 PC/세로 모바일 완주 확인, 벽 sweep·취소 actor 겹침·작은 가로 화면은 남음.
브라우저 목표 힌트: spawn에서 yaw≈-.675,pitch≈.389로 받침 중심, 프리즘yaw90. 실제 키/터치로 해법 확인은 아직 남음.
이전 지지 보완 관련10/10은 이전 증거. 바닥 9개 표본 근사(높이 차이 최대0.2m)이며 표본 사이 구멍/전체 운반 경로의 지형 충돌은 미보장.
보상/게임 저장/다른 유적은 미연결. 전체 게임 완료 아님.
전체 구축 추정 **약 35%**: [산정 근거](completion-estimate.md).
전환/카메라/터치 단위10/10, 브라우저 터치15/15·PC11/11·이동19/19. `verifications/t02c-2.md`.
사용자 최신 선호: 설명·데모 안내를 최소화한다. 토큰 절약을 위해 변경에 직접 필요한 필수 검사만 수행하고 전체 회귀/폭넓은 브라우저 QA는 모아서 진행하거나 후속 플레이 테스트로 확인한다. 같은 코드의 통과 검사를 반복하지 않는다.
...
3. **T04A-2a**: `implementation/t04a/README.md`, `planning/t04a-small-steps.md`를 읽고 데모 프리즘과 선택/운반 표시를 연결한다.
   순수 규칙은 완료했다. 안전한 취소/reset, 현재 player/wall AABB, 지형 받침과 PC/터치 입력은 연결 검증이 필요하다.
