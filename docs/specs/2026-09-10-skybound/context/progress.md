...
**T03B-1 초원 상승기류 하나 연결 저장. 다음 T03B-2 공중 착지 경로.**
단위16/16, 기류 브라우저11/11·활공/터치회귀19/19 통과. `verifications/t03b-1.md`.
기류(0,30), 반경3m, 상승4m/s. 지면 원 안에서 점프 후 돛을 펼치면 상승·기력회복한다.
표시·물리 공통 정의, pause/reset은 시뮬레이션 시간 기준. 기류 표시는 기술 초안이다.
터치 이전 실패는 이번 재실행에서 미재현, 실기기 안정성/원인 해결로 확대하지 않는다.
...
3. **T03B-2**: `verifications/t03b-1.md`, traversal README와 PRD SR-2를 읽고 중간 착지점·공중 경로를 구현한다.
   기본 기력 완주와 실패 복귀를 검증한다. 터치 실기기 타이밍과 원거리 기류 가독성은 후속.
