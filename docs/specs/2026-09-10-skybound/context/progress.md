...
**우선순위 변경: 전투보다 웹 베타 공개 준비를 먼저 진행한다.**
사용자 요청은 충분히 개발되면 웹 베타로 공개할 수 있도록 준비하는 것. 첫 베타는 탐색/활공 경로/첫 퍼즐이며 전체 PRD 완성을 기다리지 않는다.
`release/build-beta.mjs`로 독립 정적 `release/dist/` 생성 완료(약2.9MB). 로컬 에셋/해시 검사 통과, 공개 URL은 아직 없다.
베타 패키지 검사: PC/390px 터치 퍼즐10/10, 고정 스텝 비행14/14 통과. 모바일 취소 버튼42.234→44px 수정. `verifications/beta-play.md`.
다음: 남은 운반 벽 경계/취소 겹침·작은 가로 화면 점검 → 기존 공개 경로와 호스팅 방식 확인 → 산출물 확정/배포. 세로 베타 제목 줄바꿈도 정리한다.
빌드·로컬 실행: `release/README.md`, 공개 조건: `release/beta-readiness.md`, 패키지 증거: `verifications/beta-package.md`.
