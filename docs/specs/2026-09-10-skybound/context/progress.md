...
**T04A-2b 지면 지지 보완 저장. 다음 T04B-1 프리즘·받침 하나의 완료 규칙과 데모 연결.**
회전된 바닥 3×3 표본의 지면 유무/높이 차이(최대 0.2m)를 검사하고 최고 표본에 바닥을 맞춘다.
집기·이동·회전·놓기 모두 같은 순수 배치 resolver를 거친다. 무효 회전은 예고만 바뀌고 실제 held/committed는 유지한다.
이번 관련 단위10/10 및 번들 빌드 통과. 실제 지형의 초기 프리즘 지지도 Node에서 확인했다. `verifications/t04a-2b.md`.
사용자 비용 방침에 따라 이번 브라우저/전체 회귀는 미실행. 기존 T04A-2a 브라우저11/11은 이전 코드 증거다.
후속 통합 플레이 검사: 벽 sweep/취소 actor 겹침/작은 모바일 화면/지면 경계 조작. 9개 표본 사이 작은 구멍은 보장하지 않는다.
장애물 1개의 AABB만 운반 충돌에 연결했다. 다른 장식물/캐릭터 이동 충돌은 미연결.
