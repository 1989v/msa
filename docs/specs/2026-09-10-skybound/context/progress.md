...
**T03B-2b 두 착지점 경로·완료 안내 저장. 다음 T04A-1 물체 조작 규칙.**
단위48/48, 실제 기류→첫발판→둘째발판 브라우저14/14 통과. `verifications/t03b-2b.md`.
첫(8,30)/둘째(14,30,첫보다2m낮음), 순서대로 착지해야 완료. 물 리스폰 보존·전체reset초기화.
기본 기력으로 경로 통과 확인. 최종 아트·바람음향·실기기 안정성은 미완료다.
...
3. **T04A-1**: PRD SR-3와 tasks.md를 읽고 선택/집기/회전/놓기·거리 제한의 순수 상태 규칙부터 구현한다.
   두 착지점 이동 경로는 완료했으므로 재작성하지 않는다. 저장/퀘스트 본편은 후속이다.
