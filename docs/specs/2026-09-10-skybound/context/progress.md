...
**T02C-1 PC 회전·줌·카메라 기준 이동 저장. 다음 T02C-2 터치 입력.**
전체 구축 추정 **약 30%**: [산정 근거](completion-estimate.md).
카메라/전환 단위검사6/6, 브라우저 시야11/11·이동회귀19/19. `verifications/t02c-1.md`.
...
3. **T02C-2**: traversal/README.md를 읽고 터치 동시 이동·시야 입력을 작은 단위로 구현한다.
   포커스/메뉴/pointercancel 입력 해제와 카메라 시야 조작을 검증한다. PC 우클릭 회전·휠 줌은 완료했으며 장애물 충돌은 없다.
