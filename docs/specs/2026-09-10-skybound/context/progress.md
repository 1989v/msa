...
**T04A-2a 데모 프리즘 조작 저장. 다음 T04A-2b 배치 지지/장애물 경계 검증.**
프리즘 1개, E 집기/놓기·R 90도 회전·터치 공통 버튼·무효 배치 표시 연결.
운반 중 이동/점프 잠금, pause 동결, 취소는 원위치 actor 겹침 시 거부, reset은 출발점 복구.
순수/어댑터 단위29/29, 실제 브라우저11/11. `verifications/t04a-2a.md`.
장애물 1개의 실제 AABB만 운반 충돌에 연결했다. 다른 장식물/캐릭터 이동 충돌은 미연결.
다음: footprint 지면 지지(현재 중심점만 사용), 실제 벽 sweep/취소 겹침/모바일 작은 화면 QA를 보강한 뒤 T04B 받침 퍼즐.
