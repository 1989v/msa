...
회색 장애물 하나는 운반 경로를 막는다. 캐릭터의 장애물 충돌 및 다른 장식물 충돌은 아직 없다. 지면 지지는 회전된 바닥 9개 표본 기준이며 지면 누락/높이 차이 0.2m 초과를 거부한다. 최고 표본에 바닥을 맞추므로 약한 경사에서는 작은 틈이 남을 수 있고, 표본 사이 작은 구멍은 검사하지 않는다. 퍼즐 완료와 게임 저장은 미연결.
