...
시작 후 중앙 조준점으로 앞의 프리즘을 겨누고 E로 집기/놓기, R로 90도 회전한다. 화면 버튼은 터치도 지원한다. 운반 중 이동·점프는 잠기며 시야로 배치 위치를 조정한다. 취소는 마지막 배치로 복귀하되 플레이어 겹침 시 거부한다. 출발점으로 버튼은 프리즘도 초기화한다.

회색 장애물 하나는 운반 경로를 막는다. 캐릭터의 장애물 충돌 및 다른 장식물 충돌은 아직 없다. 지면 지지는 회전된 바닥 9개 표본 기준이며 지면 누락/높이 차이 0.2m 초과를 거부한다. 최고 표본에 바닥을 맞추므로 약한 경사에서는 작은 틈이 남을 수 있고, 표본 사이 작은 구멍은 검사하지 않는다. 시작점 오른쪽 받침에 프리즘을 90도 돌려 전체가 들어가게 놓으면 완료되어 고정된다. 출발점으로 버튼으로 재도전한다. 게임 저장은 미연결.
