...
- 주문서 검증: 만료 · 이미 주문에 쓰임 · 소유자 불일치는 422. 주문서 1개 = 주문 1개. 같은 상품이 요청에 여러 번 오면 처음 나온 자리에서 수량을 합쳐 한 라인으로 만든다.
...
- 이행이 생긴 뒤의 취소는 `fulfillment.command.cancel`(라인 지정 — 주문 라인 id 로 식별)을 먼저 보낸다. 이행 도메인이 출고 전이면 해당 라인을 취소하고 `fulfillment.order.cancelled`, 이미 출고면 `fulfillment.order.cancel-rejected` 로 답한다. 환불은 cancelled 뒤에만.
...
- 정산 대상은 구매 확정 라인과 그 판매자의 배송비 라인뿐이다. 배송비 라인은 그 주문에서 **해당 판매자의 첫 라인이 구매 확정되는 순간** 같은 `order.line.purchase-confirmed` 이벤트에 실려 같은 기간 정산서에 들어간다. 배송비 환불은 그 판매자 라인이 **전부** 출고 전 취소될 때뿐이라, 확정 라인이 하나라도 생긴 판매자의 배송비는 환불되지 않는다 — 그래서 첫 확정 시점에 정산 대상으로 올려도 뒤집히지 않고, 라인 없이 배송비만 싣는 이벤트가 생기지 않는다(모든 라인이 취소돼 배송비가 환불되면 정산 대상 아님). 환불은 확정 전 라인에서만 일어나 정산서에 들어오지 않는다(이중 차감 방지).
...
- 정산 배치(일 1회): 판매자 주기가 닫힌 기간의 `order.line.purchase-confirmed` 를 모아 정산서(순매출 · 배송비 · 수수료 · 지급액)를 만든다. **지급액 = Σ라인 순매출 + Σ배송비 − Σ수수료**. 0 이하면 CARRIED_OVER. 모의 송금 후 원장에 지급 거래. 정산서 지급액 = 판매자 미지급금 잔액 감소분.
  - 정산서는 명목 기간과 함께 **실제 포함 범위**(담긴 항목의 최소~최대 확정 시각)를 보인다 — 기간 끝 전 확정분을 모두 담으므로 이월·지각 항목이 기간 시작보다 앞설 수 있다.
  - 플랫폼 판매자(1)는 자기 매출이라 지급하지 않는다: 정산서는 감사용으로 만들고 항목을 묶되 상태는 `PLATFORM_RETAINED`, 송금·지급 거래 없음. 원장의 판매자 1 미지급금 잔액은 플랫폼 자기 매출 누계로 남는다.
- 정정: 어드민 `POST /api/v1/admin/settlements/ledger/journals/{id}/reverse {reason}` 가 역분개 거래를 만든다(원천 키 `reversal:journal:{id}` — 거래당 하나, 다시 요청하면 기존 역분개를 돌려준다). 행위자·사유를 거래에 남긴다. 역분개 거래는 다시 역분개하지 않는다.
- PG 입금 뒤 환불이 오면 PG 미수금 잔액이 음수가 될 수 있다 — 「PG 에 돌려받을 돈」(다음 입금에서 상계)으로 수용한다.

### SR-10 운영 보강
- 아웃박스(common, 이것을 쓰는 스키마 전부 — order · inventory · fulfillment · product · seller · payment · promotion. settlement 는 발행하는 토픽이 없어(받기만 한다) 아웃박스가 없다. quant 는 자체 릴레이라 대상 아님): 한 트랜잭션에서 최대 100행을 `FOR UPDATE SKIP LOCKED` 로 집어 `SENDING` + 리스 30초로 바꾸고 커밋 → 트랜잭션 밖에서 동기 전송(타임아웃 10초) → 새 트랜잭션에서 PUBLISHED 또는 시도 수·다음 시도 시각 갱신, 10회 초과 FAILED. 리스 만료 SENDING 은 다시 집는다. PUBLISHED 7일 뒤 삭제. 적체 게이지. 스키마 변경은 확장만(nullable 컬럼 추가)이라 옛 코드와 공존한다.
