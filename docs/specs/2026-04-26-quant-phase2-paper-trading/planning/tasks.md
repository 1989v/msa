...
- [ ] TG-P2-03.0 **Complete**: 단위 테스트로 wrap/unwrap round-trip 검증 + Adapter 3종이 동일 Port 시그니처로 교환 가능.
  - [ ] TG-P2-03.1 `quant/app/src/main/kotlin/com/kgd/quant/application/port/security/KeyManagementService.kt` Port 선언:
    ```kotlin
  ...
  - [ ] TG-P2-03.2 `infrastructure/security/kms/OciVaultKmsAdapter.kt` (운영, `@Profile("oci") @ConditionalOnProperty`):
  ...
  - [ ] TG-P2-03.3 `infrastructure/security/kms/LocalFileKmsAdapter.kt` (로컬 dev, `@Profile("local") @ConditionalOnProperty`):
    - `SEVEN_SPLIT_LOCAL_KEK` 환경변수 (hex 64 chars = 256 bits) 또는 `application-local.yml` 로드
    - AES-GCM-256 직접 wrap/unwrap (no external call)
    - 회전: `kek-v1` → `kek-v2` 환경변수 swap (TG-P2-04 lazy re-encryption hook이 자동 처리)
  - [ ] TG-P2-03.4 `infrastructure/security/kms/FakeKmsAdapter.kt` (테스트 전용 — `:quant:app/src/test/kotlin/...`):
    - 메모리 in-process AES-GCM
    - 테스트 시드 주입 가능 (deterministic)
  - [ ] TG-P2-03.5 DEK 캐시 (`Caffeine`) — TTL 30분, stale-on-error 정책 (KMS 일시 장애 시 만료 직전 entry로 degrade)
    - `KmsDekCache` 컴포넌트, hit/miss/stale 메트릭 (`quant.kek.cache.{hits,misses,stale}_total`)
  - [ ] TG-P2-03.6 단위 테스트 (Kotest BehaviorSpec):
  ...
  - [ ] TG-P2-03.7 application.yml 프로파일 분기 — `quant.security.kms.provider=oci|local|fake`
  - [ ] TG-P2-03.8 **Verify**: `./gradlew :quant:app:test --tests '*Kms*'` 성공, `OciVaultKmsAdapterIntegrationSpec`은 `-Pinclude-oci-integration=true` 일 때만 실행
...
- [ ] TG-P2-04.0 **Complete**: kek-v1 → kek-v2 회전 시뮬 후 백그라운드 잡이 모든 row를 신규 KEK로 재암호화하고 `kek_version` 컬럼이 갱신됨 (downtime 0).
  - [ ] TG-P2-04.1 Flyway `quant/app/src/main/resources/db/migration/V002__envelope_encryption_kek_version.sql`:
  ...
  - [ ] TG-P2-04.2 `infrastructure/security/EnvelopeCryptoCodec.kt` — KMS Port + AES-GCM DEK 결합. `encrypt(plaintext): Pair<ciphertext, kekVersion>`, `decrypt(ciphertext, kekVersion): plaintext`
  - [ ] TG-P2-04.3 Phase 1 `ExchangeCredential` Aggregate / `NotificationTarget` VO 의 byte 필드 read 경로에 `EnvelopeCryptoCodec.decrypt()` hook 추가 (도메인 모델 변경 최소화 — Adapter 계층에서 변환)
  - [ ] TG-P2-04.4 `LazyReencryptionJob` (Spring `@Scheduled` 또는 별도 `ApplicationRunner`):
  ...
  - [ ] TG-P2-04.5 통합 테스트 (Testcontainers MySQL + FakeKmsAdapter):
  ...
  - [ ] TG-P2-04.6 SOP 문서 `quant/docs/key-rotation-sop.md` — LocalFile 환경 수동 회전 절차 + OCI Vault 자동 회전 확인 절차 + lazy re-encryption 진행률 모니터링 쿼리
  - [ ] TG-P2-04.7 **Verify**: `./gradlew :quant:app:test --tests '*EnvelopeEncryption*' --tests '*LazyReencryption*'` 성공 (Testcontainers 활성)
...
- [ ] TG-P2-10.0 **Complete**: MockWebServer 기반 우선순위 정렬 + per-chat rate limit + 5xx 재시도 backoff + token masking 모두 green.
  - [ ] TG-P2-10.1 `application/notification/port/NotificationPriorityQueue.kt`:
    ```kotlin
    ...
    ```
  - [ ] TG-P2-10.2 `infrastructure/notification/InMemoryPriorityQueue.kt`:
  ...
  - [ ] TG-P2-10.3 `infrastructure/notification/TelegramBotNotificationSender.kt`:
  ...
  - [ ] TG-P2-10.7 단위 테스트 (MockWebServer):
  ...
  - [ ] TG-P2-10.8 **Verify**: `./gradlew :quant:app:test --tests '*TelegramBot*' --tests '*NotificationPriorityQueue*'` 성공
...
- [ ] TG-P2-11.0 **Complete**: CB OPEN/half-open/CLOSED 전이 + 분산 토큰 버킷 multi-instance 검증 + DLQ 3회 재시도 후 격리 모두 green.
  - [ ] TG-P2-11.1 `infrastructure/resilience/CircuitBreakerConfiguration.kt`:
  ...
  - [ ] TG-P2-11.3 `infrastructure/resilience/RedisTokenBucketRateLimiter.kt`:
  ...
  - [ ] TG-P2-11.4 Lua script 파일 `quant/app/src/main/resources/lua/token_bucket.lua`:
  ...
  - [ ] TG-P2-11.5 80% 도달 시 RISK 알림 enqueue (TG-P2-10), 95% 도달 시 자가 백오프
    - 메트릭 `quant.exchange.ratelimit.usage_ratio{exchange,tenant}` (Phase 1 정의 → 실 측정 활성화)
  - [ ] TG-P2-11.6 Kafka DLQ 설정:
  ...
  - [ ] TG-P2-11.8 **Verify**: `./gradlew :quant:app:test --tests '*CircuitBreaker*' --tests '*TokenBucket*' --tests '*KafkaConsumerDlq*'` 성공 (Testcontainers 활성)
...
- [ ] TG-P2-12.0 **Complete**: 미발행 row polling → 실 Kafka publish → published_at 마킹 + 멱등 consumer 중복 방어 검증 green.
  - [ ] TG-P2-12.1 `infrastructure/outbox/EventPayloadCodec.kt` — 이벤트 type별 JSON serialize/deserialize registry:
  ...
  - [ ] TG-P2-12.2 Phase 1 `OutboxRelay.kt` 리팩토링:
  ...
  - [ ] TG-P2-12.3 Flyway `V004__outbox_failure_count.sql`:
  ...
  - [ ] TG-P2-12.6 통합 테스트 (Testcontainers MySQL + Kafka):
  ...
  - [ ] TG-P2-12.7 **Verify**: `./gradlew :quant:app:test --tests '*Outbox*' --tests '*IdempotentConsumer*'` 성공 (Testcontainers 활성)
...
- [ ] TG-P2-15.0 **Complete**: `kubectl apply -k k8s/overlays/k3s-lite` + quant Pod Ready + `/actuator/health` UP + SSE 라우팅 확인.
  - [ ] TG-P2-15.1 k3s-lite overlay (`k8s/overlays/k3s-lite/quant/`):
    ...
    - Secret: `SEVEN_SPLIT_LOCAL_KEK` (hex 64 chars, dev시 `.env` 기반 — `application-local.yml`은 `.gitignore`)
  ...
  - [ ] TG-P2-15.2 prod-k8s overlay (`k8s/overlays/prod-k8s/quant/`):
  ...
  - [ ] TG-P2-15.3 Jib 이미지 빌드 — `./gradlew :quant:app:jibBuildTar` → tar 산출 → `scripts/image-import.sh --all` 에 포함
  - [ ] TG-P2-15.4 Kafka 토픽 사전 생성 (k3s-lite/prod-k8s 공통):
