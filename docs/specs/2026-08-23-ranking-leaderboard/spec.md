...
| 오피넷 전국 주유소 일 1회 수집 | 실시간 가격 조회 |
| 시군구 × 유종 최저가 보드 자동 생성 | 유료 오피넷 API |
...
기본은 **`주유소 기본정보(지역별)` 순회** — (시도 × 유종) 조합이라 호출 수가 두 자릿수로 끝난다.
지역별로 전량이 안 나오면 **반경검색 격자 순회**로 내린다(OQ-4, 키 발급 후 시도 1개로 판정).

한도 초과·429 를 만나면 **그 실행을 즉시 멈춘다.** 부분 적재를 남기면 다음 전체 동기화가
나머지를 지운다.

### 5.2 좌표 변환

응답의 `GIS_X_COOR`/`GIS_Y_COOR` 는 **KATEC(TM128)** 이다. `pyproj` 로 WGS84 로 변환해
`latitude`/`longitude` 에 넣고, **KATEC 원본도 `katec_x`/`katec_y` 에 남긴다.**
