...
- 저장 불변식: 입찰가 ≥ 타기팅한 모든 지면의 최저가, 일예산 ≥ 1회 과금액. 저장 뒤 최저가가 오른 지면은 결정에서 제외(SR-6)
...
- 후보 자격: 광고주 `ACTIVE` · 캠페인 `ACTIVE` · 기간 안 · 소재 `APPROVED` · 소재 비율과 지면 형식 일치 · 입찰가 ≥ 그 지면 최저가 · 카테고리 일치 · 일예산·총예산 여유(청구 누계 + 미정산 지출 기준) · 캠페인 시간당 지출 상한(일예산의 25%) 미달 · 지갑 여유 · 방문자 빈도 미달
...
- 퍼블리셔: 지면별 일별 요청·**유료 채움률**(서버가 아는 값)·최종 채움 출처 분포(FE 보고, 참고치로 표기)·노출·클릭·RPM·수익(퍼블리셔 몫)
- 원천은 시간별 집계 두 표와 원장뿐
...
- 에셋 응답: 저장 형식 Content-Type · `X-Content-Type-Options: nosniff` · 불변 캐시
