...
### 스핀 한 번의 비용 (2026-09-05)

폰 크기 화면 · CPU 4배 감속 · 판마다 3회 측정 중앙값 (`Tracing`, 스핀 3.2초):

| | 전 | 후 |
|---|---|---|
| 장치의 프레임 작업(rAF) | 114ms | **67ms** |
| 스타일 계산 | 122ms | 128ms (차이 없음) |
| **레이어화** | 760ms | 764ms (차이 없음) |
| 메인 스레드 전체 | 1,477ms | 1,446ms |
| 33ms 넘는 프레임 | 0 | 0 |

- **고친 것**: 매 프레임 40장의 transform 문자열을 새로 만들어 넣고 클래스·aria 를 전부 건드리던 것을,
  카드마다 마지막 값과 클래스 비트를 들고 **달라질 때만** DOM 을 건드리게 했다. 정면 칸은 각으로 바로 구하고
  (40장 훑기 제거), transform 의 고정 부분은 한 번만 만든다.
- **안 줄어드는 것**: 스핀 비용의 절반 이상이 **레이어화**다. 판 위 요소 수에 비례하고 이 코드가 무엇을 하든
  변하지 않는다 — 카드를 40장에서 16장으로 줄이면 760 → 470ms, 옆면 요소를 다 빼도 760 → 467ms 다.
  둘 다 판의 생김새를 바꾸므로 하지 않았다. 측정 표본 하나로는 ±17% 흔들려서 중앙값으로 읽어야 한다.
- **버린 안**: 판이 도는 동안 far 판정을 얼리면 10% 더 줄지만, 지나가는 카드가 보이지 않는 면으로 굳어
  **판에 구멍이 생긴다**(스핀 중 스크린샷으로 확인). 판정은 매 프레임 유지한다.
