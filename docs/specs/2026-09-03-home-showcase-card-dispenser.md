...
| 카드 수 | 전부 (관광 80·게임 전부) | **40장 상한** (`useShownItems`) — 캡션도 꽂은 수를 말한다 |
| 먼 카드 | 요소 5개 (앞·뒤·옆 셋) | **라이트 모드** — 정면 ±7칸 밖은 보이는 면 하나 + 옆면 하나 |

### 모바일 성능 (2026-09-03 추가)

배포 뒤에도 폰에서 스크롤이 버벅였다. 원인은 그림 크기가 아니라 **컴포지터 레이어 수**였다 — 카드 한 장이
3D 변환 요소 다섯 개 = 레이어 다섯 개라, 80장짜리 판 하나가 400 레이어를 만들고 화면에 들어올 때마다 그걸
한꺼번에 만든다. 모바일 에뮬레이션(390×844, DPR 3) CDP `LayerTree` 실측:

| 조치 | 레이어 | 레이어 메모리 추정 |
|---|---|---|
| 배포 상태 (80장·요소 5개·페이지 고정 먹 캔버스) | 530 | 418MB |
| 40장 상한 + 라이트 모드(먼 카드 요소 3개) | 232 | 365MB |
| + 먹 캔버스를 페이지 고정에서 **히어로 안**으로 | 228 | 246MB |
| + 먼 카드 **보이는 면 하나만** (요소 2개) | **204** | **236MB** |

- **`position: fixed; z-index: -1` 캔버스가 페이지 전체를 승격시켰다.** 고정 캔버스는 항상 컴포지터 레이어이고,
  그 뒤에 그려지는 콘텐츠는 전부 "겹침(Overlap)" 사유로 별도 레이어에 들어간다 — 문서 높이(8,162px)만 한 레이어가
  하나 더 생겨 타일이 두 벌이었다. 터치 기기는 포인터 낙묵이 없으니 먹 캔버스를 히어로 안에만 둔다(소프트웨어 캔버스).
- 먼 카드의 보이는 면은 각도가 정한다: 앞면 법선이 (cos a, 0, −sin a) 라 **왼쪽 호(a > 180°)는 앞면, 오른쪽 호는 뒷면**.
  눈 거리·기울기·원근 원점을 넣어도 부호는 sin a 뿐이다(내적이 −d·sin a·cos φ 로 정리된다).
- 시스템 코어 캔버스는 터치에서 정지 화면으로 시작하고 탭해야 12초 돈다.
- 메인 스레드(CPU 4배 느리게, 스크롤 제스처 5.5초): 컴포지터 커밋 241ms → 167ms.

### 뽑히는 과정 (2026-09-05)

| | 전 | 후 |
|---|---|---|
| 스핀 중 올라온 카드 (중앙값) | 1장 | **4장** |
| 판에 처음 붙는 그림 | 18장 | **10장** |
| 스핀 한 번 뒤 그림 | 49장 | **15장** |

- **peek 은 한 장이 아니라 무리에 걸린다** (`peekSpread` 2.4칸). 한 장만 올리면 스핀이 한 칸을 수십 ms 에 지나가
  눈에 안 보인다 — 코드에는 있는데 "적용이 안 됐다"고 보이던 이유가 이것이었다. 멈추면(reveal 1) 다시 정면 한 장만 일어난다.
- **그림은 `data-src` 로 두고 장치가 붙인다** (`photoSteps` 2칸, 판이 멈춘 뒤에만). 판이 돌면 모든 카드가 정면을 지나므로
  얼굴에 주소를 박으면 스핀 한 번에 전부 받는다. 대신 지나간 카드는 그림 없는 얼굴로 남는다 — 옆으로 선 카드라 거의 안 보인다.

### 내려받는 양 (2026-09-03 · 2026-09-04 실측 갱신)

**결과 — 모바일(390×844, 캐시 없음) 첫 로드**

| 화면 | 전 | 후 | 이미지 |
|---|---|---|---|
| 홈 (`1989v.com`) | 4,705KB | **1,819KB** | 2,614KB → 555KB |
| 관광 (`place.1989v.com`) | 13,938KB | **5,050KB** | 11,390KB → 2,479KB |

**브라우저는 큰 이미지를 작게 그릴 수는 있어도 작게 받을 수는 없다.** CSS 크기·`background-size`·캔버스
`drawImage` 는 전부 **바이트가 다 도착한 뒤** 줄이는 일이라 화면 메모리만 줄고 전송량은 그대로다.
전송을 줄이는 길은 셋뿐이다: ① 원천이 이미 주는 **작은 판**을 대신 요청한다 ② 리사이즈 프록시를 앞에 둔다
(Cloudflare Images 는 유료, 직접 돌리면 우리 CPU·egress — 무료 티어 제약과 충돌) ③ **안 보이는 것은 요청하지 않는다**.
여기서는 ①이 공짜였다 — TourAPI 가 원본(940×627·130~550KB) 옆에 `firstimage2`(300×200·약 14KB)를 함께 준다.

**남은 몫과 한계**

- 원천의 4%(52,768건 중 2,137건)는 `firstimage2` 가 원본과 **같은 URL** 이라 줄일 판이 없다. 그 행은 원본이 나간다.
- 게임 카드 그림은 320×180 PNG 인데 장당 77~104KB 다(사진에 PNG). 홈 이미지 555KB 의 대부분이 이것 —
  WebP 로 구우면 크게 줄지만 주소가 game DB 에 있어 데이터까지 함께 고쳐야 한다. 후속.
- `gtag.js` 502KB 는 페이지를 가리지 않는 측정 도구라 그대로 뒀다. 관광 화면의 구글맵(약 1.3MB)도 그 화면의 기능이다.

| 항목 | 전 | 후 | 방법 |
|---|---|---|---|
| 관광 카드 사진 | 원본 940×627 · 130~550KB | 썸네일 300×200 · 약 14KB | `thumbnailUrl`(TourAPI firstimage2)을 검색 색인에 싣고 **카드 얼굴과 목록 카드(88px 정사각)**가 쓴다. 폭을 다 쓰는 상세 사진은 원본 |
