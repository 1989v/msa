...
class IndexService(
...
) : ManageConceptIndexUseCase {
    override fun create(command: CreateIndexCommand): IndexResultDto {
    ...
    override fun findByConceptId(conceptId: String, pageable: Pageable): Page<IndexResultDto> =
        indexRepository.findByConceptId(conceptId, pageable).map { toResultDto(it) }

    override fun count(): Long = indexRepository.count()
