...
class RetentionRunnerSpec : BehaviorSpec({
    ...
    Given("두 원장이 모두 정상일 때") {
        val blogViewService = mockk<BlogViewService>()
        val resumeAccessLog = mockk<ResumeAccessLogRepositoryPort>()
        every { blogViewService.purgeOlderThan(any()) } returns 12
        every { resumeAccessLog.purgeOlderThan(any()) } returns 3

        When("배치를 실행하면") {
            RetentionRunner(blogViewService, resumeAccessLog).run(DefaultApplicationArguments())

            Then("각 원장이 자기 보존기간으로 정리된다") {
                verify(exactly = 1) { blogViewService.purgeOlderThan(BlogViewService.RETENTION_DAYS) }
                verify(exactly = 1) {
    ...
    Given("블로그 원장 정리가 실패할 때") {
        val blogViewService = mockk<BlogViewService>()
        val resumeAccessLog = mockk<ResumeAccessLogRepositoryPort>()
        every { blogViewService.purgeOlderThan(any()) } throws IllegalStateException("DB 연결 끊김")
    ...
    Given("이력서 원장 정리가 실패할 때") {
        val blogViewService = mockk<BlogViewService>()
        val resumeAccessLog = mockk<ResumeAccessLogRepositoryPort>()
        every { blogViewService.purgeOlderThan(any()) } returns 7
