 ...
 * - 유료를 받지 않는 지면, 지금 최저가보다 낮은 입찰(저장 뒤 최저가가 오른 경우), 비율이 맞지 않는 소재는 그 지면에서 빠진다
        ...
        return placements.map { placement ->
            val winner = if (placement.paidAllowed) {
                candidatesByPlacement[placement.key].orEmpty()
                    .filter { it.campaignId !in wonCampaigns && it.bid.micros >= placement.floorMicros && placement.accepts(it.aspectRatio) }
