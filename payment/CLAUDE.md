...
./gradlew :commerce:app:test --tests '*PaymentPgSelection*'      # 기본 = 모의 PG(토스 빈 없음·웹훅 404) · toss 키 없으면 기동 실패
```
...
- **PG 흐름**: 모의 PG 는 서버가 승인(`authorize`), 토스는 FE 결제창 인증 뒤 `paymentKey` 로 승인 확인(`confirm`). 명령에
  `paymentKey` 가 있으면 토스 흐름이다. 토스 카드는 승인 확인에서 자동 매입 — 토스 어댑터의 매입은 HTTP 를 부르지 않는다.
...
| `toss` | `TossPgAdapter` · `paymentTossCircuitBreaker` · `TossWebhookController` | `/api/v1/payments/webhooks/toss` | `TOSS_SECRET_KEY` · `TOSS_WEBHOOK_SECRET` |

- 두 토스 키는 설정 파일에 기본값이 없다(`${TOSS_SECRET_KEY}` 만). toss 로 켰는데 없거나 비었으면 기동하지 않는다. mock 은 읽지 않는다.
