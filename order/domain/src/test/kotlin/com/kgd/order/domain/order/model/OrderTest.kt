...
class OrderTest : BehaviorSpec({
        ...
        `when`("상품 ID와 수량이 유효하면") {
            then("PENDING 상태의 주문이 생성되어야 한다") {
                val order = Order.create(
                    userId = "user-1",
                    items = listOf(OrderItem.of(productId = 1L, quantity = 2, unitPrice = Money(5000L)))
                )
                order.status shouldBe OrderStatus.PENDING
                order.totalAmount.amount shouldBe 10000L
    ...
    given("주문 완료 처리 시") {
        `when`("PENDING 상태이면") {
            then("COMPLETED로 전환되어야 한다") {
                val order = Order.create("user-1", listOf(OrderItem.of(1L, 1, Money(1000L))))
                order.complete()
    ...
    given("OrderItem 생성 시") {
        `when`("수량이 0이면") {
            then("IllegalArgumentException이 발생해야 한다") {
                shouldThrow<IllegalArgumentException> {
                    OrderItem.of(1L, 0, Money(1000L))
                }
            }
        }
        `when`("소계 계산") {
            then("unitPrice * quantity가 반환되어야 한다") {
                val item = OrderItem.of(1L, 3, Money(1000L))
                item.subtotal.amount shouldBe 3000L
            }
        }
    }
})
