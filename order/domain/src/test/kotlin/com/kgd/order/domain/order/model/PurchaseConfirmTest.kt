...
class PurchaseConfirmTest : BehaviorSpec({
    ...
    Given("판매자 둘 · 라인 셋 주문 (ClaimFixtures 표)") {
        When("라인을 하나씩 확정한다") {
            ...
            Then("판매자의 마지막 ACTIVE 라인에만 그 판매자 배송비 라인이 실리고, 마지막에 COMPLETED") {
                order.confirmPurchases(listOf(1), NOW).single().shipping shouldBe null
                order.confirmPurchases(listOf(2), NOW).single().shipping shouldBe ShippingLine(7L, 3_000)
        ...
        When("라인 3을 부분 취소한 뒤 남은 라인을 한 번에 확정한다") {
            val order = fulfillingOrder()
            order.closeClaim(listOf(3), 7_000 + 2_500, "m-1", NOW).shouldBeEmpty()
            val confirmed = order.confirmPurchases(listOf(1, 2), NOW)
            Then("취소되지 않은 라인이 전부 확정 → COMPLETED, 배송비는 번호가 큰 라인 하나에만") {
                order.status shouldBe OrderStatus.COMPLETED
                order.refundedAmount shouldBe 9_500
                confirmed.map { it.line.lineNo to it.shipping } shouldContainExactly listOf(1 to null, 2 to ShippingLine(7L, 3_000))
            }
        }

        When("라인 1 확정 뒤 라인 2가 클레임으로 취소된다") {
            val order = fulfillingOrder()
            order.confirmPurchases(listOf(1), NOW)
            Then("판매자 7 의 ACTIVE 라인이 없어진다 — 아직 정산에 오르지 않은 배송비 라인을 돌려준다") {
                order.closeClaim(listOf(2), 4_500, "m-1", NOW) shouldContainExactly listOf(ShippingLine(7L, 3_000))
            }
        }
