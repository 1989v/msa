import {WORLD,LANDMARKS,RUNES,PLATE,BLOCK_SPAWN,ENEMY_SPAWNS,SOLIDS,heightAt,supportAt,regionAt,clamp,distance} from './world.mjs';

export const DT=1/60;
const TAU=Math.PI*2;
const angleDelta=(a,b)=>Math.atan2(Math.sin(a-b),Math.cos(a-b));
const SIGILS=['quarry','forest','ruins'];
const BUTTONS=['jump','attack','dodge','parry','skill','interact','heal'];
const ENEMY_STATS={stalker:{hp:55,speed:3.3,damage:13},ranger:{hp:44,speed:2.5,damage:12},charger:{hp:95,speed:2.5,damage:22},boss:{hp:740,speed:2.6,damage:23}};
...
export function restoreSnapshot(data){
  if(!data||data.version!==1||!Number.isFinite(data.frame)||!data.player||!Number.isFinite(data.player.x))throw new Error('Invalid simulation snapshot');
...
function horizontalMove(body,dx,dz,solids=SOLIDS,r=.37,h=1.65,step=.48) {
...
function verticalMove(body,dt,solids=SOLIDS){
...
function dropReward(s,e){
  ...
  if(e.type==='boss'){
    if(!s.progress.bossDefeated){s.progress.bossDefeated=true;s.player.crystals+=15;s.mode='won';toast(s,'바람이 돌아왔습니다. 당신의 여정이 이 하늘을 깨웠습니다.','win');}
...
function hitEnemy(s,e,damage,kind='sword',knock=3){
  if(e.hp<=0)return;
  const p=s.player;
  if(e.type==='charger'&&kind==='sword'&&e.state!=='hit'&&facing(e,p,1.25)){
...
function pulse(s){
  ...
  for(const e of s.enemies)if(e.hp>0&&distance(p,e)<7&&Math.abs(p.y-e.y)<3.2&&lineClear(p,e))hitEnemy(s,e,22+s.progress.upgrades.power*3+(s.progress.sigils.length===3?12:0),'pulse',10);
...
export function awardSigil(s,id){
  ...
  const n=s.progress.sigils.length;s.player.maxStamina=100+(n-1)*20;s.player.stamina=s.player.maxStamina;s.player.energy=100;
...
export function interact(s){
  ...
  if(near.kind==='camp'){
    ...
    s.progress.chests.push(near.id);p.crystals+=5;p.flasks=Math.min(6,p.flasks+1);s.metrics.secrets++;
...
function updateBlocks(s){
  for(const b of s.blocks){
    ...
    horizontalMove(b,b.vx*DT,b.vz*DT,SOLIDS,.95,2,.5);b.y=supportAt(b.x,b.z,b.y+.5,.85);
...
function shoot(s,e,count=1){
  const p=s.player;
  for(let i=0;i<count;i++){
    const a=Math.atan2(p.x-e.x,p.z-e.z)+(i-(count-1)/2)*.18,speed=e.type==='boss'?12:9;
    const d=Math.max(1,distance(p,e));s.projectiles.push({id:`bolt-${s.nextId++}`,type:'bolt',x:e.x,y:e.y+1.3,z:e.z,vx:Math.sin(a)*speed,vz:Math.cos(a)*speed,vy:(p.y+.8-e.y-1.3)/d*speed,life:4,damage:ENEMY_STATS[e.type].damage,owner:e.id});
  }
...
function enemyAttack(s,e){
  ...
  if(d<radius&&dy<(e.type==='boss'?2.5:1.8)&&!avoidByJump&&facing(e,p,e.type==='boss'?Math.PI:1.3)&&lineClear(e,p))playerDamage(s,ENEMY_STATS[e.type].damage,e);
...
function updateEnemy(s,e){
  ...
  if(e.state==='hit'){
    horizontalMove(e,e.vx*DT,e.vz*DT,SOLIDS,.55,1.7,.5);e.vx*=.84;e.vz*=.84;
    if(e.timer<=0){e.state='chase';e.timer=.2;}
  }else if(e.state==='telegraph'){
    if(e.timer>.28)e.yaw=Math.atan2(p.x-e.x,p.z-e.z);
    if(e.timer<=0)enemyAttack(s,e);
  }else if(e.state==='attack'){
    horizontalMove(e,e.vx*DT,e.vz*DT,SOLIDS,.6,1.8,.5);
    if(!e.didHit&&distance(e,p)<2.4&&dy<1.8&&lineClear(e,p)){e.didHit=true;playerDamage(s,ENEMY_STATS[e.type].damage,e);}
    ...
    if(d<aggro&&dy<(boss?10:5)&&distance(e,{x:e.homeX,z:e.homeZ})<30){
      ...
      if(d<range&&dy<2.4){
        ...
        horizontalMove(e,Math.sin(e.yaw)*speed*DT,Math.cos(e.yaw)*speed*DT,SOLIDS,.5,1.65,.4);
      }
    }else{
      e.state='idle';const homeD=Math.hypot(e.x-e.homeX,e.z-e.homeZ);
      if(homeD>1){e.yaw=Math.atan2(e.homeX-e.x,e.homeZ-e.z);horizontalMove(e,Math.sin(e.yaw)*2*DT,Math.cos(e.yaw)*2*DT,SOLIDS,.5,1.65,.4);}
    }
  }
  ...
  else{e.vy-=25*DT;verticalMove(e,DT);if(e.y<WORLD.waterLevel){e.x=e.homeX;e.z=e.homeZ;e.y=e.homeY;e.vy=0;}}
}
function updateProjectiles(s){
  const p=s.player;
  for(const pr of s.projectiles){
    pr.x+=pr.vx*DT;pr.y+=pr.vy*DT;pr.z+=pr.vz*DT;pr.life-=DT;
    if(pr.y<heightAt(pr.x,pr.z)||SOLIDS.some(b=>overlaps(pr.x,pr.z,b,.1)&&pr.y>b.y&&pr.y<b.y+b.h)){pr.life=0;continue;}
    if(Math.hypot(pr.x-p.x,pr.z-p.z)<.65&&pr.y>p.y&&pr.y<p.y+1.7){playerDamage(s,pr.damage,s.enemies.find(e=>e.id===pr.owner));pr.life=0;fx(s,'hit',pr,{power:8});}
  }
...
export function stepGame(s,raw={}){
  ...
  s.frame++;s.time=s.frame*DT;const p=s.player;
  ...
  for(const key of ['invulnerable','skillCooldown','parryCooldown','comboWindow','actionTime','parryTimer'])p[key]=Math.max(0,p[key]-DT);
  ...
  if(p.attackTimer>0){
    ...
    if(!p.attackHit&&p.attackElapsed>[0,.10,.12,.18][p.combo]){
      p.attackHit=true;const base=[0,16,21,38][p.combo]+s.progress.upgrades.power*5;
      for(const e of s.enemies)if(e.hp>0&&distance(p,e)<(p.combo===3?3.9:3.2)&&Math.abs(p.y-e.y)<1.9&&facing(p,e,p.combo===3?1.8:1.25)&&lineClear(p,e))hitEnemy(s,e,base,'sword',p.combo===3?8:3);
    }
  ...
  let speed=p.gliding?8.4:sprint?9:5.8;if(p.attackTimer>0)speed*=.45;
  ...
  const px=p.x,pz=p.z;
  horizontalMove(p,p.vx*DT,p.vz*DT,[...SOLIDS,...s.blocks]);s.metrics.distance+=Math.hypot(p.x-px,p.z-pz);
  ...
  if(p.dodgeTimer<=0)for(const e of s.enemies){
    ...
    if(d>0.001&&d<minimum){const push=Math.min(.2,minimum-d);horizontalMove(p,(p.x-e.x)/d*push,(p.z-e.z)/d*push,[...SOLIDS,...s.blocks]);}
  }
  p.vy-=25*DT;
  ...
  const fallSpeed=p.vy,landed=verticalMove(p,DT,[...SOLIDS,...s.blocks]);p.fallPeak=Math.max(p.fallPeak,p.y);
  if(landed){
    s.metrics.landings++;p.airState='LAND';p.gliding=false;emit(s,'land');fx(s,'land',p,{life:.4,power:Math.abs(fallSpeed)});
    if(p.plunge){for(const e of s.enemies)if(e.hp>0&&distance(e,p)<5&&Math.abs(e.y-p.y)<2&&lineClear(p,e))hitEnemy(s,e,35,'plunge',9);fx(s,'shockwave',p,{life:.65,power:5});p.plunge=false;}
  ...
  if(!s.progress.discovered.includes(region.id)){s.progress.discovered.push(region.id);toast(s,`${region.name} 발견 · ${region.description}`,'discovery');}
...
export function fastTravel(s,campId){
  if(s.mode!=='playing'||!s.progress.discovered.includes(campId)||s.enemies.some(e=>e.hp>0&&distance(e,s.player)<13&&Math.abs(e.y-s.player.y)<5&&e.state!=='idle'))return false;
  const camp=LANDMARKS.find(l=>l.kind==='camp'&&l.id===campId);if(!camp)return false;
  const p=s.player;Object.assign(p,{x:camp.x,y:camp.y,z:camp.z-2,vx:0,vy:0,vz:0,gliding:false,grounded:true,checkpoint:campId,safeX:camp.x,safeY:camp.y,safeZ:camp.z-2});p.hp=p.maxHp;emit(s,'rest');return true;
}
export function exportSave(s){return {version:1,seed:s.seed,progress:structuredClone(s.progress),crystals:s.player.crystals,checkpoint:s.player.checkpoint,metrics:{...s.metrics},time:s.time};}
export function loadSave(data){
  if(!data||data.version!==1||!data.progress||!Array.isArray(data.progress.sigils)||!Array.isArray(data.progress.discovered)||!Array.isArray(data.progress.chests))throw new Error('올바른 WINDWAKE 저장 파일이 아닙니다.');
  const s=createGame(data.seed),p=s.player,raw=data.progress;
  s.progress.sigils=[...new Set(raw.sigils.filter(v=>SIGILS.includes(v)))];
  s.progress.discovered=[...new Set(['meadow','camp',...raw.discovered.filter(v=>typeof v==='string'&&v.length<40)])].slice(0,40);
  ...
  p.crystals=number(data.crystals,9999);p.maxHp=100+s.progress.upgrades.health*25;p.hp=p.maxHp;p.maxStamina=100+Math.max(0,s.progress.sigils.length-1)*20;p.stamina=p.maxStamina;
  if(LANDMARKS.some(l=>l.kind==='camp'&&l.id===data.checkpoint)&&s.progress.discovered.includes(data.checkpoint))p.checkpoint=data.checkpoint;
  const camp=LANDMARKS.find(l=>l.id===p.checkpoint);Object.assign(p,{x:camp.x,y:heightAt(camp.x,camp.z-2),z:camp.z-2,safeX:camp.x,safeY:heightAt(camp.x,camp.z-2),safeZ:camp.z-2});
