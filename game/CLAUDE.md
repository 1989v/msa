...
| `abyssal-crown` | `portal-fe/public/games/abyssal-crown/` (**다중 파일**) | **클린룸 1호 · 신규 품질 마지노선** (V30 시드). 하데스류 — 2560×1440 절차 벡터, JS 16모듈 10K줄, 축복 26종·보스 3종(3페이즈), 신스 BGM. 통합은 `lib/platform.js`(랭킹+세이브 동기화). 설계: `abyssal-crown/DESIGN.md` |
| `raging-fist-saga` | `portal-fe/public/games/raging-fist-saga/` (**다중 파일**) | **클린룸 2호** (V30 시드). 벨트스크롤+KOF 커맨드 — 스켈레톤 리그→15비트 양자화 베이크, 모션 커맨드 4종+초필 2종(캔슬·입력버퍼 6F), 스테이지 3+히든 1, 숨김 요소 5종. 통합은 `lib/platform.js`. 설계: `raging-fist-saga/DESIGN.md` |
