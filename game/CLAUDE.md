...
| 방향 | 메시지 |
|---|---|
| C→S | `{"t":"join","room":"<code>\|null","nick":"…","seats":20?}` · `{"t":"move","d":{…},"to":0?}` · `{"t":"leave"}` · `{"t":"ping"}` |
| S→C | `{"t":"joined","room":"ABC123","seat":0,"seats":2}` · `{"t":"start","seed":123,"players":["…",…]}` · `{"t":"move","seat":1,"d":{…}}` · `{"t":"opponentLeft"}`(2석) / `{"t":"left","seat":3}`(3석 이상) · `{"t":"seat","seat":1,"nick":"…"}`(3석 이상 로비 입장 알림) · `{"t":"error","code":"…"}` · `{"t":"pong"}` · `{"t":"ping"}`(유휴 확인) |
...
- **N석 (ADR-0088)**: 방을 만드는 join 의 `seats`(2~20, 생략 시 2)가 좌석 수를 정한다. 3석 이상 방은
  **로비 30초 마감**(인원 무관 시작 — `players` 의 빈 문자열 좌석은 호스트 클라이언트가 봇으로 채운다) ·
  **시작 후 잠금**(`ROOM_STARTED`) · `move` 의 `to` 좌석 지정 전달(게스트 의도 → 호스트 전용)이 붙는다.
  2석 방은 기존 프로토콜 그대로 — 배포된 2인 게임 클라이언트 무변경
- 상한: 메시지 4KB · 40 msg/s(초과 시 close — 의도 20Hz + 여유) · 동시 방 200 · 유휴 60초 ping / 90초 종료
...
- **파일은 막았지만 릴레이 방은 안 막았다.** `deep-night` 에 「같이 하기」(좌석 4)가 붙으면서
  `/ws/games/deep-night` 를 쓰는데, 그 라우트(`game-relay-ws`)는 게스트 통과 경로라
  **허용 명단이 걸리지 않는다.** 방 코드가 32글자 6자리(약 10억)이고 빠른 매칭을 안 쓰므로
...
- **새 마이그레이션을 붙이기 전에 최신 번호를 확인한다.** 여러 세션이 한 워킹트리를 쓰기 때문에
  ...
  **번호가 겹치지 않아도, 나중 번호가 먼저 배포되면 같은 문제가 난다** — 우리 Flyway 는
  `outOfOrder` 를 켜지 않으므로(`common/ScopedFlywayMigrator`) 73 이 적용된 스키마에서 미적용
  72 를 만나면 validate 가 막는다. **커밋했다고 적용된 것이 아니다**: 2026-09-03 에 V72(인피니티
  스탭)가 커밋만 되고 이미지가 안 구워진 사이 V73(마지막 한 사람)이 먼저 배포돼, V72 는 영영
  적용될 수 없는 상태가 됐다(V74 로 옮겨 풀었다). **배포 확인은 `flyway_schema_history` 조회까지다**
  — 게임 파일(portal-fe)이 떠 있어도 시드(code-dictionary)는 안 갔을 수 있다. 둘은 다른 이미지다
...
| `archer-outbreak` | `portal-fe/public/games/archer-outbreak/` (**Unity WebGL 빌드 산출물**) | **유니티 라인 1호** (V61 시드, `beta` 태그 · `engine_type=UNITY_WEBGL`). 「궁수 키우기」 — 유즈맵 계열 키우기형 생존 디펜스를 3D 탑다운으로 옮겼다. **원작 뼈대 그대로**: 처치 → 고철, 고철 → 기지 안에서만 보급품 환전, 보급품으로 **화살 위력 무한 강화**, 그리고 **몇 단계부터 한 방인지가 화면에 상시 표시**된다(원작의 "공 60업부터 저글링 원샷"이 이 게임의 이정표다). 시설 6종(화톳불·작업대·의무소·훈련소·감시탑·무전기)을 보급품으로 켜면 관통·다중 사격·동료 궁수·자동 포탑이 열리고, 구역 5곳(주차장→주택가→상가→병원→도심)을 하나씩 산다. 러시 92초 주기로 기지에 오고 **밖에 나가 있으면 기지가 털린다**(환전 정지 + 보급품 도난 + 전용 경고 배너). 무전기를 켜면 3페이즈 변이체. 손실 5회면 종료. **소스는 `unity/games/archer-outbreak/`** 이고 여기 있는 것은 빌드 산출물뿐이다 — `Build/` 는 파일명이 해시라 immutable 캐시이고 `.gz` 는 nginx 가 `Content-Encoding` 을 붙여야 푼다(폴백 꺼 둠). 전송량 **7.6MB**(gzip · 상한 15MB) 중 5.3MB 가 유니티 엔진이라 **게임을 더 내면 그만큼 반복 다운로드된다**. 3D 모델은 Kenney CC0 5팩(캐릭터 리깅+애니메이션 3클립 · 묘지 · 서바이벌 · 시티 · 자동차), 한글은 Gothic A1(OFL) 정적 SDF 아틀라스(글자를 소스 리터럴에서 추출해 굽는다). 소리·지면·이펙트·셰이더·후처리는 전부 코드. **Unity 물리 미사용** — 엔진 스트리핑이 Physics 를 빼기도 하고 좀비 100마리엔 자체 원형 충돌+격자가 싸다. **가상패드 배선**(강궁·회피·폭발·의무·기지) — 패드 입력은 KeyboardEvent 합성이 아니라 `GameTouch.axis()` 를 `.jslib` 로 직접 읽어 C# 에 넣는다. 두 방향 실측: 세로 390×608(밴드 236) · 가로 844×390, 궁수 29.7 CSS px, 120fps. 설계: 게임 폴더의 `DESIGN.md` · 에셋 출처: `CREDITS.md` · 파이프라인: `docs/plans/2026-08-27-unity-web-game-pipeline.md` |
| `last-one` | `portal-fe/public/games/last-one/` (**Unity WebGL 빌드 산출물**, 소스는 `_src/last-one/`) | **유니티 라인 3호 · 플랫폼 첫 실시간 20인 대전** (V73 시드, `status=BETA`). 「마지막 한 사람」 — 아홉 종 챕터 1 의 산(`KgdMountain`, 지름 620·7층·고도 130)에 스무 명이 떨어져 한 사람이 남을 때까지. **의도(Intent) 단일 통로**: 시뮬(`Match`·`Fighter`)은 이동 벡터·버튼 5·조준·이모트만 소비하고 사람·봇·원격이 같은 구조체를 채운다 — 봇이 사람보다 잘할 수 있는 것은 반응 속도뿐. **네트워크는 ADR-0088 호스트 권위**: 릴레이 `join(seats:20)` → 30초 마감 → 좌석 최저 번호의 사람이 봇 포함 시뮬 + 10Hz 스냅샷(≤4KB, 실측 ~1KB), 게스트는 의도 20Hz 를 `to:호스트` 로. 릴레이에 못 붙으면 그 자리에서 혼자 + 봇 19(사람 1 도 정상 판). 승계는 `left` 즉시 / 침묵 2.5초+0.5초×순위. 무기 5 + 방패는 지도에서 **위에 0.45초 서면 바꿔 든다**(버튼 5개가 다 찼다), 공격은 **톡 약공 / 누르면 강공**(`KgdWeapon.Heavy` — 1.6×·막기 관통·몸이 부풀어 예고), 화살 한 발 스태미나 5(끝없이 쏘며 도망 못 한다). 경계는 `KgdStorm` 고도 3단계(기슭+1층 → 4층 → 정상 테라스만, 예고 115/85/75초 + 차오름 30/30/40초, 안개 속 초당 6). **PvP 피해는 PvE 의 0.55~0.6**(검 12·도끼 20·창 14·활 7·맨손 4) — PvE 값 그대로면 착지 30초 만에 절반이 죽었다. 게이트 9회 반복에서 잡힌 것은 값보다 **봇의 결함**이었다(층을 모르는 직선 이동·점프 값이 문턱을 깎아 공중에서 손 놓기·무기가 소품 위에 떨어짐·활과 같은 속도로 쫓는 교착) — 표는 `DESIGN.md` §5. 커스터마이징 3색은 셰이더 마스크 대신 옷 tint + 머리·띠 **부위 메시**, 색은 표 번호(스냅샷 9비트). 랭킹은 순위 점수(1위 20…20위 1) — 평균 순위는 서버가 최고 기록만 남겨 못 낸다. 게이트(`Assets/Editor/LastOneGates.cs`, 봇 20 판 100회): PaceGate·BalanceGate·StormGate·NetGate·FillGate 실측치는 `DESIGN.md` §5·§7. 설계: `_src/last-one/DESIGN.md` · PRD: `docs/specs/2026-09-02-last-one-prd.md` |
| `infinity-step` | `portal-fe/public/games/infinity-step/` (**Unity WebGL 빌드 산출물**, 소스는 `_src/infinity-step/`) | **유니티 라인 4호 · 전투 없는 첫 게임** (V74 시드 · V75 로 `status=BETA` · **V80 도시 테마**). 「인피니티 스탭」 — 밤 뒷골목에서 시작해 도시를 타고 올라 끝에 **블랙홀**로 들어간다(그 너머는 2탄). **조작은 Only Up 을 따라간다** (2026-09-06 확장 — 참고작이 우리보다 관대했다: Shift 달리기 · 자동 모서리 잡기). **차지 점프 5단 양자화**(누름 0.12초/단, 공중 조향 잠김) + **달리기**(`Z`, 걷기 5.5 → 8.6, 달려서 뛰면 수평 +18%) + **모서리 잡기**(윗면 아래 **0.5~0.95** 창에서만 손이 걸린다 — 무릎 0.45 바로 위, 그보다 아래는 그대로 떨어진다) + **벽 매달림**(배수관이 붙은 등반벽, 잡기 버튼이 방향과 무관하게 붙는다). **첫 완주 기준선은 조작 세트를 따라간다** — Jump King 기준 6~12시간에서 **2.5~10시간**으로 옮겼다(측정 3.1시간). 게이트도 「보정이 없다」가 아니라 **「창이 정확하다」**를 잰다(0.8 안은 올라섬 · 1.4 밖은 못 올라옴). 발판은 **단단한 덩이**라 밑에서 뚫고 들어가지 못하고 머리를 부딪는다(`IKgdCeiling`), 옆면은 막고 비탈은 미끄러진다. 낙하 상한 없음 — 공사장에서 미끄러지면 옥상까지, 운이 나쁘면 골목까지 간다(`FallGate` 표본 200 에 보이지 않는 바닥 0). 구간 7(뒷골목·외벽·옥상·마천루·공사장·성층권·블랙홀)은 경계가 아니라 **고도 함수의 키프레임**이라 밤→여명→낮→우주가 연속으로 바뀐다(`LookGate`). 장치: 부서짐(낡은 차양) · 무버(곤돌라) · 얼음(젖은 유리) · 옆바람 · 배수관 외벽. **탑이 아니라 도시를 가로지른다** — 나선 반지름 50~95 · 한 바퀴 고도 800 이라 경로의 수평 반지름 평균 **69**(최대 104, 지도 반경 110), 8방위 전부. **생성은 보장 경로 먼저**(G3, 링크 744) + **곁길**(여섯 걸음마다 갈라지는 2~4칸 사슬 — 같은 물리 검사를 통과한 진짜 길이라 한 줄로만 오르지 않는다). 발판은 구간별 **외장**을 입는다(컨테이너·창틀·파라펫·커튼월·비계·그레이팅·위성 패널, 윗면은 안 건드린다). 시드는 KST 날짜(같은 날 같은 도시), 세이브는 착지 고도·낙하 횟수뿐(되돌림 없음). 랭킹 보드 둘: `height`(최고 고도) · `summit`(블랙홀 도달 `43200−초` — **키는 그대로**, 이름만 바뀌었다). **패키지에 새로 올라간 것**: `KgdTraverse` 차지 점프·`StopAtWalls`·`AirStepUp`·`PushOutOfWalls`·`GrazeMove`, `IKgdCeiling`, `KgdBody.CanEnter/PushOut`, `KgdMover`, `KgdSky.cloudBelow`. 게이트 5종(`Assets/Editor/` — Move·Height·Fall·Look·Pace) 전부 회귀를 주입해 빨간불을 본 뒤 통과: 완벽 봇 **23.4분 놓침 0** · **학습 봇** 6.7시간 · 끼임 검사 1,080판(도약·낙하·옆면 비비기 각 360)에 잠김 0·튀어오름 0. 전송량 **7.06MB**(아트가 전부 코드), 첫 프레임 세로 4.10초 · 가로 3.99초, 예외 0. 설계: `_src/infinity-step/DESIGN.md` · PRD: `docs/specs/2026-09-02-infinity-step-prd.md` |
| `zombie-march` | `portal-fe/public/games/zombie-march/` (**다중 파일**) | **클린룸 8호** (V79 시드). 「좀비 행군」 — 영지를 넓히는 좀비 전략. 벤치마크는 모바일 세로 전략물이고, 가져온 것은 장르 문법(타일 확장 · 손가락으로 부대 유도 · 본진 레벨 게이트)뿐이다. **축은 「죽인 사람이 몇 초 뒤 일어나 내 편이 된다」** — 그래서 「이겼다」와 「군단이 늘었다」가 같은 말이고, **시체를 태우는 사제가 최우선 표적**이 된다(사제가 있는 판은 시체가 재원이 아니다). 두 층: 33×33 아이소 영지(안개 4단계, 전선만 선택 가능 · 건물 4종×3단계 · 묘지성 5단계가 출정 상한(50→300)·건물 자리·영웅 자리·좀비 해금을 함께 잠근다 · **지역 관문**: 앞 지역 성채를 무너뜨려야 다음 지역을 칠 수 있다 · 오프라인 8시간 정산)와 세로 전장(전열 2~3줄, 조작은 **집결 지점 하나**, 무리에서 340 유닛 넘게 못 떨어진다). 사람 8종이 각자 다른 답을 요구한다 — 창병은 앞 ±0.95rad 관통이라 옆으로 돌고, 궁수는 뒷걸음질이라 뛰는 좀비로 파고들고, **기사 대장**은 광역 휘두르기+돌진에 절반에서 2페이즈(종·소환·2.5초 무적)이며 **돌진 뒤 2.2초 「빈틈」**(피해 1.6배·갑옷 0)이 유일한 창이다. 보스 정의는 하나지만 **지역 배율**(1.0/1.7/2.3/2.9)이 체력·공격·휩쓸기·갑옷을 키우고 2페이즈 호위가 민병→창병→사제→기사로 갈린다. **전장 폭이 방향에 따라 다르다** — 세로 1080(개체 1/18.0 · 21.7 CSS px), 가로 1560(1/12.0 · 32.5 CSS px). 세로용 폭을 가로에 그대로 쓰면 화면 절반이 빈다. 줌 3단(전투·지도 각각). **가상패드를 붙이지 않는다** — 끌기로 집결 지점을 옮기고 나머지는 탭이라 패드가 전장 아래를 가린다. **폰은 프레임이 아니라 일의 양을 깎는다**(rAF 격자상 45fps 가 불가능하고 30fps 는 끌기가 끊긴다): 60fps 유지 + 안개 1겹·파티클 340·광원 4, 멈춤·모달은 20fps. 에셋 파일 0개(스프라이트·지형·소리 전부 코드, 252KB). 통합은 `lib/platform.js`, 세이브 키 `zombie-march.save`, 랭킹 스칼라는 먹은 땅의 적 전력 누적. **전멸해도 막다른 길이 없다** — 패배 전리품을 「남은 시체」로 계산하면 항상 0이라(패배 판정이 시체가 다 일어난 뒤에 난다) 군단 0·시체 0 의 되돌릴 수 없는 상태가 생겼다. 지금은 죽인 수에서 수거하고, 그래도 0이면 묘지성이 잡좀비 12를 다시 일으킨다. 실측 함정 2건이 여기서 나왔다 — **좀비의 `rising` 상태를 사람에게 쓰면 `hurt()` 가 무적으로 다뤄 전투가 영영 안 끝난다**(보스 소환 8기), **패치가 `onKey` 를 통째로 복제해 옛 코드가 이겼다**(파일엔 새 코드가 보이는데 키가 안 먹었다). 설계: `docs/specs/2026-09-05-zombie-march-design.md` |
| `gwiyahaeng` | `portal-fe/public/games/gwiyahaeng/` (**다중 파일**) | **클린룸 9호** (V92 시드, BETA). 「귀야행 척준경」 — 고려 무장 척준경이 귀굴 아홉 층을 내려가며 혼백을 모아 귀장을 봉인하는 2D 픽셀 탐험 액션. **아트는 전부 클로드 디자인 산출**(프로젝트 「척준경 — 아트 시안」, 목표 화면 v2 + 애니메이션 시트 + 배치 1·2, 시트의 그리기 함수를 `js/sheet*.js` 에 그대로 옮겨 실행 시 굽는다 — 캐릭터·타일을 여기서 고치지 않는다). 규모는 벤치마크(16비트풍 탐험 액션 인디 명작) 이상: 층 9(1층 수작업, 2~9층 `js/gen.js` 층 시드 고정 생성 + `js/reach.js` 도달성 게이트 — 막다른 곳 0 인 시드만 채택), 방 179, 층주 10(패턴 라이브러리 14종, 정답이 3종 이상 다르게 배분, 鬼將 3페이즈), 적 27종(전부 예고 → 공격, `js/enemies.js`), 혼백 198·부적 조각 27·숨은 발판 27, 능력 6(이단 점프·갈고리 발사·낙하 베기 강화·검기 강화·벽 차기·부적 결계). 축척은 사용자 지정 **주인공 26px = 짧은 변 1/13.8**(「층·발판이 주역」), 세로 360×(360~720)·가로 (480~800)×360 뷰포트라 폰에서 28.2 CSS px. 조작: 걷기 88·달리기 150(대시 키 유지, 점프 62→107)·3단 검·↑+공격 부적·↓+검기 회선참·공중 ↓+공격 낙하 베기. **층주 공략은 정답/오답 봇 대조로 잰다**(`verifications/boss-bot.mjs`, 1~6 비율 0.12~0.38 ≤ 0.40) — 대조가 잡아낸 것: 붙어 있으면 접촉 피해가 계속 들어와 「붙어서 베기」가 성립 안 함(→돌진 중에만), 표식이 예고 내내 따라와 「표시 보고 옆으로」가 불가능(→0.1s 고정), 60px 보스 돌진을 44px 점프로 못 넘음(→판정 아래 22px), 물보라가 위로만 날아가 오답이 무처벌(→수평 부채꼴). 가상패드 액션 4(공격·점프·달리기·검기) + `data-fit="0"` 로 세로 패드 밴드를 게임이 비운다(HUD 는 셸 칩 아래로). 남은 것: 배치 3(7~9층 아트), 플레이타임 실측. 설계: `portal-fe/public/games/gwiyahaeng/DESIGN.md`, 스펙 `docs/specs/2026-09-13-cheok-jungyeong/` |
| `zombie-lane` | `portal-fe/public/games/zombie-lane/` (**다중 파일**) | **클린룸 9호** (V82 시드). 「좀비 레인」 — **게이트 러너**(군중 클래시) 장르. 조선 군사를 이끌고 역병에 걸린 무리를 막는다. 길이 세 줄로 갈리고 줄마다 숫자 문이 서 있어, 지나간 문의 계산이 부대에 걸린다. **벤치마크를 앱이 아니라 그 앱의 광고에서 가져온 건**이다 — 요청받은 참조작은 실제로는 영지 건설 전략이고 광고에 나오는 러너가 원한 것이었다(먼저 만든 `zombie-march` 는 그 앱 본편 쪽이라 지우지 않고 둘 다 서비스한다). **숫자 규칙 셋이 이 게임의 전부다**: ① 덧셈은 구간 기준(`addBase`=시작×1.6)·곱셈은 부대 기준 — 덧셈을 부대에 비례시키면 언제나 ×2 와 같아져 「지금 몇 명인가」를 물을 이유가 사라진다 ② **곱셈은 세 줄에 한 번, 한 장만** — 무작위로 뿌렸더니 같은 실력에 결과가 **350명~550만명**으로 벌어졌다(복리) ③ **카드는 부대를 0으로 못 만든다**(하한 1), 0은 좀비만 만든다. 병종 3종(창병 기본 / 궁수 붙기 전 원거리로 깎지만 붙으면 약함 / 방패병 덜 죽지만 느림)은 부대를 통째로 바꾼다 — 수 하나가 진실이어야 해서 섞지 않는다. 충돌은 란체스터 제곱이라 `이김 ⇔ 부대 > 0.55×좀비hp`, 남는 수 `√(부대²−0.3×좀비hp²)`. **무리 크기는 지수**(뒤로 ×3) — 균등하게 나누면 첫 무리가 시작 부대보다 산더미다. **결정 간격은 거리가 아니라 시간으로 잡는다** — 문 간격 205 로 뒀더니 0.68초라 세 장을 읽을 수 없었고, 660(2.2초)이 최소치다. **구간의 끝은 거리가 아니라 성문이다** — 길 끝에 성문이 서고 그 앞을 지키는 무리를 이길 병력(`⌈√0.3×성문좀비hp⌉`)이 안 되면 성문 90 유닛 앞에서 멈춘 채 못 지나간다. 정해진 거리를 다 달리면 이기게 뒀더니 속도가 일정해 시간제한처럼 동작했고, 길 위에 목적지를 보여 주는 것이 없어 사용자가 **「시간 초과로 끝났다」**로 읽었다(타이머는 없었다). 필요한 수는 HUD 띠와 성벽 면 두 곳이 동시에 말한다. **무리 예산은 마리 수가 아니라 한 마리의 무게로 쓴다** — 한 마리씩 밀어 넣으면 루프 상한에 먼저 걸려 후반 구간이 예산의 16~28% 만 쓰고, 그 탓에 9구간 성문(65,806)이 10구간 성문(16,181)보다 무거워져 진행이 뒤집혔다. 지금은 전 구간이 예산의 92~101% 를 쓰고 성문 hp 가 단조증가한다(160 → 428K). 한 무리는 32~57마리(전에는 400마리). **모드 둘 · 판단 축 셋 · 장수 여덟**(2026-09-06, 탑 로드 인게임 27장을 레퍼런스로 반영) — 진격전(12구간, 성문 돌파)과 **수성전**(8구간, 세상이 내려오고 앞에 선 것 하나만 쏜다. 큰 패 앞에 체력 있는 녹채가 서서 「좀비를 쏠까 녹채를 부술까」가 판단이 된다). **수성전 난이도는 고정 체력이 아니라 내 화력 기준**이다 — 고정으로 두면 부대가 커진 뒤 어떤 물결도 1초에 녹아 전 구간 5/5 가 된다(실측). 판단 축은 숫자 문 · 화력 통 · **조준 표적**(좌우로 흔들리는 ×N, 값이 클수록 폭이 좁다) 셋이고, 이 셋째 축이 실력 차를 벌린다(넣기 전 숙련 0.7 이 6구간까지 따라왔고, 넣은 뒤 5구간에서 무너진다). 장수는 한국사 여덟(을지문덕·양만춘·김유신·계백·강감찬·최영·이순신·권율), 셋 편성, 패시브 하나 + 쿨다운 기술 하나, 엽전으로 Lv1→10. **시야**는 지평선 0.26·CAM_BACK 340 으로 세 줄 앞까지 읽히고(문 글자 10.6 → 16.6 CSS px), **부대 상한 220/320** 으로 수가 면적으로 읽힌다. **레인 중심은 ±2/3 다** — ±0.62 로 뒀더니 문이 제 칸에서 0.047 치우쳐 「살짝 틀어져」 보였다. **스프라이트는 배율을 1.25배 계단으로 올려 굽고 그릴 때 줄인다** — 연속 배율을 캐시 키로 쓰면 깊이가 다른 만큼 매 프레임 다시 굽는다(220명 화면에서 초당 4,857회, 완주 중앙 37fps → 0회, 60fps). **같은 말이 겹쳐 뜨면 억제한다** — 폭발 좀비 여럿이 동시에 죽으면 「펑!」이 12개 겹쳐 글자가 뭉갰다. **성벽 전체가 화면에 들어오도록 배율을 줄이고**(가로는 지평선 위 하늘이 얇아 지붕이 HUD 뒤로 잘렸다) 안내 칩은 가로에서 왼쪽으로 비킨다. **지평선 안개를 불투명하게 두면 성문과 먼 카드가 통째로 지워진다** — 위쪽 정지점을 0.72 로 낮췄다. 12구간 25~54초, 1회차 25~35분. 세로 기준 선두 병사 39.5 CSS px(1/9.9), **대형 깊이 90 고정**(넘으면 뒷줄이 카메라보다 가까워져 화면 밖으로 흘러나간다), HUD 가림 4.4%. **가상패드 없음** — 좌우 하나뿐이라 화면 드래그와 아래 세 칸 탭으로 받는다. 에셋 파일 0개(136KB). 통합은 `lib/platform.js`, 세이브 키 `zombie-lane.save`, 랭킹은 구간·부대·연속의 단일 스칼라. 설계: `docs/specs/2026-09-05-zombie-lane-design.md` |
| `landslide` | `portal-fe/public/games/landslide/` (**다중 파일**, 정수 물리 시뮬 js/00~08 + 렌더·HUD·소리, 도구 `tools/gates.sh`) | **클린룸 10호** (V91 시드, `status=BETA`, `orientation=LANDSCAPE`, `supports_mobile=1`, `sdk_integrated=1`, 보드 `practice`). 「사태(沙汰)」 — **흐르는 지형 위의 턴제 포격 대전**. 규칙은 전부 재질표 6종에서 파생(가루 안식각·용암 굳기·눈/얼음 녹기·매몰·지진), 기류장(기본 바람 + 용암 위 상승)이 탄을 밀고 유선으로 항상 보인다. 삼국~조선 무장 병과 5(지형 동사 하나씩)·시대 스킨 4 — 아트는 클로드 디자인 프로젝트 `사태 — 아트 시안` 산출물(`characters.js`·`world.js`·`hud.css`)을 그대로 쓴다. **결정론 락스텝** 전제: 16.16 정수 물리, 시드 + 입력 레코드(이동 이벤트 열·각도·파워·무기)만으로 재생, 턴 끝 FNV-1a 해시 — `tools/gates.sh` 가 재생·jsc 교차·회귀 주입·봇 공개 뷰 tsc·lint·IP 스캔 9종을 한 번에 돈다. **지금은 슬라이스**: 궁수 노템전 1v1 vs 봇(실제 탄도 시뮬, 오차 폭만 조절), 화산 1. 병과 4·템전(뭉친 기)·팀전 8인·바이옴·온라인(릴레이 N석, 서버 변경 0)은 다음 단계 — 구조는 자리 있음. 터치는 네이티브(드래그 조준·놓으면 발사·탭 이동·점프 버튼), 세로 기기는 스스로 90° 회전. 🏆 버튼은 우측 중단으로 비켰다(우하단이 발사 버튼). PRD `docs/specs/2026-09-13-landslide-prd.md`, 검증 `…/verifications/slice-report.md` |
