...
| `infinity-step` | `portal-fe/public/games/infinity-step/` (**Unity WebGL 빌드 산출물**, 소스는 `_src/infinity-step/`) | **유니티 라인 4호 · 전투 없는 첫 게임** (V72 시드, `status=DRAFT` — 캐릭터가 아홉 종 리그의 틴트라 아홉 종과 같은 기준). 「인피니티 스탭」 — 점프 하나로 우주(3,000)까지, 그 위로도 끝없이 오르는 탑. **차지 점프 5단 양자화**(누름 0.12초/단, 공중 조향 잠김 — 뛰기 전에 다 정한다) + **벽 매달림** + **모서리 잡기**(윗면 아래 0.5~1.0 만 — 「아깝게」까지). 낙하 상한 없음: 발판은 **one-way 슬래브**(위에서만 잡힌다, 두꺼운 벽만 옆을 막는다)라 옆을 스치면 받아 주는 것이 나올 때까지 떨어진다(`FallGate` — 표본 200 에 보이지 않는 바닥 0, 1,500 위에서 지상까지 닿는 시드 83%). 구간 7(지상·숲·폐허·구름·설산·성층권·우주)은 경계가 아니라 **고도 함수의 키프레임**이라 하늘·안개·해·바람·중력이 연속(`LookGate`). 장치: 부서짐(밟고 0.55초 → 0.6초 흔들림 → 8초 뒤 재생) · 무버(정지 발판 사이 하나씩, 연속 금지) · 얼음(관성 + 착지 감쇠) · 옆바람(고도 해시). **생성은 보장 경로 먼저**(G3) — 링크 830여 개, 정밀 링크 6~11%, 실수 궤적 아래 받침 30%. 시드는 KST 날짜(같은 날 같은 탑), 세이브는 착지 고도·낙하 횟수뿐(되돌림 없음). 랭킹 보드 둘: `height`(최고 고도) · `summit`(우주 등정 `43200−초`). **패키지에 새로 올라간 것**: `KgdTraverse` 차지 점프·모서리 잡기·`NoGlide`·`GravityScale`·`Slip`·`TerminalFall`·빠른 낙하 세분화·착지 감쇠, `KgdMover`, `KgdSky.cloudBelow`. 게이트 5종(`Assets/Editor/` — Move·Height·Fall·Look·Pace) 전부 실제 결함으로 빨간불을 낸 뒤 통과: 완벽 봇 24.2분 · **학습 봇**(첫 시도 실패 50%, 재시도마다 반감) 첫 완주 6.9시간 — 기억 없는 50% 봇은 850 링크 랜덤워크라 시간이 폭발해 판정에 못 쓴다(DESIGN §9). 전송량 **7.06MB**(챕터 번들 없음 — 아트가 전부 코드), 세로 390×608 첫 프레임 4.10초 · 가로 844×390 3.99초, 예외 0. 설계: `_src/infinity-step/DESIGN.md` · PRD: `docs/specs/2026-09-02-infinity-step-prd.md` |
