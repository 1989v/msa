...
- **모바일이 1순위 타겟이다** (2026-08-24 개편). 데스크톱에서 좋고 모바일에서 나쁘면 그 게임은
  실패다 — 반대는 허용된다. 50여 종에서 "캐릭터가 크고 맵이 확대돼 못 즐긴다"가 반복된 원인은
  게임마다의 실수가 아니라 **완성 판정이 데스크톱 전용이었던 것**이다. 지금은 세로 390×844 ·
  가로 844×390 **두 방향 CDP 실측**이 완성의 조건이고, 크기는 **레터박스 배율을 포함한 CSS px**
  로 잰다(G1 의 비율 통과와 별개 — 2560×1440 캔버스는 390px 폰에서 390×219 가 된다).
  조작 방식(네이티브 터치 / 가상패드)은 **골라서 근거를 남기고**, 정보 패널은 접힌다.
  규칙 원본은 `docs/standards/game-cleanroom-pipeline.md` 전역 가드레일 **G5**,
  배치 수치는 `docs/conventions/game-input-standard.md` 세로모드 패드 비율 규격
- **문서에만 있는 규격은 지켜지지 않는다 — `scripts/lint-game-mobile.py` 가 정적으로 잡는다.**
  실측: 3부 한글 라벨 준수 4/48, rank.js 누락 4종이 몇 주간 점수 0건. 게임 파일을 고치면
  `.claude/hooks/game-mobile-check.sh` 가 자동으로 돌고, 통합 시에는 `--strict` 로 경고까지 없앤다
...
| `bee-guard` | `portal-fe/public/games/bee-guard/` (**다중 파일**) | **「그어서 막기」** (V54 시드 · **V58 개편**). 장르 프리셋 **`draw-to-solve`** 산출물 — 그린 선이 그대로 물리 물체가 된다. **막기 유형 3종**을 인게임에서 고른다: **물**(흐름·고임·부착) / **돌**(무게로 선이 부러짐·깨짐·관통) / **벌**(분열·밀기·반사). 위협 **14종**이 질량 0.5~6.2 · 반발 0~0.56 · 마찰 0.02~0.7 로 갈려 **유형마다 정답이 다르다** — 같은 방어선을 세 유형에 그으면 순위가 뒤집힌다(천막이 물 1009 / 돌 798 인데 벌은 383, 막으면 갈라지기 때문). **판 모드 27판**(랭킹 없음)과 **끝없이 모드**(유형별 랭킹 3보드 `leak`/`rockfall`/`bee`). 뼈대 변경: **그린 선에 체력**을 줘서 받침(벽·바닥·다른 획) 유무로 세기가 갈린다 — 이게 없으면 '무게'와 '관통'을 표현할 데가 없다. **잉크는 회복 자원**이라 아끼기 대회가 되지 않는다(최대 절약 전략이 세 유형 전부 최저 점수 — 실측). 기존 10판은 값 그대로 벌 유형 판 모드로 보존(개편 전후 실행 비교로 확인). **가상패드 미배선** — 손으로 그리는 게임이라 패드가 판을 가린다. 설계: `docs/specs/2026-08-24-bee-guard-design.md` |
| `random-card-defense` | `portal-fe/public/games/random-card-defense/` (**다중 파일**) | **클린룸 7호** (V57 시드). 뽑은 패로 만든 **포커 족보가 세워지는 타워를 정한다** — 10단계 족보, 무늬별 성격(관통/회복/광역/독). 손패 위에 **지금 가능한 조합·한 장 남은 조합**이 실시간 표시되고 전장 받침에 같은 족보 번호가 붙어 손패↔전장이 같은 시각 언어를 쓴다. **모드 3종** — 혼자 / 협동(인원수만큼 전장 분할, 웨이브 공유) / **대전(개인전)**. 맵은 성문→성벽 균열→윗길 3단계로 경로가 바뀐다. 월드 **9,216²**, 한 화면 **4.34%**. **대전 공정성**: 같은 시드만으로는 부족해 자리 간 공유 상태 5종(난수 스트림·보스 패턴·보스 효과·광장 터·수집물 장부)을 전부 분리했고 AI 대 AI 완전 동형으로 검증했다. 2인 로컬은 좌/우 키 클러스터(1P 방향키+`J K L ; '` / 2P WASD+`F G V B`). **가상패드 미배선**(탭 기반). 통합은 `lib/platform.js`, 세이브 키 `randomCardDefense.v1`, 랭킹 스칼라는 도달 웨이브. 설계: `docs/specs/2026-08-24-random-card-defense-design.md` |
| `infinity-tower` | `portal-fe/public/games/infinity-tower/` (**다중 파일**) | **클린룸 6호** (V56 시드, `beta` 태그). **최대 4슬롯 협동 액션** — 직업 4종(수호기사/밀정/사수/술사)로 **모드 2종**: 「첨탑 방어」(사방 6갈래, 14:06)와 「타워 등반」(한 층 = 한 전투, 60~90초·층지기 120초). 등반은 **층 번호 = 시드**인 절차 생성이라 같은 층은 항상 같고, 10층마다 층지기 + 체크포인트로 이어 오른다(999층 목표, 현재 1~10층 개방). 레벨·성장이 층을 넘어 이월. 각 슬롯이 `human|ai` 런타임 전환이라 **자리를 비우면 AI 가 이어받는다**(`1.2초 무입력 = 이탈` 판정). 월드 **10,240²**, 한 화면이 보는 면적 **5.88%**, 주인공 **1/19.4**, 줌 3단. **가상패드 배선**(방향 + Z/X/C/V/Space) — 캐릭터 조작 게임이라 필요하다(랜덤 타워 디펜스는 반대 판단). 통합은 `lib/platform.js`, 세이브 키 `infinity-tower.save.v1`, 랭킹 스칼라는 두 모드 공통 `world.score`. 릴레이 훅 `InfinityTower.net.applyIntent(slotId, intent)` 준비됨. 설계: `docs/specs/2026-08-24-infinity-tower-design.md` |
