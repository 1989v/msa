    ...
    given("게임 상세 조회 시") {
        `when`("PUBLISHED 게임이면") {
            then("통계와 함께 반환되어야 한다") {
                ...
                val detail = service.execute(GetGameDetailUseCase.Query("alpha"))
    ...
    given("홈 큐레이션 조회 시") {
        `when`("MANUAL 컬렉션이면") {
            then("지정한 순서대로 게임이 채워져야 한다") {
                ...
                val collections = service.execute()
        ...
        `when`("MANUAL 컬렉션에 공개 불가 상태 게임이 섞여 있으면") {
            then("공개 목록에서 빠진다 — findByIds 는 상태를 거르지 않는다") {
                ...
                service.execute()[0].games.map { it.slug } shouldBe listOf("alpha")
            }
        }
        ...
        `when`("자동 산출 컬렉션(TRENDING)에 gameIds 가 있으면") {
            then("그 게임들이 맨 앞에 고정되고 나머지는 산출 순서대로 이어진다") {
                ...
                service.execute()[0].games.map { it.slug } shouldBe listOf("pinned", "top")
            }
        }
    }
    ...
    given("큐레이션 행 간 중복 제거 시") {
        ...
        `when`("산출 목록이 정원을 넘으면") {
            then("여유분 20개를 받아 걷어낸 뒤 10개로 자른다") {
                ...
                service.execute()[0].games.size shouldBe 10
            }
        }
        ...
        `when`("행을 채울 게임이 하나도 없으면") {
            then("빈 행은 응답에서 뺀다") {
                ...
                service.execute().map { it.slug } shouldBe listOf("new-games")
            }
        }
    }
})
