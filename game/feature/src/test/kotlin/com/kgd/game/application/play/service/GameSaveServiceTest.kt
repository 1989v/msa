...
class GameSaveServiceTest : BehaviorSpec({
    ...
    given("세이브 로드 시") {
        `when`("로그인 사용자가 코드 없이 부르면") {
            then("계정 슬롯의 세이브를 돌려줘야 한다") {
                val saveRepository = mockk<GameSaveRepositoryPort>()
                every { saveRepository.find(1L, 7L) } returns SaveSnapshot("{}", 2L, "CODE12345678")

                serviceWith(saveRepository)
                    .execute(LoadGameSaveUseCase.Query("roguelike", memberId = 7L, code = null))?.version shouldBe 2L
            }
        }
        ...
        `when`("세이브가 없으면") {
            then("null을 반환해야 한다 (신규 유저)") {
                ...
                serviceWith(saveRepository).execute(LoadGameSaveUseCase.Query("roguelike", 7L, null)) shouldBe null
            }
        }
        ...
        `when`("게스트가 이어하기 코드를 제시하면") {
            then("코드로 세이브를 찾아야 한다") {
                ...
                val result = serviceWith(saveRepository)
                    .execute(LoadGameSaveUseCase.Query("roguelike", memberId = null, code = "ABCD2345WXYZ"))
        ...
        `when`("로그인 사용자의 계정 슬롯이 아직 비어 있고 게스트 코드를 함께 제시하면") {
            then("그 코드의 세이브를 돌려줘야 한다 (게스트 진행도가 로그인 직후 사라지지 않게)") {
                ...
                val result = serviceWith(saveRepository)
                    .execute(LoadGameSaveUseCase.Query("roguelike", memberId = 7L, code = "ABCD2345WXYZ"))
        ...
        `when`("신원(로그인/코드)이 전혀 없으면") {
            then("불러올 세이브가 없어 null을 반환해야 한다") {
                serviceWith().execute(LoadGameSaveUseCase.Query("roguelike", memberId = null, code = null)) shouldBe null
            }
        }
    }
    ...
    given("세이브 저장 시") {
        ...
        `when`("버전이 맞으면") {
            then("업서트 결과를 반환해야 한다") {
                ...
                val result = serviceWith(saveRepository)
                    .execute(StoreGameSaveUseCase.Command("roguelike", 7L, null, """{"floor":3}""", 2L))
                result.version shouldBe 3L
            }
        }

        `when`("게스트의 첫 저장이면") {
            then("이어하기 코드가 발급되어야 한다") {
                val saveRepository = mockk<GameSaveRepositoryPort>()
                every { saveRepository.upsert(1L, null, null, """{"floor":1}""", 0L) } returns
                    SaveSnapshot(data = """{"floor":1}""", version = 1L, code = "NEWCODE23456")

                val result = serviceWith(saveRepository)
                    .execute(StoreGameSaveUseCase.Command("roguelike", null, null, """{"floor":1}""", 0L))
