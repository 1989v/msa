...
--   · 아래 항목을 씨앗으로 섞으면 균등해진다 — n=2 chi2 0.0 / n=6 chi2 3.1 (임계 3.84 / 11.07)
--
-- supports_mobile=1 근거 (터치 에뮬레이션 414x896 · 캔버스 500x666 CSS 실측):
--   · 터치로 준비 화면 → 머리표 탭(한 줄만 먼저) → 출발까지 확인
--   · 화면 버튼 157.4 x 44.4 CSS px (터치 타깃 44px 상회) · 머리표 칸 폭 75 CSS px
...
VALUES
    ('ladder-draw', '사다리타기',
     '커피 사는 사람 정하기, 역할 나누기, 순서 정하기에 쓰는 사다리타기. 이름을 넣으면 사다리가 만들어지고, 가로줄은 처음부터 전부 화면에 보인다 — 가리는 것은 아래 결과뿐이다. 한 명이 걸린다 / 순서 정하기 / 항목 배정 세 가지로 쓸 수 있고, 머리표를 누르면 그 사람 줄만 먼저 내려가는 것도 볼 수 있다. 사다리는 구조상 항상 일대일 대응이라 두 사람이 같은 결과를 받는 일이 없다. 다만 사다리 자체는 균등하지 않아서, 사람별 확률을 고르게 만드는 장치는 아래 항목 배치를 씨앗으로 섞는 것이다(실측: 6명 기준 섞기 전 카이제곱 640, 섞은 뒤 3.1). 결과를 미리 정해 두고 영상만 재생하는 방식이 아니며, 판마다 씨앗을 띄우고 같은 씨앗으로 다시 만들면 똑같은 사다리가 나오는 것으로 확인할 수 있다. 이름과 항목은 이 기기에만 저장되고 어디로도 전송하지 않는다. 앱 안에 돈이나 포인트는 없다. 그래픽·사운드 전부 절차 생성(1080×1440).',
     'Ladder Draw',
