...
--
-- 처음에 V49 로 붙였다가 V51 로 옮겼다. 같은 워킹트리를 쓰는 다른 작업이 먼저 V49
-- (game_score_daily)를 커밋했고, 버전이 겹치면 Flyway 는 마이그레이션을 세는 단계에서
-- 기동을 거부한다 — 테스트 게이트가 죽어 **그 커밋의 모든 서비스 이미지**가 안 만들어졌다.
-- 옮겨도 안전한 근거는 확인했다: 운영 flyway_schema_history 의 49번은 game_score_daily 이고
-- 이 시드는 어디에도 적용된 적이 없다(운영 카탈로그에 marble-race 가 없었다).
-- 새 마이그레이션을 붙이기 전에 **먼저 최신 번호를 확인할 것**.
--
