...
--   · 결과를 미리 정하고 애니메이션만 재생하지 않는다. 판 번호 공개 + 같은 판 번호 재현 제공
--
...
VALUES
    ('marble-race', '구슬 레이스',
     '커피 사는 사람 정하기처럼 여럿이서 뭔가 정할 때 쓰는 구슬 경주. 이름을 넣고 출발하면 구슬이 못밭과 범퍼, 회전 십자, 시소, 컨베이어를 지나 실제로 굴러 내려가고, 결승선을 넘는 순서가 그대로 결과가 된다. 꼴찌가 걸린다 / 1등이 뽑힌다 / 몇 등 지정 / 순서 정하기 네 가지로 쓸 수 있고, 이름 뒤에 x3 을 붙이면 그 사람 구슬이 세 개가 되어 그만큼 확률이 올라간다. 결과를 미리 정해 두고 영상만 재생하는 방식이 아니라 물리를 실제로 돌린 결과이며, 판마다 판 번호를 화면에 띄우고 같은 판 번호로 다시 돌리면 똑같은 결과가 나오는 것으로 그 사실을 확인할 수 있다. 구슬은 반지름·질량·반발계수가 모두 같고 출발 자리는 매 판 무작위로 섞는다. 참가자 이름은 이 기기에만 저장되고 어디로도 전송하지 않는다. 앱 안에 돈이나 포인트는 없다. 맵 3종, 그래픽·사운드 전부 절차 생성(1080×1440).',
     'Marble Race',
