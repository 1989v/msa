# AGENTS.md — Commerce Platform (MSA)

**On conflict**: AGENTS.md wins over plugin/skill defaults.
...
| Study notes (백엔드 시니어 학습 노트) | `study/` — 19 주제 / 355 파일 / ~107K 줄. entry: `study/AGENTS.md`, master index: `study/docs/00-INDEX.md`, ADR 후보: `study/docs/00-ADR-CANDIDATES.md` |

### 서비스별 문서

각 서비스 디렉토리에 `AGENTS.md` + `docs/`가 있다. 서비스 작업 시 자동 로드됨.

| 서비스 | AGENTS.md | 비고 |
|--------|-----------|------|
| product | `product/AGENTS.md` | SSOT, Kafka 발행 |
| order | `order/AGENTS.md` | 결제 연동, 상태 전이 |
| search | `search/AGENTS.md` | OpenSearch 인덱싱, 4개 모듈 |
| gateway | `gateway/AGENTS.md` | 인증 필터, Rate Limiting, K8s DNS 라우팅 |
| common | `common/AGENTS.md` | 공유 라이브러리 |
| analytics | `analytics/AGENTS.md` | 이벤트 수집, 스코어 산출 (Kafka Streams + ClickHouse) |
| experiment | `experiment/AGENTS.md` | A/B 테스트 플랫폼 |
| member | `member/AGENTS.md` | 회원 식별, 프로필 관리 (최소 개인정보) |
| wishlist | `wishlist/AGENTS.md` | 상품 위시리스트 (회원별) |
| quant | `quant/AGENTS.md` | 통합 트레이딩 플랫폼 — sealed Strategy(Tranche/Signal/Hybrid) + 차트 분석 + 입문자 지표 학습 CMS + Phase 3 실매매 (ADR-0033/0036/0037, Phase 3 코어 구현 완료, 거래소 어댑터 4종 wire-up 후 Beta) |
| auth | (AGENTS.md 미작성) | OAuth 인증, RBAC (ROLE_USER/SELLER/ADMIN) — 서비스 코드 존재 |
| gifticon | (AGENTS.md 미작성) | 기프티콘 관리, 공유 그룹 — 서비스 코드 존재 |
| code-dictionary | `code-dictionary/AGENTS.md` | IT 개념 사전, OpenSearch 검색, 트리맵/그래프 시각화, 어드민 CRUD + 포트폴리오 카드. FE 는 portal-fe 단일 SPA 의 메인 콘텐츠로 통합 (2026-05-05, scroll anchor 기반) |
| inventory | (AGENTS.md 미작성) | 재고 관리, 예약 — 서비스 코드 존재 |
| fulfillment | (AGENTS.md 미작성) | 주문 풀필먼트 — 서비스 코드 존재 |
| warehouse | (AGENTS.md 미작성) | 창고 관리 — 서비스 코드 존재 |
| chatbot | (AGENTS.md 미작성) | 대화형 AI — 서비스 코드 존재 |
| admin | (AGENTS.md 미작성) | 백오피스 관리 도구 (FE only) — admin/ 디렉토리 존재 |
| place | `place/AGENTS.md` | 행정 지리 계층(대륙/국가/광역/도시) + POI, OpenSearch geo_distance 근처검색. 오픈데이터(GeoNames/상가정보) 적재 (ADR-0056) |
...
| 레포 | 형태 | 위치 | 설명 |
|------|------|------|------|
| `ai` | submodule (`1989v/ai`) | `ai/` (msa 내부) | Codex 플러그인 모노레포 — hns / ai-debugger / private-repo / content-analyzer 4종 (`ai/AGENTS.md`) |
