...
- **백업/복구**: **운영(oci-arm)은 `k8s/base/db-backup/` 의 MySQL 논리 백업이 전부다** — 매일 03:00 UTC mysqldump + gzip, 보관 7일, RPO 24시간. `local-path` 라 백업 PVC 가 데이터와 같은 디스크에 놓이므로 **덤프 후 OCI Object Storage(`msa-db-backup`)로 한 부 더 올린다** — PAR 쓰기 전용 + `Content-MD5` 서버 대조(2026-09-13~). **PAR 만료 2027-09-13** 이고 만료되면 업로드가 404 로 죽으므로 Job 이 실패하게 해 뒀다 — 만료를 방치하면 로컬 사본만 쌓이면서 오프노드가 있다고 믿게 된다. 단일 AD 리전이라 **리전 손실은 여전히 못 막는다**. 복구 시 `--default-character-set=utf8mb4` 필수 (빼면 한글이 깨진 채 행 수만 맞는다) → `k8s/base/db-backup/README.md`. XtraBackup + Binlog PITR(RPO ~0)는 `docker/backup/` · `k8s/infra/prod/backup/` 에 있는 **설계안이고 운영에 배선돼 있지 않다** — 2026-09-12 확인
...
- **카드 디스펜서 / 뽑기 (2026-09-03)**: 회전판에 옆으로 꽂힌 카드 중 정면 것이 일어나는 장치. 메인 서비스 섹션 넷과 place·game·blog·shop 의 "뽑기"(지금 건 필터에서 무작위 하나)가 같은 것을 쓴다. 장치는 npm 패키지 **`card-dispenser`**(의존성 0, 소스는 별도 레포 `1989v/card-dispenser`), 판 위 배치는 `components/dispenser`. 장치를 고치려면 그 레포에서 고쳐 게시하고 여기 버전을 올린다 — 사본을 만들지 않는다. **터치 기기에서 스크롤은 판을 돌리지 않는다**(판은 멈춰 있고 뽑기가 돌린다) → 장치 문서는 `github.com/1989v/card-dispenser` (패키지 README), 홈 개편 → `docs/specs/2026-09-03-home-showcase-card-dispenser.md`
...
- **하네스 변경 원장 (필수)**: CLAUDE.md·memory·훅·스킬을 바꾸면 `docs/changelog/harness-changelog.md` 에 한 줄. **채택뿐 아니라 기각·되돌림·보류도 행을 갖는다** — 안 적으면 같은 제안이 몇 달 뒤 다시 올라오고 그때는 왜 안 받았는지 아무도 모른다. **되돌릴 때 옛 규칙의 이유를 지우지 않는다**(규칙은 되돌아가도 지식은 남긴다). 실제로 2026-09-07 에 넣은 라우팅 규칙이 하루 만에 hns 0.16.0 과 충돌해 철회됐다
- **에이전트 훅·지식베이스**: `.claude/hooks/hns/` 훅 6종 — 세션 복구 · 컴팩션 보존 · **커밋 범위 노출**(스테이지된 서브모듈 포인터가 뒤로 가면 차단, 나머지는 알림) · **커밋 전 바뀐 모듈 컴파일**(실패 시 거부) · 편집 후 린트(이 레포는 `HNS_LINT_CMD=""` 라 무동작) · 완료 주장 증거 게이트(tier `enforce`, `.claude/hns-hooks.env`). 게이트가 막으면 컴파일을 고친다. 레포 밖 결정·함정은 `HNS_KB_PATH`(옵시디언 `1989v` 볼트)를 `hns:kb` 가 읽기 전용으로 조회한다 → `docs/adr/ADR-0091-agent-harness-hooks-kb.md`
