...
val approvedPods: Set<String> = setOf(
    ...
    // 폴드 호스트
    "commerce", "account", "engagement", "sideapp", "content", "atlas",
...
fun renderTopologyShell(pods: List<Pod>): String {
    val jvm = pods.filter { it.name != "agent-viewer" }.map { it.name }.sorted()
    val lines = StringBuilder()
    lines.append("# 생성물 — 손으로 고치지 않는다. ./gradlew generateTopology 가 만든다.\n")
    lines.append("# 원본은 settings.gradle.kts 의 :{x}:app 과 호스트 앱의 scanBasePackages 다.\n\n")
    lines.append("TOPOLOGY_ALL_JVM=\"${jvm.joinToString(" ")}\"\n\n")

    lines.append("# 변경 경로 → 재빌드할 파드\n")
    lines.append("topology_pod_for_path() {\n  case \"\$1\" in\n")
    pods.sortedBy { it.name }.forEach { pod ->
        val owned = (pod.domains + pod.name).distinct().sorted()
        val pattern = owned.joinToString("|") { "$it/*" }
        lines.append("    $pattern) echo \"${pod.name}\" ;;\n")
    }
    lines.append("  esac\n}\n\n")

    lines.append("# 파드 → 돌려야 할 테스트 태스크\n")
    ...
    pods.sortedBy { it.name }.forEach { pod ->
        ...
        lines.append("    ${pod.name}) echo \"$tasks\" ;;\n")
    }
    lines.append("  esac\n}\n")
    return lines.toString()
}

fun renderTopologyProps(pods: List<Pod>): String = buildString {
    append("# 생성물 — 손으로 고치지 않는다. ./gradlew generateTopology 가 만든다.\n")
    pods.sortedBy { it.name }.forEach { append("pod.${it.name}.mainClass=${it.mainClass}\n") }
    pods.sortedBy { it.name }.forEach { append("pod.${it.name}.domains=${it.domains.joinToString(",")}\n") }
}

val generateTopology by tasks.registering {
    group = "build"
    description = "폴드 배치 파생물을 소스에서 다시 만든다 (CI 스크립트 · jib 매핑)"
    doLast {
        val pods = TopologyModel.read(rootProject.projectDir)
        rootProject.file(topologyShell).apply { parentFile.mkdirs() }.writeText(renderTopologyShell(pods))
        rootProject.file(topologyProps).apply { parentFile.mkdirs() }.writeText(renderTopologyProps(pods))
        logger.lifecycle("토폴로지 생성: 파드 ${pods.size} — $topologyShell, $topologyProps")
    }
}

val verifyTopologyGenerated by tasks.registering {
    group = "verification"
    description = "생성물이 커밋본과 같은지 (doc-index.lock 과 같은 방식)"
    doLast {
        val pods = TopologyModel.read(rootProject.projectDir)
        val mismatched = mapOf(
            topologyShell to renderTopologyShell(pods),
            topologyProps to renderTopologyProps(pods),
        ).filter { (path, expected) ->
            val f = rootProject.file(path)
            !f.exists() || f.readText() != expected
        }.keys

        if (mismatched.isNotEmpty()) {
            throw GradleException(
                mismatched.joinToString(
                    prefix = "토폴로지 생성물이 소스와 다르다:\n  ",
                    separator = "\n  ",
                    postfix = "\n\n./gradlew generateTopology 로 다시 만들고 커밋할 것. " +
                        "이 파일들은 손으로 고치지 않는다.",
                ),
            )
        }
